...
int conn_select_fd(struct conn_t *conn, select_ctx * sctx)
{
	...
	if (!conn->sock)
		return -1;
	if (conn->write_buf && conn->write_pos < conn->write_buf->slen)
		evts |= SELECT_WRITE;
...
static int _conn_bstring_readhandler(struct conn_t *conn, void *ctx)
{
	...
	ret = safe_read(conn->sock,
			data->data + data->slen, data->mlen - data->slen);
