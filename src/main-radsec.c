...
static int connect_ssl(struct in_addr *addr, int port)
{
	...
	server.conn.sslcon =
	    openssl_connect_fd(server.env, server.conn.sock, 10);
	if (!server.conn.sslcon) {
		log_err(errno, "Failed to connect to %s:%d", inet_ntoa(*addr),
			port);
		return -1;
	}
	return 0;
	...
	if (!server.conn.connected) {
		if (connect_ssl(&_options.radiusserver1, 2083)) {
			...
			if (connect_ssl(&_options.radiusserver2, 2083)) {
				log_err(errno,
					"Could not connect to RadSec server %s!",
	...
	if (l == 4) {
		...
		if (l == len) {
			...
			case RADIUS_CODE_ACCESS_CHALLENGE:
				...
				radius_reply(server.radius_auth, &server.pack,
					     &server.auth_peer);
				break;
			case RADIUS_CODE_ACCOUNTING_RESPONSE:
				log_dbg("reply acct %d", len);
				radius_reply(server.radius_acct, &server.pack,
					     &server.acct_peer);
				break;
			...
			case RADIUS_CODE_STATUS_REQUEST:
				if (_options.coaport) {
					...
					radius_reply(server.radius_cli,
						     &server.pack,
						     &server.acct_peer);
				}
				break;
...
int main(int argc, char **argv)
{
	...
	if (radius_new(&server.radius_auth, &radiuslisten,
		       _options.radiusauthport ? _options.
		       radiusauthport : RADIUS_AUTHPORT, 0, 0)) {
		log_err(0, "Failed to create radius");
		return -1;
	}

	if (radius_new(&server.radius_acct, &radiuslisten,
		       _options.radiusacctport ? _options.
		       radiusacctport : RADIUS_ACCTPORT, 0, 0)) {
		log_err(0, "Failed to create radius");
		return -1;
	}
		...
		default:
			if (status > 0) {
				...
				if (FD_ISSET(server.radius_auth->fd, &fdread)) {
					...
					if ((status =
					     recvfrom(server.radius_auth->fd,
						      &radius_pack,
						      sizeof(radius_pack), 0,
				...
				if (FD_ISSET(server.radius_acct->fd, &fdread)) {
					...
					if ((status =
					     recvfrom(server.radius_acct->fd,
						      &radius_pack,
						      sizeof(radius_pack), 0,
						      (struct sockaddr *)&addr,
						      &fromlen)) <= 0) {
				...
				if (server.radius_cli) {
					if (FD_ISSET
					    (server.radius_cli->fd, &fdread)) {
						radius_decaps(server.radius_cli,
							      0);
					}
				}
