...
int redir_hextochar(unsigned char *src, int slen, unsigned char *dst, int len)
{
	...
	for (n = 0; n < len; n++) {
		...
		if (i < slen) {
			...
			default:
				log_err(errno,
					"HEX conversion failed (src='%s', len=%d, n=%d, y=%d)!",
					src, len, n, y);
				return -1;
			}
		}
			     ...
			     uint8_t * hismac, struct in_addr *hisip, char *amp)
{
	...
	bassignformat(str, "%s%sres=%s%suamip=%s%suamport=%d",
		      ...
		      inet_ntoa(redir->addr), amp, redir->port);
	...
	if (_options.nasmac)
		...
		bassignformat(bt, "%.2X-%.2X-%.2X-%.2X-%.2X-%.2X",
			      redir->nas_hwaddr[0], redir->nas_hwaddr[1],
			      redir->nas_hwaddr[2], redir->nas_hwaddr[3],
			      redir->nas_hwaddr[4], redir->nas_hwaddr[5]);
	...
	if (conn->s_state.location[0]) {
		...
		log_dbg("found %.*s", bt->slen, bt->data);
	...
	if (_options.uamuissl && _options.uamuiport) {
		...
		if (_options.uamaliasname && _options.domain) {
			bassignformat(bt, "https://%s.%s:%d/",
				      ...
				      _options.domain, _options.uamuiport);
		} else {
...
static void redir_xmlchilli_reply(struct redir_t *redir,
				  struct redir_conn_t *conn, int res,
				  long int timeleft, char *hexchal, char *reply,
				  char *redirurl, bstring b)
{
	bstring bt = bfromcstr("");;
	...
	case REDIR_STATUS:
		if (conn->s_state.authenticated == 1) {
			...
			bassignformat(bt,
				      "<MaxInputOctets>%d</MaxInputOctets>\r\n",
				      conn->s_params.maxinputoctets);
			bconcat(b, bt);

			bassignformat(bt,
				      "<MaxOutputOctets>%d</MaxOutputOctets>\r\n",
				      conn->s_params.maxoutputoctets);
			bconcat(b, bt);

			bassignformat(bt,
				      "<MaxTotalOctets>%d</MaxTotalOctets>\r\n",
	...
	default:
		log_err(0,
			"redir_wispr1_reply: Unhandled response code in switch: %d",
			res);
	}
	bcatcstr(b, "</ChilliSpotSession>\r\n" "-->\r\n");
	bdestroy(bt);
			...
			char *reply, char *redirurl, bstring b)
{
	...
	case REDIR_NOTYET:
		...
		bassignformat(bt,
			      "<LoginURL>%s%sres=wispr&amp;uamip=%s&amp;uamport=%d&amp;challenge=%s</LoginURL>\r\n",
			      _options.wisprlogin ? _options.
			      wisprlogin : redir->url,
			      strchr(_options.wisprlogin ? _options.
				     wisprlogin : redir->url,
				     '?') ? "&amp;" : "?",
	...
	case REDIR_SUCCESS:
		...
		bcatcstr(b, "<MaxSessionTime>3600</MaxSessionTime>\r\n");	/* Max time of the session in seconds */

		bcatcstr(b, "</AuthenticationPollReply>\r\n");
		break;

	case REDIR_LOGOFF:
		bcatcstr(b, "<LogoffReply>\r\n" "<MessageType>130</MessageType>\r\n"	/* logoff notification */
	...
	case REDIR_STATUS:
		...
		if (conn->s_state.authenticated != 1) {
			bcatcstr(b, "<ResponseCode>150</ResponseCode>\r\n"	/*logoff succeeded */
				 "<ReplyMessage>Not logged on</ReplyMessage>\r\n");
		} else {
			bcatcstr(b, "<ResponseCode>50</ResponseCode>\r\n"	/* login succeeded */
				 "<ReplyMessage>Already logged on</ReplyMessage>\r\n");
		}
		bcatcstr(b, "</AuthenticationPollReply>\r\n");
		break;
			...
			char *reply, char *redirurl, bstring b)
{
	...
	case REDIR_NOTYET:
		...
		if (!_options.no_wispr1)
			bcatcstr(b, "<VersionLow>1.0</VersionLow>\r\n" "<AccessProcedure>1.0</AccessProcedure>\r\n");	/* Indicate support for WISPr 1.0 */
		else
			bcatcstr(b, "<VersionLow>2.0</VersionLow>\r\n");	/* Indicate only support for WISPr 2.0 */

		if (_options.radiuslocationid) {
			bassignformat(bt,
				      "<AccessLocation>CDATA[[%s]]</AccessLocation>\r\n",
				      _options.radiuslocationid);
			bconcat(b, bt);
		}
		...
		if (_options.radiuslocationname) {
			bassignformat(bt,
				      "<LocationName>CDATA[[%s]]</LocationName>\r\n",
				      _options.radiuslocationname);
			bconcat(b, bt);
		}
		...
		if (!base64encoder(&(conn->authdata.v.eapmsg),
			...
			log_dbg
			    ("Base64 encoding of eap identity request failed");
		}
		bcatcstr(b, "</Redirect>\r\n");
		break;
	...
	case REDIR_FAILED_REJECT:
		...
		bcatcstr(b, "<ResponseCode>100</ResponseCode>\r\n");	/* login failed (Access REJECT) */
	...
	case REDIR_FAILED_NOROUTE:
		...
		bcatcstr(b, "<ResponseCode>105</ResponseCode>\r\n");	/* RADIUS serveur error/timeout */
	...
	case REDIR_FAILED_MTU:
		...
		bcatcstr(b, "<ResponseCode>253</ResponseCode>\r\n");	/* MTU is too big */

		bcatcstr(b,
			 "<ReplyMessage>AAA MTU is too big</ReplyMessage>\r\n");
	...
	case REDIR_FAILED_TIMEOUT:
		...
		bcatcstr(b, "<ResponseCode>102</ResponseCode>\r\n");	/* RADIUS serveur timeout */
		...
		if (reply) {
			...
			bcatcstr(b,
				 "<ReplyMessage>Radius timeout</ReplyMessage>\r\n");
		}
	...
	case REDIR_ERROR_PROTOCOL:
		...
		bcatcstr(b, "<ResponseCode>254</ResponseCode>\r\n");	/* WISPr 2.0 protocol error */
		...
		if (reply) {
			...
			bcatcstr(b,
				 "<ReplyMessage>WISPr 2.0 protocol error</ReplyMessage>\r\n");
		}
	...
	case REDIR_FAILED_OTHER:
		...
		bcatcstr(b, "<ResponseCode>255</ResponseCode>\r\n");	/* RADIUS serveur error */
	...
	case REDIR_CHALLENGE:
		...
		bassignformat(bt,
			      "<LoginURL>%s%sres=wispr&amp;uamip=%s&amp;continue=1&amp;uamport=%d&amp;challenge=%s</LoginURL>\r\n",
			      _options.wisprlogin ? _options.
			      wisprlogin : redir->url,
			      strchr(_options.wisprlogin ? _options.
				     wisprlogin : redir->url,
	...
	case REDIR_SUCCESS:
		...
		if (conn->authdata.type == REDIR_AUTH_EAP) {
			if (!base64encoder(&(conn->authdata.v.eapmsg),
				...
				log_dbg
				    ("Base64 encoding of radius eap message failed");
			}
		}
...
static int redir_json_reply(struct redir_t *redir, int res,
			    struct redir_conn_t *conn, char *hexchal,
			    char *userurl, char *redirurl, uint8_t * hismac,
			    struct in_addr *hisip, char *reply, char *qs,
			    bstring s)
{
...
#define FLG_redir 16
	...
	int splash =
	    (conn->s_params.flags & REQUIRE_UAM_SPLASH) == REQUIRE_UAM_SPLASH;
	...
	bcatcstr(s, "\r\nContent-Type: ");
	if (tmp->slen)
		bcatcstr(s, "text/javascript");
	else
		bcatcstr(s, "application/json");
...
ssize_t
tcp_write_timeout(int timeout, struct redir_socket_t *sock, char *buf,
		  size_t len)
{
	fd_set fdset;
	struct timeval tv;
	...
	if (resp) {
		...
		if (url) {
			...
			redir_buildurl(conn, bt, redir, resp, timeleft, hexchal,
				       uid, userurl, reply, redirurl, hismac,
				       hisip);
			bconcat(buffer, bt);
			bdestroy(bt);
		}
		...
		if (res == REDIR_NOTYET) {
			...
			if (!_options.no_wispr1 && _options.no_wispr2)
				redir_wispr1_reply(redir, conn, REDIR_NOTYET,
						   timeleft, hexchal, reply,
						   redirurl, bbody);

			else if (!_options.no_wispr2)
				redir_wispr2_reply(redir, conn, REDIR_NOTYET,
						   timeleft, hexchal, reply,
						   redirurl, bbody);

#ifdef ENABLE_CHILLIXML
			if (_options.chillixml)
				redir_xmlchilli_reply(redir, conn, REDIR_NOTYET,
						      timeleft, hexchal, reply,
						      redirurl, bbody);
#endif
			...
			if (conn->s_state.redir.
			    uamprotocol & REDIR_UAMPROT_WISPR2)
				redir_wispr2_reply(redir, conn, res, timeleft,
						   hexchal, reply, redirurl,
						   bbody);
			else
				redir_wispr1_reply(redir, conn, res, timeleft,
						   hexchal, reply, redirurl,
						   bbody);

#ifdef ENABLE_CHILLIXML
			if (conn->s_state.redir.uamprotocol ==
			    REDIR_UAMPROT_CHILLI)
				redir_xmlchilli_reply(redir, conn, res,
						      timeleft, hexchal, reply,
						      redirurl, bbody);
#endif
		}
...
int redir_listen(struct redir_t *redir)
{
	...
	for (n = 0; n < 2 && redir->fd[n]; n++) {
		...
		optval = 1;
		if (setsockopt
		    (redir->fd[n], SOL_SOCKET, SO_REUSEADDR, &optval,
		     sizeof(optval))) {
		...
		optval = 1;
		if (setsockopt
		    (redir->fd[n], SOL_SOCKET, SO_REUSEPORT, &optval,
		     sizeof(optval))) {
		...
		while (bind
		       (redir->fd[n], (struct sockaddr *)&address,
			sizeof(address)) == -1) {
			if ((EADDRINUSE == errno) && (10 > tries++)) {
				log_warn(errno,
					 "IP: %s Port: %d - Waiting for retry.",
...
int redir_ipc(struct redir_t *redir)
{
	...
	if ((sock = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
		...
		statedir_file(filedest, sizeof(filedest), _options.unixipc,
			      "chilli.ipc");

		local.sun_family = AF_UNIX;

		safe_strncpy(local.sun_path, filedest, sizeof(local.sun_path));
		unlink(local.sun_path);

		if (bind(sock, (struct sockaddr *)&local,
			 sizeof(struct sockaddr_un)) == -1) {
			log_err(errno, "could bind UNIX Socket to %s!",
				filedest);
			safe_close(sock);
			sock = -1;
		} else {
...
static int redir_getreq(struct redir_t *redir, struct redir_socket_t *sock,
			...
			struct redir_httpreq_t *httpreq, redir_request * rreq)
{
	int fd = sock->fd[0];
	fd_set fds;
	...
	while (!done
	       && (redir->starttime + REDIR_HTTP_MAX_TIME) > mainclock_now()) {
		...
		if (!read_waiting && forked) {
			...
			do {
				...
				status =
				    select(fd + 1, &fds, NULL, NULL, &idleTime);
		...
		if (read_waiting || !forked) {
			...
			if (buflen + 2 >= sizeof(buffer)) {	/* ensure space for a least one more byte + null */
				log_err(0, "Too much data in http request! %d",
					(int)buflen);
				return -1;
			}
			...
			if (recvlen == 0)
				...
				bcatblk(httpreq->data_in, buffer + buflen,
					recvlen);

				if (httpreq->allow_post &&
				    httpreq->data_in->slen > 4 &&
				    strncmp((char *)httpreq->data_in->data,
					    "POST", 4))
		...
		if (httpreq->data_in && !forked) {
			...
			if (httpreq->data_in->slen >= sizeof(buffer)) {
				log_err(0, "buffer too long (%d)",
					httpreq->data_in->slen);
				return -1;
			} else {
			...
			*eol = 0;
			...
			if (lines++ == 0) {	/* first line */
				...
				if (!strncmp("GET ", p1, 4)) {
					...
					p1 += 5;
				} else if (httpreq->allow_post
					   && !strncmp("POST ", p1, 5)) {
					p1 += 5;
					httpreq->is_post = 1;
				} else {
					log_dbg("Unhandled http request: %s %d",
						p1, _options.uamallowpost);
					return -1;
				}

				while (*p1 == ' ')
					p1++;	/* Advance through additional white space */
				...
				*p2 = 0;
				...
				if (!strncmp(path, "json/", 5)
					...
					log_dbg("The (json format) path: %s",
						path);
				}
#endif

				if ((!strcmp(path, "logon"))
				    || (!strcmp(path, "login")))
					conn->type = REDIR_LOGIN;
				else if ((!strcmp(path, "logoff"))
					 || (!strcmp(path, "logout")))
					conn->type = REDIR_LOGOUT;
				else if (!strncmp(path, "www/", 4)
					 && strlen(path) > 4)
					conn->type = REDIR_WWW;
				...
				if (qs_delim == '?') {
					...
					if (p2) {
						*p2 = 0;
						...
						safe_strncpy(httpreq->qs, p1,
							     sizeof(httpreq->
								    qs));

#if(_debug_ > 1)
						log_dbg("Query string: %s",
							httpreq->qs);
#endif
					}
				}
				...
				if (!strncasecmp(buffer, "Host:", 5)) {
					...
					log_dbg("Content-Length: %s", p);
					 ...
					 (buffer, "User-Agent:", 11)) {
					p = buffer + 11;
					while (*p && isspace((int)*p))
						p++;
					safe_strncpy(conn->s_state.redir.
						     useragent, p,
						     sizeof(conn->s_state.redir.
							    useragent));
#if(_debug_ > 1)
					log_dbg("User-Agent: %s",
						conn->s_state.redir.useragent);
#endif
				}
#endif
#ifdef ENABLE_ACCEPTLANGUAGE
				else if (!strncasecmp
					 (buffer, "Accept-Language:", 16)) {
					p = buffer + 16;
					while (*p && isspace((int)*p))
						p++;
					safe_strncpy(conn->s_state.redir.
					...
					while (*p && isspace((int)*p))
						p++;
					safe_strncpy(conn->httpcookie, p,
						     sizeof(conn->httpcookie));
#if(_debug_ > 1)
	...
	case REDIR_LOGIN:
		{
			...
			if (!redir_getparam
			    (redir, httpreq->qs, "WISPrVersion", bt)) {
				char rxversion[10];
				btrimws(bt);
				bstrtocstr(bt, rxversion, sizeof(rxversion));
				if (!strcmp(rxversion, "2.0")) {
					conn->s_state.redir.uamprotocol =
					    REDIR_UAMPROT_WISPR2;
					log_dbg
					    ("using uamprotocol: WISPr 2.0 (%d)",
					     conn->s_state.redir.uamprotocol);
				} else {
					conn->s_state.redir.uamprotocol =
					    REDIR_UAMPROT_WISPR1;
					log_dbg
					    ("using uamprotocol: WISPr 1.0 (%d)",
					     conn->s_state.redir.uamprotocol);
				}
			} else {
				conn->s_state.redir.uamprotocol =
				    REDIR_UAMPROT_WISPR1;
				log_dbg("using uamprotocol: WISPr 1.0 (%d)",
					conn->s_state.redir.uamprotocol);
			}

			if (!redir_getparam
			    (redir, httpreq->qs, "ntresponse", bt)) {
				conn->authdata.type = REDIR_AUTH_MSCHAPv2;
				redir_hextochar(bt->data, bt->slen,
						conn->authdata.v.chapmsg.
						password, RADIUS_CHAPSIZE);
			} else
			    if (!redir_getparam
				(redir, httpreq->qs, "response", bt)) {
				...
				if (!redir_getparam
				    (redir, httpreq->qs, "ident", bt)
				    && bt->slen)
					conn->authdata.v.chapmsg.identity =
					    atoi((char *)bt->data);
				else
					conn->authdata.v.chapmsg.identity = 0;
			} else
			    if (!redir_getparam
				(redir, httpreq->qs, "password", bt)) {
				...
				redir_hextochar(bt->data, bt->slen,
						conn->authdata.v.papmsg.
						password,
						conn->authdata.v.papmsg.len);
			} else {
				if ((conn->s_state.redir.uamprotocol ==
				     REDIR_UAMPROT_WISPR2)
				    && !redir_getparam(redir, httpreq->qs,
						       "WISPrEAPMsg", bt)) {
					int rc;

					if (conn->authdata.type !=
					    REDIR_AUTH_NONE) {
						log_err(0,
							"Request contains both password and eap message");
						conn->response =
						    REDIR_ERROR_PROTOCOL;
						bdestroy(bt);
						return 0;
					}
					...
					if (rc == 1) {
						log_err(0,
							"EAP message is too big, max allowed 1265 bytes");
						conn->response =
						    REDIR_FAILED_MTU;
						bdestroy(bt);
						return 0;
					...
					if (_options.debug) {
						char buffer[conn->authdata.v.
							    eapmsg.len * 2 + 1];
						bytetohex(conn->authdata.v.
							  eapmsg.data,
							  conn->authdata.v.
							  eapmsg.len, buffer,
							  conn->authdata.v.
							  eapmsg.len * 2 + 1);
						log_dbg
						    ("decoded eap message from WISPr request (%d): %s",
				     ...
				     void *cbp)
{
	...
	if (!radius_getattr(pack, &attr, RADIUS_ATTR_CLASS, 0, 0, 0)) {
		...
		if (_options.debug) {
			...
			bytetohex(conn->s_state.redir.classbuf,
				  conn->s_state.redir.classlen, buffer,
				  conn->s_state.redir.classlen * 2 + 1);
			log_dbg("+attribute Class (%d bytes): %s",
				conn->s_state.redir.classlen, buffer);
		}
	} else {
	...
	if (!radius_getattr
	    (pack, &attr, RADIUS_ATTR_CHARGEABLE_USER_IDENTITY, 0, 0, 0)) {
		...
		if (_options.debug) {
			...
			bytetohex(conn->s_state.redir.cuibuf,
				  conn->s_state.redir.cuilen, buffer,
				  conn->s_state.redir.cuilen * 2 + 1);
			log_dbg("+attribute CUI (%d bytes): %s",
				conn->s_state.redir.cuilen, buffer);
		}
	} else {
	...
	if (!radius_getattr(pack, &attr, RADIUS_ATTR_STATE, 0, 0, 0)) {
		...
		log_dbg("+attribute State (%d bytes)",
	...
	do {
		...
		if (!radius_getattr(pack, &attr, RADIUS_ATTR_EAP_MESSAGE, 0, 0,
				    instance++)) {
			if ((conn->authdata.v.eapmsg.len + (size_t) attr->l -
			     2) > MAX_EAP_LEN) {
				log_err(0,
					"received EAP message from Radius packet is too big");
				conn->authdata.v.eapmsg.len = 0;
				return -1;
			}
			memcpy(conn->authdata.v.eapmsg.data +
			       conn->authdata.v.eapmsg.len, attr->v.t,
			       (size_t) attr->l - 2);
	...
	if (_options.debug) {
		...
		if (!radius_getattr(pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
			...
			bytetosphex(&attr->t, attr->l, hexString,
				    sizeof(hexString));

			/* Now decode the MPPE attribue */
			if (!radius_keydecode
			    (radius, dstbuffer, RADIUS_ATTR_VLEN, &dstlen,
			     (uint8_t *) & attr->v.t, attr->l - 2,
			     pack_req->authenticator, radius->secret,
			     radius->secretlen) != 0) {
				bytetosphex(dstbuffer, dstlen, hexString,
					    sizeof(hexString));
				log_dbg
				    ("plainstring MPPE_SEND_KEY: len %d key %s",
				     dstlen, hexString);
			} else {
				log_dbg
				    ("received radius MPPE_SEND_KEY attribute (%d bytes): %s",
				     attr->l, hexString);
				log_err(0,
					"Decryption of MPPE_SEND_KEY failed");
			}
		}
		if (!radius_getattr(pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
				    RADIUS_VENDOR_MS,
			...
			if (!radius_keydecode
				...
				bytetosphex(dstbuffer, dstlen, hexString,
					    sizeof(hexString));
				log_dbg
				    ("plainstring MPPE_RECV_KEY: len %d key %s",
				     dstlen, hexString);
			} else {
				log_dbg
				    ("received radius MPPE_RECV_KEY attribute (%d bytes): %s",
				     attr->l, hexString);
				log_err(0,
					"Decryption of MPPE_RECV_KEY failed");
			}
		}
	}
	...
	if (conn->s_params.sessionterminatetime) {
		...
		if (timenow > conn->s_params.sessionterminatetime) {
			conn->response = REDIR_FAILED_OTHER;
			log_warn(0,
				 "WISPr-Session-Terminate-Time in the past received: %s",
				 attrs);
			return 0;
		}
	}
	...
	case RADIUS_CODE_ACCESS_REJECT:
		...
		break;
	default:
		log_err(0, "Unsupported radius access reply code %d",
			pack->code);
		return -1;
	}
	return 0;
			...
			struct redir_conn_t *conn, char reauth)
{
	...
	if (conn->lang[0])
		radius_addattr(radius, &radius_pack,
			       ...
			       RADIUS_ATTR_CHILLISPOT_LANG, 0,
			       (uint8_t *) conn->lang, strlen(conn->lang));
	...
	if (redir->secret && *redir->secret) {
		...
		MD5Update(&context, (uint8_t *) redir->secret,
			  strlen(redir->secret));
		MD5Final(chap_challenge, &context);
	} else {
		memcpy(chap_challenge, conn->s_state.redir.uamchal,
		       REDIR_MD5LEN);
	}
	...
	case REDIR_AUTH_PAP:
		...
		if (_options.mschapv2) {
			...
			GenerateNTResponse(chap_challenge, chap_challenge,
					   (u_char *) conn->s_state.redir.
					   username,
					   strlen(conn->s_state.redir.username),
					   user_password,
					   strlen((char *)user_password),
					   ntresponse);
			...
			radius_addattr(radius, &radius_pack,
				       RADIUS_ATTR_VENDOR_SPECIFIC,
				       RADIUS_VENDOR_MS,
				       RADIUS_ATTR_MS_CHAP_CHALLENGE, 0,
				       chap_challenge, 16);

			radius_addattr(radius, &radius_pack,
				       RADIUS_ATTR_VENDOR_SPECIFIC,
				       RADIUS_VENDOR_MS,
				       RADIUS_ATTR_MS_CHAP2_RESPONSE, 0,
				       response, 50);
		} else {
#endif
			radius_addattr(radius, &radius_pack,
				       RADIUS_ATTR_USER_PASSWORD, 0, 0, 0,
	...
	case REDIR_AUTH_CHAP:
		...
		memcpy(chap_password + 1, conn->authdata.v.chapmsg.password,
		       REDIR_MD5LEN);
		radius_addattr(radius, &radius_pack, RADIUS_ATTR_CHAP_CHALLENGE,
			       0, 0, 0, chap_challenge, REDIR_MD5LEN);
		radius_addattr(radius, &radius_pack, RADIUS_ATTR_CHAP_PASSWORD,
			       0, 0, 0, chap_password, REDIR_MD5LEN + 1);
		break;
	case REDIR_AUTH_MSCHAPv2:
		{
			...
			/* peer challenge - same as auth challenge */
			memcpy(response + 2, chap_challenge, 16);
			memcpy(response + 26, conn->authdata.v.chapmsg.password,
			       24);
	...
	case REDIR_AUTH_EAP:
		/* Add one or more EAP-MSG attribute in Radius packet  */
		{
			...
			while (offset < conn->authdata.v.eapmsg.len) {
				size_t eaplen = 0;
				if ((conn->authdata.v.eapmsg.len - offset) >
				    RADIUS_ATTR_VLEN)
					eaplen = RADIUS_ATTR_VLEN;
				else
					eaplen =
					    conn->authdata.v.eapmsg.len -
					    offset;

				if (radius_addattr(radius, &radius_pack,
						   RADIUS_ATTR_EAP_MESSAGE, 0,
						   0, 0,
						   conn->authdata.v.eapmsg.
						   data + offset, eaplen)) {
					log_err(0,
						"EAP message segmentation in EAP attributes failed");
					radius_free(radius);
					return -1;
				}
				offset += eaplen;
			...
			if (_options.debug) {
				char buffer[conn->authdata.v.eapmsg.len * 2 +
					    1];
				bytetohex(conn->authdata.v.eapmsg.data,
					  conn->authdata.v.eapmsg.len, buffer,
					  conn->authdata.v.eapmsg.len * 2 + 1);
...
int is_local_user(struct redir_t *redir, struct redir_conn_t *conn)
{
	...
	log_dbg("checking %s for user %s", _options.localusers,
		conn->s_state.redir.username);

	if (!(f = fopen(_options.localusers, "r"))) {
		log_err(errno, "fopen() failed opening %s!",
			_options.localusers);
		return 0;
	}
	...
	/**/ if (redir->secret && *redir->secret) {
		MD5Init(&context);
		MD5Update(&context, (uint8_t *) conn->s_state.redir.uamchal,
			  REDIR_MD5LEN);
		MD5Update(&context, (uint8_t *) redir->secret,
			  strlen(redir->secret));
		MD5Final(chap_challenge, &context);
	...
	case REDIR_AUTH_PAP:
		...
		break;
	case REDIR_AUTH_CHAP:
		memcpy(user_password, conn->authdata.v.chapmsg.password,
		       REDIR_MD5LEN);
		break;
	default:
		log_err(0,
			"Authentication method not supported for locally authenticated users: %d",
			conn->authdata.type);
		return 0;
	}
	...
	while ((len = getline(&line, &sz, f)) > 0) {
		if (len > 3 && len < sizeof(u) && line[0] != '#') {
			...
			if (*pl == ':')
				...
				log_warn(0, "not a valid localusers line: %s",
					 line);
				continue;
			}
			...
			*pp = 0;
			...
			if (usernamelen == strlen(u) &&
			    !strncmp(conn->s_state.redir.username, u,
				     usernamelen)) {
				...
				if (conn->authdata.type == REDIR_AUTH_PAP) {
					if (!strcmp((char *)user_password, p))
						match = 1;
				} else if (conn->authdata.type ==
					   REDIR_AUTH_CHAP) {
					uint8_t tmp[REDIR_MD5LEN];
					MD5Init(&context);
					MD5Update(&context,
						  (uint8_t *) & conn->authdata.
						  v.chapmsg.identity, 1);
					MD5Update(&context, (uint8_t *) p,
						  strlen(p));
					MD5Update(&context, chap_challenge,
						  REDIR_MD5LEN);
					MD5Final(tmp, &context);

					if (!memcmp
					    (user_password, tmp, REDIR_MD5LEN))
						match = 1;
					else {
						log_dbg("bad password for %s",
							u);
					}
				}
	...
	log_dbg("user %s %s", conn->s_state.redir.username,
...
int redir_send_msg(struct redir_t *this, struct redir_msg_t *msg)
{
	...
	safe_strncpy(remote.sun_path, filedest, sizeof(remote.sun_path));
...
#if defined(F_DUPFD)
		if (fcntl(in, F_GETFL, 0) == -1)
			return -1;
		safe_close(0);
		if (fcntl(in, F_DUPFD, 0) == -1)
			return -1;
		if (fcntl(out, F_GETFL, 1) == -1)
			return -1;
		safe_close(1);
...
#endif
		if (forked)
			_redir_close_exit(socket.fd[0], socket.fd[1]);
...
#define redir_msg_send(msgopt) \
  ...
  if (msgsnd(redir->msgid, (void *)&msg, sizeof(msg.mdata), 0) < 0) { \
	 ...
	 */
#if(_debug_ > 1)
	log_dbg("Receiving HTTP%s Request",
		(conn.flags & USING_SSL) ? "S" : "");
#endif
	...
	if ((conn.flags & USING_SSL) == USING_SSL) {
		...
		if (!rreq || !rreq->sslcon) {
			socket.sslcon =
			    openssl_accept_fd(initssl(), socket.fd[0], 10,
					      &conn);
			if (rreq) {
				/* we were forked */
...
#if(_debug_ > 1)
	log_dbg("Processing HTTP%s Request",
		(conn.flags & USING_SSL) ? "S" : "");
#endif
	...
	case REDIR_WWW:
		{
			...
			if (_options.wwwdir && ((conn.wwwfile && *conn.wwwfile)
				...
				if (!strcmp
					    (filename + (namelen - 5), ".html"))
					ctype = "text/html";
				else if (!strcmp
					 (filename + (namelen - 4), ".gif"))
					ctype = "image/gif";
				else if (!strcmp
					 (filename + (namelen - 3), ".js"))
					ctype = "text/javascript";
				else if (!strcmp
					 (filename + (namelen - 4), ".css"))
					ctype = "text/css";
				else if (!strcmp
					 (filename + (namelen - 4), ".jpg"))
					ctype = "image/jpeg";
				else if (!strcmp
					 (filename + (namelen - 4), ".mp4"))
					ctype = "video/mp4";
				else if (!strcmp
					 (filename + (namelen - 4), ".ogv"))
					ctype = "video/ogg";
				else if (!strcmp
					 (filename + (namelen - 4), ".dat"))
					ctype =
					    "application/x-ns-proxy-autoconfig";
				else if (!strcmp
					 (filename + (namelen - 4), ".png"))
					ctype = "image/png";
				else if (!strcmp
					 (filename + (namelen - 4), ".swf"))
					ctype = "application/x-shockwave-flash";
				else if (!strcmp
					 (filename + (namelen - 4), ".chi")) {
					ctype = "text/html";
					parse = 1;
				} else
				    if (!strcmp
					(filename + (namelen - 4), ".cjs")) {
					ctype = "text/javascript";
					parse = 1;
				} else {
					/* we do not serve it! */
				...
				if (parse) {
					...
					if (!_options.wwwbin) {
						log_err(0,
							"the 'wwwbin' setting must be configured for CGI use");
						return redir_main_exit();
					}
					...
					setenv("AUTHENTICATED",
					       conn.s_state.authenticated
					       && (conn.s_params.
						   flags & REQUIRE_UAM_SPLASH)
					       == 0 ? "1" : "0", 1);

					setenv("CHI_SESSION_ID",
					       conn.s_state.sessionid, 1);
					setenv("CHI_USERNAME",
					       conn.s_state.redir.username, 1);
					setenv("CHI_USERURL",
					       conn.s_state.redir.userurl, 1);
					safe_snprintf(buffer, sizeof(buffer),
						      "%lld",
						      conn.s_state.
						      input_octets);
					...
					safe_snprintf(buffer, sizeof(buffer),
						      ...
						      conn.s_params.
						      sessiontimeout);
					setenv("CHI_SESSION_TIMEOUT", buffer,
					       1);

					redir_chartohex(conn.s_state.redir.
							uamchal, buffer,
							REDIR_MD5LEN);
				...
				if ((_options.uid == 0
				     && !chroot(_options.wwwdir) && !chdir("/"))
				    || (_options.uid != 0
					&& !chdir(_options.wwwdir))) {
					int gzip = 1;
					char filebuff[1024];

					safe_snprintf(filebuff,
						      sizeof(filebuff), "%s.gz",
						      filename);
					...
					if (fd > 0) {
						...
						log_err(0,
							"could not open local content file %s!",
							filename);
				} else
					log_err(0,
						"chroot/chdir to %s was not successful\n",
						_options.wwwdir);
	...
	case REDIR_LOGIN:{
			...
			if (state == 1) {
				...
				if (splash) {
					...
					log_dbg
					    ("redir_accept: already logged on");

					redir_reply(redir, &socket, &conn,
						    REDIR_ALREADY, NULL, 0,
						    NULL, NULL,
						    conn.s_state.redir.userurl,
						    NULL,
			...
			if (_options.challengetimeout2 &&
			    (conn.s_state.uamtime +
			     _options.challengetimeout2) < mainclock_now()) {
				log_dbg
				    ("redir_accept: challenge expired: %d : %d",
				...
				redir_reply(redir, &socket, &conn,
					    REDIR_FAILED_OTHER, NULL, 0,
					    hexchal, NULL, NULL, NULL, 0,
					    conn.hismac, &conn.hisip,
					    httpreq.qs);
			...
			if (is_local_user(redir, &conn)) {
				...
				if (flags & CHILLI_MOD_REDIR_SKIP_RADIUS) {
					log_dbg
					    ("Skipping RADIUS authentication");
				} else {
#endif
					...
					redir_radius(redir, &address->sin_addr,
						     &conn, reauth);
			...
			if (conn.response == REDIR_SUCCESS) {	/* Accept-Accept */
				...
				log_dbg("%s handling Access-Accept",
					__FUNCTION__);
				...
				redir_msg_send(REDIR_MSG_OPT_REDIR |
					       REDIR_MSG_OPT_PARAMS);

			} else {	/* Access-Reject */

				int hasnexturl =
				    (strlen((char *)conn.s_params.url) > 5);

				log_dbg("%s handling Access-Reject",
					__FUNCTION__);
				...
				redir_reply(redir, &socket, &conn,
					    conn.response, NULL, 0, hexchal,
					    NULL, conn.s_state.redir.userurl,
					    conn.reply,
					    (char *)conn.s_params.url,
					    conn.hismac, &conn.hisip,
					    httpreq.qs);
				...
				redir_msg_send(REDIR_MSG_OPT_REDIR |
					       REDIR_MSG_OPT_PARAMS | (conn.
								       response
								       ==
								       REDIR_CHALLENGE
								       ? 0 :
								       REDIR_MSG_NSESSIONID));
			}
	...
	case REDIR_LOGOUT:
		{
			...
			redir_reply(redir, &socket, &conn, REDIR_LOGOFF, NULL,
				    0, hexchal, NULL,
	...
	case REDIR_PRELOGIN:
		...
		if (state == 1) {
			redir_reply(redir, &socket, &conn, REDIR_ALREADY,
				    NULL, 0, NULL, NULL,
				    conn.s_state.redir.userurl, NULL,
	...
	case REDIR_STATUS:
		{
			...
			if (conn.s_params.sessiontimeout)
				timeleft =
				    conn.s_params.sessiontimeout - sessiontime;
			else
				timeleft = 0;

			redir_reply(redir, &socket, &conn, REDIR_STATUS, NULL,
				    timeleft, hexchal,
				    conn.s_state.redir.username,
				    conn.s_state.redir.userurl, conn.reply, 0,
				    conn.hismac, &conn.hisip, httpreq.qs);
	...
	if (redir->homepage) {	/* ||
		...
		redir_reply(redir, &socket, &conn,
			    ...
			    splash ? hexchal : NULL, NULL,
			    conn.s_state.redir.userurl, NULL,
			    (char *)conn.s_params.url, conn.hismac, &conn.hisip,
			    httpreq.qs);
	} else {
		redir_reply(redir, &socket, &conn,
