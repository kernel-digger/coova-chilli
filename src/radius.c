			     ...
			     struct session_state *state)
{
	...
	if (state->redir.calledlen) {
		radius_addattr(radius, pack, RADIUS_ATTR_CALLED_STATION_ID, 0,
			       0, 0, state->redir.called,
			       state->redir.calledlen);
		return;
	}
#endif
	...
	if (_options.nasmac)
		...
		safe_snprintf((char *)(mac =
				       b), sizeof(b),
			      "%.2X-%.2X-%.2X-%.2X-%.2X-%.2X",
			      radius->nas_hwaddr[0], radius->nas_hwaddr[1],
			      radius->nas_hwaddr[2], radius->nas_hwaddr[3],
			      radius->nas_hwaddr[4], radius->nas_hwaddr[5]);
...
int radius_queue_in(struct radius_t *this,
		    struct radius_packet_t *pack, void *cbp)
{
	...
	if (!radius_getattr(pack, &ma,
		...
		radius_hmac_md5(this, pack,
				this->secret, this->secretlen, ma->v.t);
	}
	...
	if (!RADIUS_QUEUE_HASPKT(this->queue[qnext].p))
		return -1;
	memcpy(RADIUS_QUEUE_PKTPTR(this->queue[qnext].p), pack,
	       RADIUS_PACKSIZE);
...
radius_queue_out(struct radius_t *this, int idx,
		 ...
		 struct radius_packet_t *pack_out, void **cbp)
{
	int id = 0;
	...
	if (this->queue[idx].state != 1) {
		log_err(0, "RADIUS id=%d idx=%d with state != 1", id, idx);
		return -1;
	}
#if(_debug_ > 1)
	if (_options.debug) {
	...
	if (RADIUS_QUEUE_HASPKT(this->queue[idx].p)) {
		if (pack_in &&
			...
			log_warn(0,
				 "Authenticator does not match! req-id=%d res-id=%d",
 ...
 */
int radius_cmptv(struct timeval *tv1, struct timeval *tv2)
{
	struct timeval diff;
...
int radius_timeout(struct radius_t *this)
{
	...
	if (this->first != -1 &&
		...
		if (this->queue[this->first].retrans < _options.radiusretry) {
			...
			if (this->queue[this->first].retrans ==
			    (_options.radiusretrysec - 1)) {
			...
			if (RADIUS_QUEUE_HASPKT(this->queue[this->first].p)) {
				...
				if (sendto(this->fd,
					   RADIUS_QUEUE_PKTPTR(this->
							       queue[this->
								     first].p),
					   ntohs(RADIUS_QUEUE_PKT
			...
			if (radius_queue_reschedule(this, this->first)) {
				log_warn(0,
					 "Matching request was not found in queue: %d!",
					 this->first);
				return -1;
			}
			...
			if (radius_queue_out(this, this->first,
					     0, &pack_req, &cbp)) {
				log_warn(0,
					 "RADIUS idx=%d was not found in queue!",
					 this->first);
				return -1;
			}
			...
			if ((pack_req.code == RADIUS_CODE_ACCOUNTING_REQUEST) &&
			    (this->cb_acct_conf))
				return this->cb_acct_conf(this, NULL, &pack_req,
							  cbp);

			if ((pack_req.code == RADIUS_CODE_ACCESS_REQUEST) &&
			    (this->cb_auth_conf))
				return this->cb_auth_conf(this, NULL, &pack_req,
							  cbp);
		}
	}
#if(_debug_ > 1)
	       ...
	       uint32_t value, uint8_t * data, uint16_t dlen)
{
	...
	if (type != RADIUS_ATTR_VENDOR_SPECIFIC) {
		...
		if (vlen > RADIUS_ATTR_VLEN) {
			log_warn(0,
				 "Truncating RADIUS attribute (type:%d/%d/%d) from %d to %d bytes [%s]",
		...
		if (vlen > RADIUS_ATTR_VLEN - 8) {
			log_warn(0,
				 "Truncating RADIUS attribute (type:%d/%d/%d) from %d to %d [%s]",
				 type, vendor_id, vendor_type, vlen,
...
int radius_cmpattr(struct radius_attr_t *t1, struct radius_attr_t *t2)
{
	...
	if (t1->l != t2->l)
		return -1;
	if (memcmp(t1->v.t, t2->v.t, t1->l))
		return -1;	/* Also int/time/addr */
	return 0;
}
		     ...
		     uint8_t * authenticator, char *secret, size_t secretlen)
{
	...
	if (srclen < 18) {
		log_err(0,
			"radius_keydecode MPPE attribute content len must be at least 18, len = %d",
			srclen);
		return -1;
	}
	...
	*dstlen = (size_t) (src[2] ^ b[0]);
	...
	if (*dstlen > (srclen - 3)) {
		log_err(0,
			"radius_keydecode not enough encrypted data bytes for indicated key length = %d (bytes)",
			*dstlen);
		return -1;
	}
	...
	if (*dstlen > dstsize) {
		log_err(0,
			"radius_keydecode output buffer for plaintext key is too small");
		return -1;
	}
	...
	for (n = 1; n < blocks; n++) {
		MD5Init(&context);
		MD5Update(&context, (uint8_t *) secret, secretlen);
		MD5Update(&context, &src[2 + ((n - 1) * RADIUS_MD5LEN)],
			  RADIUS_MD5LEN);
		MD5Final(b, &context);
		for (i = 0; i < RADIUS_MD5LEN; i++) {
			if ((i - 1 + n * RADIUS_MD5LEN) < (int)*dstlen) {
				dst[i - 1 + n * RADIUS_MD5LEN] =
				    src[i + 2 + n * RADIUS_MD5LEN] ^ b[i];
			}
		}
	}
		     ...
		     uint8_t * authenticator, char *secret, size_t secretlen)
{
	...
	if ((blocks * RADIUS_MD5LEN) < ((int)srclen + 1))
		blocks++;
	...
	*dstlen = (size_t) ((blocks * RADIUS_MD5LEN) + 2);
	...
	for (n = 1; n < blocks; n++) {
		MD5Init(&context);
		MD5Update(&context, (uint8_t *) secret, secretlen);
		MD5Update(&context, &dst[2 + ((n - 1) * RADIUS_MD5LEN)],
			  RADIUS_MD5LEN);
		MD5Final(b, &context);
		for (i = 0; i < RADIUS_MD5LEN; i++)
			if ((i - 1) < (int)srclen)
				dst[i + 2 + n * RADIUS_MD5LEN] =
				    src[i - 1 + n * RADIUS_MD5LEN] ^ b[i];
			else
...
int radius_new(struct radius_t **this,
	       struct in_addr *listen, uint16_t port, int coanocheck, int proxy)
{
	struct sockaddr_in addr;
	...
	if (proxy) {		/* Proxy parameters */
		if (_options.proxyport && _options.proxysecret) {
			new_radius->proxylisten.s_addr =
			    _options.proxylisten.s_addr;
			new_radius->proxyport = _options.proxyport;

			if (_options.proxyaddr.s_addr) {
				new_radius->proxyaddr.s_addr =
				    _options.proxyaddr.s_addr;
				if (_options.proxymask.s_addr)
					new_radius->proxymask.s_addr =
					    _options.proxymask.s_addr;
				else
			...
			if ((new_radius->proxysecretlen =
			     strlen(_options.proxysecret)) <
			    RADIUS_SECRETSIZE) {
				memcpy(new_radius->proxysecret,
	...
	log_dbg("RADIUS client %s:%d",
		inet_ntoa(new_radius->ouraddr), new_radius->ourport);
	...
	if (proxy) {		/* Initialise proxy socket */
		...
		if (bind
		    (new_radius->proxyfd, (struct sockaddr *)&addr,
		     sizeof(addr)) < 0) {
...
void radius_set(struct radius_t *this, unsigned char *hwaddr, int debug)
{
	...
	if (_options.radsec) {
		...
		if ((this->secretlen =
		     strlen(_options.radiussecret)) > RADIUS_SECRETSIZE) {
			log_err(0,
 ...
 */
int radius_req(struct radius_t *this, struct radius_packet_t *pack, void *cbp)
{
	struct sockaddr_in addr;
	...
	log_dbg("RADIUS id=%d sent to %s:%d",
		pack->id, inet_ntoa(addr.sin_addr), ntohs(addr.sin_port));
#endif
		...
		struct sockaddr_in *peer, uint8_t * req_auth)
{
	...
	/* If packet contains message authenticator: Calculate it! */
	if (!radius_getattr
	    (pack, &ma, RADIUS_ATTR_MESSAGE_AUTHENTICATOR, 0, 0, 0)) {
		radius_hmac_md5(this, pack, this->proxysecret,
				this->proxysecretlen, ma->v.t);
	}

	radius_authresp_authenticator(this, pack, req_auth,
				      this->proxysecret, this->proxysecretlen);
		   ...
		   struct sockaddr_in *peer, uint8_t * req_auth)
{
	...
	if (!radius_getattr
	    (pack, &ma, RADIUS_ATTR_MESSAGE_AUTHENTICATOR, 0, 0, 0)) {
		radius_hmac_md5(this, pack, this->secret, this->secretlen,
				ma->v.t);
	}
...
radius_default_pack(struct radius_t *this,
		    struct radius_packet_t *pack, int code)
{
	...
	case RADIUS_CODE_ACCOUNTING_REQUEST:
		...
		if (code == RADIUS_CODE_ACCOUNTING_REQUEST) {
			...
			radius_addattr(this, pack, RADIUS_ATTR_EVENT_TIMESTAMP,
				       0, 0, mainclock_wall(), NULL, 0);
			     ...
			     struct radius_packet_t *resp)
{
	...
	if (!radius_getattr(req, &attr, RADIUS_ATTR_SERVICE_TYPE, 0, 0, 0)) {
	...
	if (_options.extadmvsa[0].attr &&
		...
		if (req && !resp) {
			...
			for (i = 0; i < EXTADMVSA_ATTR_CNT; i++) {
				...
				if ((fd =
				     open(_options.extadmvsa[i].data,
					  O_RDONLY)) < 0) {
					log_err(errno, "Failed to open %s",
						_options.extadmvsa[i].data);
					continue;
				}
			...
			for (i = 0; i < EXTADMVSA_ATTR_CNT; i++) {
				...
				if (!_options.extadmvsa[i].attr_vsa) {
					...
					if (radius_getattr
					    (resp, &attr,
					     _options.extadmvsa[i].attr, 0, 0,
					     0)) {
						log_dbg("didn't find attr %d",
							_options.extadmvsa[i].
							attr);
						attr = 0;
					}
				} else {
...
int radius_decaps(struct radius_t *this, int idx)
{
	...
	case RADIUS_CODE_COA_REQUEST:
		if (!this->coanocheck) {
			...
			if ((addr.sin_addr.s_addr != this->hisaddr0.s_addr) &&
				...
				log_warn(0,
					 "Received RADIUS from wrong address %.8x!",
		...
		if (radius_acctcheck(this, &pack)) {
			log_warn(0, "RADIUS id=%d Authenticator did not match!",
				 pack.id);
			return -1;
		}
		break;
	...
	default:
		...
		if ((addr.sin_addr.s_addr != this->hisaddr0.s_addr) &&
		    (addr.sin_addr.s_addr != this->hisaddr1.s_addr)) {
			log_warn(0,
				 "Received radius reply from wrong address %s!",
		...
		if ((addr.sin_port != htons(this->authport)) &&
		    (addr.sin_port != htons(this->acctport))) {
			log_warn(0,
				 "Received radius packet from wrong port %d!",
				 ntohs(addr.sin_port));
			return -1;
		}
...
#ifdef ENABLE_EXTADMVSA
	chilli_extadmvsa(this, (struct app_conn_t *)cbp, &pack, &pack_req);
#endif
...
int radius_proxy_ind(struct radius_t *this, int idx)
{
	...
	if ((this->cb_ind) &&
		...
		if ((addr.sin_addr.s_addr & this->proxymask.s_addr) !=
			...
			log_warn(0,
				 "Received RADIUS proxy request from wrong address %s",
				 inet_ntoa(addr.sin_addr));
