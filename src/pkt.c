	...
	if (iph->protocol == PKT_IP_PROTO_TCP) {
			...
			while (!done && (i / 4) < words) {
				...
				default:
					...
					if (len < 2 || len > TCP_MAX_OPTION_LEN) {
						log_err(0,
							"bad TCP option during parse, len=%d",
							len);
						return -1;
					}
					if (type == 2 && len == 4) {
					...
					} else if (_options.scalewin
						   && type == 3 && len == 3) {
						log_dbg
						    ("TCP OPTIONS: window scale was %d",
						     (int)opts[i]);
						if (opts[i] > 0) {
							opts[i] = 0;
							chksum(iph);
		...
		if (!hasmss && *length < 1400 && tcphdr_syn(tcph)) {
			...
			memcpy(p, packet, *length);
			{
				...
				uint8_t *dst_opt =
				    p_tcph->options + ((off - 5) * 4);
				uint8_t *src_opt =
				    tcph->options + ((off - 5) * 4);

				int dlen =
				    *length - sizeofip(packet) - (off * 4);
