	...
	while (fd <= 0) {
		...
		if (fd <= 0) {
			if (done_before)
				...
				log_warn(0,
					 "could not generate configuration (%s), sleeping one second",
					 file);
				sleep(1);
			}
		}
...
int options_fromfd(int fd, bstring bt)
{
	...
	if (has_error) {
		log_err(errno,
			"could not read configuration, some kind of mismatch fd=%d %s",
			fd, SBINDIR);
		return 0;
	}

	if (!option_s_l(bt, &o.binconfig))
		return 0;
	if (!option_s_l(bt, &o.pidfile))
		return 0;
	if (!option_s_l(bt, &o.statedir))
		return 0;
	if (!option_s_l(bt, &o.usestatusfile))
		return 0;
	if (!option_s_l(bt, &o.tundev))
		return 0;
	if (!option_s_l(bt, &o.dynip))
		return 0;
	if (!option_s_l(bt, &o.statip))
		return 0;
	if (!option_s_l(bt, &o.ethers))
		return 0;

	if (!option_s_l(bt, &o.domain))
		return 0;
	if (!option_s_l(bt, &o.ipup))
		return 0;
	if (!option_s_l(bt, &o.ipdown))
		return 0;
	if (!option_s_l(bt, &o.conup))
		return 0;
	if (!option_s_l(bt, &o.condown))
		return 0;
	if (!option_s_l(bt, &o.macup))
		return 0;
	if (!option_s_l(bt, &o.macdown))
		return 0;
#ifdef ENABLE_IEEE8021Q
	if (!option_s_l(bt, &o.vlanupdate))
		return 0;
#endif
#ifdef ENABLE_PROXYVSA
	if (!option_s_l(bt, &o.locationupdate))
		return 0;
#endif

	if (!option_s_l(bt, &o.radiussecret))
		return 0;
#ifdef ENABLE_LARGELIMITS
	if (!option_s_l(bt, &o.radiusacctsecret))
		return 0;
	if (!option_s_l(bt, &o.radiusadmsecret))
		return 0;
#endif
	if (!option_s_l(bt, &o.radiusnasid))
		return 0;
	if (!option_s_l(bt, &o.radiuslocationid))
		return 0;
	if (!option_s_l(bt, &o.radiuslocationname))
		return 0;
	if (!option_s_l(bt, &o.locationname))
		return 0;
	if (!option_s_l(bt, &o.proxysecret))
		return 0;

	if (!option_s_l(bt, &o.dhcpif))
		return 0;
#ifdef ENABLE_MULTILAN
	for (i = 0; i < MAX_MOREIF; i++) {
		if (!option_s_l(bt, &o.moreif[i].dhcpif))
			return 0;
		if (!option_s_l(bt, &o.moreif[i].vlan))
			return 0;
	}
#endif
	if (!option_s_l(bt, &o.routeif))
		return 0;
	if (!option_s_l(bt, &o.peerkey))
		return 0;

	if (!option_s_l(bt, &o.macsuffix))
		return 0;
	if (!option_s_l(bt, &o.macpasswd))
		return 0;

	if (!option_s_l(bt, &o.uamsecret))
		return 0;
	if (!option_s_l(bt, &o.uamurl))
		return 0;
	if (!option_s_l(bt, &o.uamaaaurl))
		return 0;
	if (!option_s_l(bt, &o.uamhomepage))
		return 0;
	if (!option_s_l(bt, &o.wisprlogin))
		return 0;

	if (!option_s_l(bt, &o.wwwdir))
		return 0;
	if (!option_s_l(bt, &o.wwwbin))
		return 0;
	if (!option_s_l(bt, &o.uamui))
		return 0;
	if (!option_s_l(bt, &o.localusers))
		return 0;
#ifdef HAVE_SSL
	if (!option_s_l(bt, &o.sslkeyfile))
		return 0;
	if (!option_s_l(bt, &o.sslkeypass))
		return 0;
	if (!option_s_l(bt, &o.sslcertfile))
		return 0;
	if (!option_s_l(bt, &o.sslcafile))
		return 0;
#endif
#ifdef USING_IPC_UNIX
	if (!option_s_l(bt, &o.unixipc))
		return 0;
#endif
#ifdef HAVE_NETFILTER_COOVA
	if (!option_s_l(bt, &o.kname))
		return 0;
#endif
#ifdef ENABLE_DNSLOG
	if (!option_s_l(bt, &o.dnslog))
		return 0;
#endif
#ifdef ENABLE_IPWHITELIST
	if (!option_s_l(bt, &o.ipwhitelist))
		return 0;
#endif
#ifdef ENABLE_UAMDOMAINFILE
	if (!option_s_l(bt, &o.uamdomainfile))
		return 0;
#endif
#ifdef ENABLE_MODULES
	if (!option_s_l(bt, &o.moddir))
		return 0;
#endif

	if (!option_s_l(bt, &o.adminuser))
		return 0;
	if (!option_s_l(bt, &o.adminpasswd))
		return 0;
	if (!option_s_l(bt, &o.adminupdatefile))
		return 0;
	if (!option_s_l(bt, &o.rtmonfile))
		return 0;

	if (!option_s_l(bt, &o.ssid))
		return 0;
	if (!option_s_l(bt, &o.vlan))
		return 0;
	if (!option_s_l(bt, &o.nasmac))
		return 0;
	if (!option_s_l(bt, &o.nasip))
		return 0;
	if (!option_s_l(bt, &o.cmdsocket))
		return 0;

	if (!option_s_l(bt, &o.uamaliasname))
		return 0;
	if (!option_s_l(bt, &o.uamhostname))
		return 0;

#ifdef ENABLE_REDIRINJECT
	if (!option_s_l(bt, &o.inject))
		return 0;
	if (!option_s_l(bt, &o.inject_ext))
		return 0;
#endif
...
#include EX_OPTIONS_LOAD
	...
	for (i = 0; i < MAX_MODULES; i++) {
		...
		if (!_options.modules[i].ctx)
			...
			if (!strcmp
			    (_options.modules[i].name, o.modules[i].name))
				isReload[i] = 1;
			if (m->destroy)
...
int options_save(char *file, bstring bt)
{
	...
	if (!option_s_s(bt, &o.binconfig))
		return 0;
	if (!option_s_s(bt, &o.pidfile))
		return 0;
	if (!option_s_s(bt, &o.statedir))
		return 0;
	if (!option_s_s(bt, &o.usestatusfile))
		return 0;
	if (!option_s_s(bt, &o.tundev))
		return 0;
	if (!option_s_s(bt, &o.dynip))
		return 0;
	if (!option_s_s(bt, &o.statip))
		return 0;
	if (!option_s_s(bt, &o.ethers))
		return 0;

	if (!option_s_s(bt, &o.domain))
		return 0;
	if (!option_s_s(bt, &o.ipup))
		return 0;
	if (!option_s_s(bt, &o.ipdown))
		return 0;
	if (!option_s_s(bt, &o.conup))
		return 0;
	if (!option_s_s(bt, &o.condown))
		return 0;
	if (!option_s_s(bt, &o.macup))
		return 0;
	if (!option_s_s(bt, &o.macdown))
		return 0;
#ifdef ENABLE_IEEE8021Q
	if (!option_s_s(bt, &o.vlanupdate))
		return 0;
#endif
#ifdef ENABLE_PROXYVSA
	if (!option_s_s(bt, &o.locationupdate))
		return 0;
#endif

	if (!option_s_s(bt, &o.radiussecret))
		return 0;
#ifdef ENABLE_LARGELIMITS
	if (!option_s_s(bt, &o.radiusacctsecret))
		return 0;
	if (!option_s_s(bt, &o.radiusadmsecret))
		return 0;
#endif

	if (!option_s_s(bt, &o.radiusnasid))
		return 0;
	if (!option_s_s(bt, &o.radiuslocationid))
		return 0;
	if (!option_s_s(bt, &o.radiuslocationname))
		return 0;
	if (!option_s_s(bt, &o.locationname))
		return 0;
	if (!option_s_s(bt, &o.proxysecret))
		return 0;

	if (!option_s_s(bt, &o.dhcpif))
		return 0;
#ifdef ENABLE_MULTILAN
	for (i = 0; i < MAX_MOREIF; i++) {
		if (!option_s_s(bt, &o.moreif[i].dhcpif))
			return 0;
		if (!option_s_s(bt, &o.moreif[i].vlan))
			return 0;
	}
#endif
	if (!option_s_s(bt, &o.routeif))
		return 0;
	if (!option_s_s(bt, &o.peerkey))
		return 0;

	if (!option_s_s(bt, &o.macsuffix))
		return 0;
	if (!option_s_s(bt, &o.macpasswd))
		return 0;

	if (!option_s_s(bt, &o.uamsecret))
		return 0;
	if (!option_s_s(bt, &o.uamurl))
		return 0;
	if (!option_s_s(bt, &o.uamaaaurl))
		return 0;
	if (!option_s_s(bt, &o.uamhomepage))
		return 0;
	if (!option_s_s(bt, &o.wisprlogin))
		return 0;

	if (!option_s_s(bt, &o.wwwdir))
		return 0;
	if (!option_s_s(bt, &o.wwwbin))
		return 0;
	if (!option_s_s(bt, &o.uamui))
		return 0;
	if (!option_s_s(bt, &o.localusers))
		return 0;
#ifdef HAVE_SSL
	if (!option_s_s(bt, &o.sslkeyfile))
		return 0;
	if (!option_s_s(bt, &o.sslkeypass))
		return 0;
	if (!option_s_s(bt, &o.sslcertfile))
		return 0;
	if (!option_s_s(bt, &o.sslcafile))
		return 0;
#endif
#ifdef USING_IPC_UNIX
	if (!option_s_s(bt, &o.unixipc))
		return 0;
#endif
#ifdef HAVE_NETFILTER_COOVA
	if (!option_s_s(bt, &o.kname))
		return 0;
#endif
#ifdef ENABLE_DNSLOG
	if (!option_s_s(bt, &o.dnslog))
		return 0;
#endif
#ifdef ENABLE_IPWHITELIST
	if (!option_s_s(bt, &o.ipwhitelist))
		return 0;
#endif
#ifdef ENABLE_UAMDOMAINFILE
	if (!option_s_s(bt, &o.uamdomainfile))
		return 0;
#endif
#ifdef ENABLE_MODULES
	if (!option_s_s(bt, &o.moddir))
		return 0;
#endif

	if (!option_s_s(bt, &o.adminuser))
		return 0;
	if (!option_s_s(bt, &o.adminpasswd))
		return 0;
	if (!option_s_s(bt, &o.adminupdatefile))
		return 0;
	if (!option_s_s(bt, &o.rtmonfile))
		return 0;

	if (!option_s_s(bt, &o.ssid))
		return 0;
	if (!option_s_s(bt, &o.vlan))
		return 0;
	if (!option_s_s(bt, &o.nasmac))
		return 0;
	if (!option_s_s(bt, &o.nasip))
		return 0;
	if (!option_s_s(bt, &o.cmdsocket))
		return 0;

	if (!option_s_s(bt, &o.uamaliasname))
		return 0;
	if (!option_s_s(bt, &o.uamhostname))
		return 0;

#ifdef ENABLE_REDIRINJECT
	if (!option_s_s(bt, &o.inject))
		return 0;
	if (!option_s_s(bt, &o.inject_ext))
		return 0;
#endif
...
int options_binload(char *file)
{
	...
	if (fd > 0) {
		...
		log_dbg("PID %d loading binary options file %s", getpid(),
			file);
