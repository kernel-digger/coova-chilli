...
#endif
			      "\n",
			      inet_ntoa(pt->host), mask, pt->proto, pt->port
#ifdef ENABLE_GARDENEXT
			  ...
			  struct pkt_ipphdr_t *ipph, int dst)
{
	...
	if (pfx) {
		...
		if (nd) {
			...
			case -1:
				if (pt)
					pass_through_rem(ptlist, ptcnt, pt,
							 ptree);
				break;
			}
		}
	...
	if (pfx != NULL) {
		...
		if (nd != NULL) {
			...
			for (i = 0; i < nd->ptcnt; i++) {
				if (pt_equal(&nd->ptlist[i], pt)) {
					log_dbg
					    ("Uamallowed removing #%d:%d: proto=%d host=%s port=%d",
					     i, nd->ptcnt, pt->proto,
					     inet_ntoa(pt->host), pt->port);

					log_dbg
					    ("Shifting uamallowed list %d to %d",
					     i, nd->ptcnt);
    ...
    )
{
	...
	for (i = 0; i < ptcnt; i++) {
		...
		if (pt->proto == 0 || ipph->protocol == pt->proto)
			if (pt->host.s_addr == 0 ||
			    pt->host.s_addr ==
			    ((dst ? ipph->daddr : ipph->saddr) & pt->mask.
			     s_addr))
				if (pt->port == 0
				    ||
				    ((ipph->protocol == PKT_IP_PROTO_TCP
				      || ipph->protocol == PKT_IP_PROTO_UDP)
				     && (dst ? ipph->dport : ipph->sport) ==
				     htons(pt->port))) {
					if (pt_match)
						*pt_match = pt;
#ifdef ENABLE_GARDENEXT
					if (pt->expiry
					    && pt->expiry < mainclock_now()) {
						return -1;
					}
#endif
					return 1;
	...
	for (; p1; p1 = p2) {
		...
		if ((t = strchr(p1, ':'))) {
			...
			*t = 0;
			...
			if (!strcmp(p1, "tcp")) {
				pnum = 6;
			} else if (!strcmp(p1, "udp")) {
				pnum = 17;
			} else if (!strcmp(p1, "icmp")) {
				pnum = 1;
			}
#endif
		...
		if (strchr(p1, '/')) {	/* parse a network address */
			...
			if (is_rem) {
				if (pass_through_rem(ptlist, ptcnt, &pt
					...
					log_err(0,
						"Too many pass-throughs! skipped %s",
						s);
			} else {
				if (pass_through_add
				    (ptlist, ptlen, ptcnt, &pt, is_dyn
#ifdef HAVE_PATRICIA
				     , ptree
#endif
				    ))
...
int regex_pass_throughs_from_string(regex_pass_through * ptlist, uint32_t ptlen,
				    uint32_t * ptcnt, char *s, char is_dyn)
{
	uint32_t cnt = *ptcnt;
	...
	for (st = s; (p = strtok(st, "::")); st = 0, stage++) {
		...
		if (!is_wild) {
			...
			switch (stage) {
			case 0:
				safe_strncpy(pt.regex_host, p,
					     sizeof(pt.regex_host));
				pt.neg_host = is_negate;
				break;
			case 1:
				safe_strncpy(pt.regex_path, p,
					     sizeof(pt.regex_path));
				pt.neg_path = is_negate;
				break;
			case 2:
				safe_strncpy(pt.regex_qs, p,
					     sizeof(pt.regex_qs));
				pt.neg_qs = is_negate;
				break;
			}
		...
		while ((read = getline(&line, &len, fp)) != -1) {
			if (read <= 0)
				...
				if (regcomp
					...
					log_err(0, "could not compile regex %s",
						line);
					free(uam_re);
					continue;
				}
