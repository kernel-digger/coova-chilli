...
int printstatus()
{
	...
	while (dhcpconn) {
		...
		case DHCP_AUTH_ROUTER:
#endif
			log_dbg
			    ("Saving dhcp connection %.2X-%.2X-%.2X-%.2X-%.2X-%.2X %s",
			     dhcpconn->hismac[0], dhcpconn->hismac[1],
			     dhcpconn->hismac[2], dhcpconn->hismac[3],
			     dhcpconn->hismac[4], dhcpconn->hismac[5],
			...
			if (appconn) {
				fwrite(appconn, sizeof(struct app_conn_t), 1,
				       file);
				fputc(MARK_NEXT, file);
			}
			break;
...
int printstatus()
{
	...
	while (dhcpconn) {
		...
		if (appconn && appconn->s_state.authenticated == 1) {
			fprintf(file,
				"%s, %s, %.2X-%.2X-%.2X-%.2X-%.2X-%.2X, %s, %d, %d, %d\n",
				appconn->s_state.redir.username,
				inet_ntoa(appconn->hisip), appconn->hismac[0],
				appconn->hismac[1], appconn->hismac[2],
				appconn->hismac[3], appconn->hismac[4],
				appconn->hismac[5], appconn->s_state.sessionid,
