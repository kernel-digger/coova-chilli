	       ...
	       iphash_callback callback)
{
	...
	for (n->hashlog = 0; ((1 << n->hashlog) < listsize); n->hashlog++) ;
...
int iphash_get(struct iphash_t *this, struct iphashm_t **member,
...
int iphash_hashdel(struct iphash_t *this, struct iphashm_t *member)
{
	...
	if (p != member) {
		log_err(0,
			"iphash_hashdel: Tried to delete member not in hash table");
		return -1;
	}
...
int iphash_add(struct iphash_t *this, struct iphashm_t **member,
	       struct in_addr *addr, uint16_t port)
{
	struct iphashm_t *p;
	/*uint32_t hash; */
