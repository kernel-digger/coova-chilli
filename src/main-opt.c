...
static const char *usage = "Usage: chilli [OPTIONS]...\n";
...
static void options_print_version(void)
{
	printf("%s %s\n", PACKAGE, VERSION);
}

static void options_print_help(void)
{
...
int main(int argc, char **argv)
{
  ...
  /** simple configuration parameters **/
	...
	if (_options.layer3)
		log_warn(0,
			 "layer3 not implemented. build with --enable-layer3");
#endif
	...
	if (args_info.proxyport_arg)
		log_err(0,
			"radproxy not implemented. build with --enable-radproxy");
#endif
	...
	if (_options.redir)
		log_err(0,
			"chilli_redir not implemented. build with --enable-chilliredir");
#endif
	...
	if (_options.radsec)
		log_err(0,
			"chilli_radsec not implemented. build with --enable-chilliradsec");
#endif
	...
	if (_options.peerid)
		log_err(0,
			"clustering not implemented. build with --enable-cluster");
#endif
	_options.redirdnsreq = args_info.redirdnsreq_flag;
#if(_debug_ && !defined(ENABLE_REDIRDNSREQ))
	if (_options.redirdnsreq)
		log_err(0,
			"redirdnsreq not implemented. build with --enable-redirdnsreq");
#endif
	...
	_options.location_immediate_update =
	    args_info.locationimmediateupdate_flag;
	...
	if (args_info.proxylocattr_given) {
		for (numargs = 0; numargs < args_info.proxylocattr_given
			...
			switch (sscanf(args_info.proxylocattr_arg[numargs],
				       "%u,%u", &i[0], &i[1])) {
			case 0:
				log_err(0, "invalid input %s",
					args_info.proxylocattr_arg[numargs]);
				break;
			case 1:
	...
	if (args_info.dhcpgateway_arg &&
		...
		log_err(0, "Invalid DHCP gateway IP address: %s!",
			args_info.dhcpgateway_arg);
		if (!args_info.forgiving_flag)
			goto end_processing;
	}
	...
	if (args_info.dhcprelayagent_arg &&
		...
		log_err(0, "Invalid DHCP gateway relay IP address: %s!",
			args_info.dhcprelayagent_arg);
		if (!args_info.forgiving_flag)
			goto end_processing;
	}
	...
	if (!args_info.nexthop_arg) {
		...
		if ((macstrlen =
		     strlen(args_info.nexthop_arg)) >= (RADIUS_ATTR_VLEN - 1)) {
	...
	if (!args_info.dhcpmac_arg) {
		...
		if ((macstrlen =
		     strlen(args_info.dhcpmac_arg)) >= (RADIUS_ATTR_VLEN - 1)) {
			log_err(0, "MAC address too long");
			if (!args_info.forgiving_flag)
				goto end_processing;
		}
	...
	if (args_info.net_arg) {
		if (option_aton
		    (&_options.net, &_options.mask, args_info.net_arg, 0)) {
			log_err(0, "Invalid network address: %s!",
				args_info.net_arg);
			if (!args_info.forgiving_flag)
				goto end_processing;
		}
		if (!args_info.uamlisten_arg) {
			_options.uamlisten.s_addr =
			    htonl(ntohl(_options.net.s_addr) + 1);
		} else
		    if (!inet_aton
			(args_info.uamlisten_arg, &_options.uamlisten)) {
			log_err(0, "Invalid UAM IP address: %s!",
				args_info.uamlisten_arg);
			if (!args_info.forgiving_flag)
				goto end_processing;
		}
		if (!args_info.dhcplisten_arg) {
		    ...
		    if (!inet_aton
			(args_info.dhcplisten_arg, &_options.dhcplisten)) {
			log_err(0, "Invalid DHCP IP address: %s!",
				args_info.dhcplisten_arg);
			if (!args_info.forgiving_flag)
				goto end_processing;
		}
	} else {
		log_err(0,
			"Network address must be specified ('net' parameter)!");
		if (!args_info.forgiving_flag)
			goto end_processing;
	}
	...
	if (args_info.uamserver_arg) {
		...
		if (get_urlparts(args_info.uamserver_arg, hostname, USERURLSIZE,
				 &uamserverport, 0)) {
			log_err(0, "Failed to parse uamserver: %s!",
				args_info.uamserver_arg);
			if (!args_info.forgiving_flag)
				goto end_processing;
		}
			...
			if (!(host = gethostbyname(hostname))) {
				...
				while (host->h_addr_list[j] != NULL) {
					...
					if (pass_through_add
						...
						log_err(0,
							"Too many pass-throughs! skipped %s:%d",
							inet_ntoa(pt.host),
							pt.port);
				}
			}
	...
	for (numargs = 0; numargs < args_info.uamallowed_given; ++numargs) {
		pass_throughs_from_string(_options.pass_throughs,
					  ...
					  args_info.uamallowed_arg[numargs], 0,
					  0
#ifdef HAVE_PATRICIA
					  , 0
#endif
	...
	for (numargs = 0; numargs < args_info.dhcpopt_given; ++numargs) {
		...
		if (hex_length > 0 && (bin_length * 2) == hex_length &&
			...
			log_dbg("DHCP Options %s",
				args_info.dhcpopt_arg[numargs]);
			if (redir_hextochar
			    ((unsigned char *)args_info.dhcpopt_arg[numargs],
				...
				if (_options.dhcp_options_len + bin_length <
					...
					log_dbg("No room for DHCP option %d",
						(int)binopt[0]);
				}
			} else {
		...
		if (numargs < MAX_MODULES) {
			...
			if (sc && len > (nlen + 1)) {
				safe_snprintf(_options.modules[numargs].conf,
					      sizeof(_options.modules[numargs].
						     conf), "%.*s",
					      len - nlen - 1, sc + 1);
			}
		}
	...
	memset(_options.regex_pass_throughs, 0,
	       sizeof(_options.regex_pass_throughs));
	...
	if (args_info.radiuslisten_arg) {
		if (!(host = gethostbyname(args_info.radiuslisten_arg))) {
			...
			memcpy(&_options.radiuslisten.s_addr, host->h_addr,
			       host->h_length);
		}
	} else {
	...
	if (args_info.natip_arg) {
		if (!(host = gethostbyname(args_info.natip_arg))) {
			...
			memcpy(&_options.natip.s_addr, host->h_addr,
			       host->h_length);
		}
	}
#endif
	...
	if (args_info.uamlogoutip_arg) {
		if (!(host = gethostbyname(args_info.uamlogoutip_arg))) {
			...
			memcpy(&_options.uamlogout.s_addr, host->h_addr,
			       host->h_length);
		}
	}
	...
	if (args_info.uamaliasip_arg) {
		if (!(host = gethostbyname(args_info.uamaliasip_arg))) {
			...
			memcpy(&_options.uamalias.s_addr, host->h_addr,
			       host->h_length);
		}
	}
	...
	if (args_info.postauthproxy_arg) {
		if (!(host = gethostbyname(args_info.postauthproxy_arg))) {
			...
			memcpy(&_options.postauth_proxyip.s_addr, host->h_addr,
			       host->h_length);
		}
	}
	...
	if (args_info.radiusserver1_arg) {
		if (!(host = gethostbyname(args_info.radiusserver1_arg))) {
			...
			memcpy(&_options.radiusserver1.s_addr, host->h_addr,
			       host->h_length);
		}
	} else {
	...
	if (args_info.radiusserver2_arg) {
		if (!(host = gethostbyname(args_info.radiusserver2_arg))) {
			...
			memcpy(&_options.radiusserver2.s_addr, host->h_addr,
			       host->h_length);
		}
	} else {
	...
	if (args_info.proxylisten_arg) {
		...
		if (!(host = gethostbyname(args_info.proxylisten_arg))) {
			...
			memcpy(&_options.proxylisten.s_addr, host->h_addr,
			       host->h_length);
		}
	} else {
	...
	if (args_info.proxyclient_arg) {
		...
		if (option_aton(&_options.proxyaddr, &_options.proxymask,
			...
			log_err(0, "Invalid proxy client address: %s!",
				args_info.proxyclient_arg);
			if (!args_info.forgiving_flag)
				goto end_processing;
		}
	} else {
	...
	for (numargs = 0; numargs < args_info.macallowed_given; ++numargs) {
		...
		char *p2 = NULL;
		char *p3 =
		    malloc(strlen(args_info.macallowed_arg[numargs]) + 1);
		int i;

		unsigned int mac[6];

		log_dbg("Macallowed #%d: %s", numargs,
		...
		while (p1) {
			if (_options.macoklen >= MACOK_MAX) {
				...
				if (sscanf(p1, "%2x %2x %2x %2x %2x %2x",
					...
					log_dbg
					    ("Macallowed address #%d: %.2X-%.2X-%.2X-%.2X-%.2X-%.2X",
					     _options.macoklen, mac[0], mac[1],
					     mac[2], mac[3], mac[4], mac[5]);

					for (i = 0; i < 6; i++)
						_options.macok[_options.
							       macoklen][i] =
						    (unsigned char)mac[i];
  ...
  /** string parameters **/
	...
	if (args_info.uamdomainfile_arg)
		log_err(0,
			"option uamdomainfile given when no support built-in");
#endif
		...
		for (numargs = 0; numargs < args_info.extadmvsa_given
			...
			if (len > 0 && len < 256) {
				...
				if (sscanf(args_info.extadmvsa_arg[numargs],
					   "%u,%u:%s", &i[0], &i[1], s) == 3) {
					char *idx = strchr(s, ':');
					_options.extadmvsa[numargs].attr_vsa =
					    i[0];
					_options.extadmvsa[numargs].attr = i[1];
					if (idx)
						*idx = 0;
					safe_strncpy(_options.
						     extadmvsa[numargs].script,
						     s,
						     sizeof(_options.
							    extadmvsa[numargs].
							    script) - 1);
				    ...
				    if (sscanf
					(args_info.extadmvsa_arg[numargs],
					 "%u:%s", &i[0], s) == 2) {
					char *idx = strchr(s, ':');
					...
					log_err(0, "invalid input %s",
						args_info.
						extadmvsa_arg[numargs]);
				}
			}

			log_dbg
			    ("Extended admin-user attr (%d/%d) data=%s script=%s",
...
#include EX_OPT_MAIN
	...
	if (_options.binconfig) {	/* save out the configuration */
		...
		if (!ok)
			log_err(0, "could not save configuration options!");
		bdestroy(bt);
	}
