	...
	for (i = 0; i <= len; i++) {
		...
		if (eq_pos && end_pos && start_pos != eq_pos) {
			...
			if ((eq_pos + 1) == end_pos || (end_pos - eq_pos) <= 1) {
				...
				bassignblk(value, src + eq_pos + 1,
					   end_pos - eq_pos - 1);
				/*bunescape(value); */
				log_dbg("%s = %s", name->data, value->data);
				setenv((char *)name->data, (char *)value->data,
				       1);
			}
			start_pos = end_pos + 1;
			if (start_pos < len)
...
static int chilli_sessions(bstring b)
{
	...
	while (conn) {
		...
		conn = conn->next;
		if (appconn) {
			session_json_params(&appconn->s_state,
					    &appconn->s_params, s, 0);
			bcatcstr(s, ",");
			session_json_acct(&appconn->s_state, &appconn->s_params,
					  s, 0);
		}
		bcatcstr(s, "},");
		bconcat(b, s);
...
static void json_walk(bstring prefix, struct json_object *obj)
{
	json_object_object_foreach(obj, key, val) {
		...
		case json_type_array:
			bassign(tmp, prefix);
			bcatcstr(tmp, key);
			log_dbg("a %s=%s", tmp->data,
				json_object_to_json_string(val));
			break;
		default:
			bassign(tmp, prefix);
			bcatcstr(tmp, key);
			log_dbg("%s=%s", tmp->data,
				json_object_to_json_string(val));
			break;
		}
		bdestroy(tmp);
	   ...
	   struct redir_conn_t *conn, struct redir_httpreq_t *httpreq)
{
	...
	safe_snprintf(path, sizeof(path), "/var/coova/scripts/%s.sh", s->data);

	for (i = 0; i < NUM_SERVICES; i++) {
		bassigncstr(b, "");
		if (!strcmp(ewt_services[i].name, (char *)s->data)) {
			ewt_services[i].func(b);
			log_dbg("Internal EWT Service %s -> %s",
				ewt_services[i].name, b->data);
			if (safe_write(1, b->data, b->slen) < 0) {
				log_err(errno, "redir_write()");
			}
			bdestroy(b);
