...
#define LIST 3
	...
	safe_snprintf(line, sizeof(line),
		      ...
		      (int)(this->firstdyn ? this->firstdyn -
			    this->member : -1),
		      (int)(this->lastdyn ? this->lastdyn - this->member : -1),
		      (int)(this->firststat ? this->firststat -
			    this->member : -1),
		      (int)(this->laststat ? this->laststat -
			    this->member : -1), this->listsize);
	...
	for (n = 0; n < this->listsize; n++) {
		...
		if (this->member[n].in_use) {
			...
			if (this->member[n].next == 0 &&
			    (this->member[n].is_static ? this->laststat : this->
			     lastdyn) != &this->member[n]) {
				st[ERR]++;
			} else if (this->member[n].prev == 0 &&
				   (this->member[n].is_static ? this->
				    firststat : this->firstdyn) !=
				   &this->member[n]) {
				st[ERR]++;
			} else {
				st[FREE]++;
		...
		if (this->member[n].peer) {
			struct app_conn_t *appconn =
			    (struct app_conn_t *)this->member[n].peer;
		...
		if (this->member[n].in_use) {
			...
			safe_snprintf(useLine, sizeof(useLine), "%3d/%3d",
				      this->member[n].prev ? (int)(this->
								   member[n].
								   prev -
								   this->
								   member) : -1,
				      this->member[n].next ? (int)(this->
								   member[n].
								   next -
								   this->
								   member) :
	...
	safe_snprintf(line, sizeof(line),
		      ...
		      dyn[FREE], dyn[LIST], dyn[USED], dyn[ERR],
		      dyn[0] + dyn[1] + dyn[2], this->dynsize,
		      dyn[FREE] != dyn[LIST] ? " - Problem!" : "");
	safe_write(fd, line, strlen(line));

	safe_snprintf(line, sizeof(line),
		      "Static address: free %d, avail %d, used %d, err %d, sum %d/%d%s\n",
		      stat[FREE], stat[LIST], stat[USED], stat[ERR],
		      stat[0] + stat[1] + stat[2], this->statsize,
	       ...
	       int allowdyn, int allowstat)
{
	...
	if (!allowdyn) {
		...
		if (((ntohl(addr.s_addr) + start) & m) !=
		    (ntohl(addr.s_addr) & m)) {
			addr.s_addr = htonl(ntohl(addr.s_addr) + start);
			log_err(0,
				"Invalid dhcpstart=%d (%s) (outside of subnet)!",
				start, inet_ntoa(addr));
			return -1;
		}

		if (((ntohl(addr.s_addr) + end) & m) !=
		    (ntohl(addr.s_addr) & m)) {
		...
		if (start > 0 && end > 0) {
			...
			if (end < start) {
				log_err(0,
					"Bad arguments dhcpstart=%d and dhcpend=%d",
					start, end);
				return -1;
			}
			...
			if ((end - start) > dynsize) {
				log_err(0,
					"Too many IPs between dhcpstart=%d and dhcpend=%d",
					start, end);
				return -1;
			}
...
int ippool_newip(struct ippool_t *this,
		 struct ippoolm_t **member, struct in_addr *addr, int statip)
{
	struct ippoolm_t *p;
