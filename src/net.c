...
int net_open(net_interface * netif)
{
	...
	if (
		   ...
		   !_options.layer3 &&
#endif
		   (!(netif->devflags & IFF_UP)
		    || !(netif->devflags & IFF_RUNNING))) {
...
int net_reopen(net_interface * netif)
{
	...
	net_setsockopt(netif->fd, SOL_SOCKET, SO_SNDBUF, &option,
		       sizeof(option));

	option = (int)(default_rcvbuf * 1.1);
	net_setsockopt(netif->fd, SOL_SOCKET, SO_RCVBUF, &option,
		       sizeof(option));
	...
	for (i = 0; i < sctx->count; i++) {
		if (sctx->desc[i].fd == oldfd) {
			for (; i < sctx->count - 1; i++)
				memcpy(&sctx->desc[i], &sctx->desc[i + 1],
				       sizeof(select_fd));
	...
	for (i = 0; i < sctx->count; i++) {
		if (sctx->desc[i].fd == oldfd) {
...
#if defined(USING_POLL) && defined(HAVE_SYS_EPOLL_H)
			{
				...
				if (epoll_ctl
				    (sctx->efd, EPOLL_CTL_DEL, oldfd, &event))
					log_err(errno, "epoll fd %d not found",
						oldfd);
				...
				if (epoll_ctl
				    (sctx->efd, EPOLL_CTL_ADD, newfd, &event))
...
int net_select_rmfd(select_ctx * sctx, int fd)
{
	...
	if (epoll_ctl(sctx->efd, EPOLL_CTL_DEL, fd, &event))
		log_err(errno, "Failed to remove fd %d (%d)", fd, sctx->efd);
#endif
	return 0;
}
...
int net_select_addfd(select_ctx * sctx, int fd, int evts)
{
	 ...
	 */
	if (epoll_ctl(sctx->efd, EPOLL_CTL_ADD, fd, &event))
		log_err(errno, "Failed to add fd %d (%d)", fd, sctx->efd);
#endif
	return 0;
}
...
int net_select(select_ctx * sctx)
{
	...
	do {
		...
		status = select(sctx->maxfd + 1,
				&sctx->rfds,
				&sctx->wfds, &sctx->efds, &sctx->idleTime);

#if(0)
		if (status)
			log_dbg("select() == %d", status);
		{
			int i;
...
#if(_debug_ > 1)
			log_dbg("write %d",
				(sctx->events[idx].events & EPOLLOUT) != 0);
#endif
...
#endif
			if (has_read) {
				sctx->desc[i].cb(sctx->desc[i].ctx,
						 sctx->desc[i].idx);
			}
		}
	}
...
net_read_dispatch_eth(net_interface * netif, net_handler func, void *ctx)
{
	...
	if (netif->pd) {
		...
		cnt =
		    pcap_dispatch(netif->pd, 1, net_pcap_handler,
				  (u_char *) & np);
...
ssize_t net_read_dispatch(net_interface * netif, net_handler func, void *ctx)
{
	...
	length = safe_read(netif->fd,
			   pkt_buffer_head(&pb), pkt_buffer_size(&pb));
	if (length <= 0)
		return length;
	pb.length = length;
	/* tun_decaps_cb */
...
ssize_t net_read_eth(net_interface * netif, void *d, size_t dlen)
{
	ssize_t len = 0;
	...
	if (netif->pd) {
		...
		cnt =
		    pcap_dispatch(netif->pd, 1, net_pcap_read, (u_char *) & np);
	...
	if (netif->rx_ring.frames) {
		log_err(0,
			"shouldn't be reading a mmap'ed interface this way, use dispatch");
		return -1;
	}
#endif
	...
	if (netif->fd) {
		...
		if (len < 0) {
			...
			if (len > dlen) {
				log_warn(0,
					 "data truncated, sending ICMP error %d/%d",
					 len, dlen);
				len = dlen;
			}
		}
		...
		if (len < 0) {
			log_err(errno,
				"net_read_eth(fd=%d, len=%d, mtu=%d) == %d",
		...
		if (_options.ieee8021q) {
			for (cmsg = CMSG_FIRSTHDR(&msg); cmsg;
				...
				if (ulen < 2 * PKT_ETH_ALEN ||
				    len >= PKT_MAX_LEN - 4) {
					log_err(0,
						"bad pkt length to add 802.1q header %d/%d",
						ulen, len);
					break;
				}
#if(_debug_ > 1)
...
ssize_t net_write(int sock, void *d, size_t dlen)
{
	...
	while (left > 0) {
		...
		if (w < 0) {
			log_err(errno, "safe_send(%d, d+%d,%d)", sock, off,
				left);
		      ...
		      struct sockaddr_ll * dest)
{
	...
	len = safe_sendto(fd, d, dlen, 0,
			  (struct sockaddr *)dest, sizeof(struct sockaddr_ll));
	...
	if (len < 0) {
		...
		log_err(errno, "net_write_eth(fd=%d, len=%d) failed", netif->fd,
			dlen);
		return -1;
	}
...
int net_set_mtu(net_interface * netif, size_t mtu)
{
	...
	if (ioctl(netif->fd, SIOCSIFMTU, &ifr) < 0) {
		log_err(errno, "could not set MTU on fd %d", netif->fd);
		return -1;
	}
#endif
	return 0;
...
int net_getip(char *dev, struct in_addr *addr)
{
	...
	if (ioctl(fd, SIOCGIFADDR, (caddr_t) & ifr) >= 0) {
		...
		memcpy(addr, &((struct sockaddr_in *)sa)->sin_addr,
...
int net_open_eth(net_interface * netif)
{
	...
	if (ioctl(netif->fd, SIOCGIFHWADDR, (caddr_t) & ifr) < 0) {
		log_err(errno, "ioctl(d=%d, request=%d) failed", netif->fd,
			SIOCGIFHWADDR);
		return -1;
	}
	...
	if (ifr.ifr_hwaddr.sa_family == ARPHRD_ETHER) {
		...
		if ((netif->flags & NET_USEMAC) == 0) {
			memcpy(netif->hwaddr, ifr.ifr_hwaddr.sa_data,
			       PKT_ETH_ALEN);
		} else if (_options.dhcpmacset) {
			safe_strncpy(ifr.ifr_name, netif->devname,
				     sizeof(ifr.ifr_name));
			memcpy(ifr.ifr_hwaddr.sa_data, netif->hwaddr,
			       PKT_ETH_ALEN);
			if (ioctl(netif->fd, SIOCSIFHWADDR, (caddr_t) & ifr) <
			    0) {
				log_err(errno, "ioctl(d=%d, request=%d) failed",
					netif->fd, SIOCSIFHWADDR);
				return -1;
			}
		}
	}
...
int net_open_eth(net_interface * netif)
{
	...
	*/
	...
	if ((netif->fd = socket(PF_PACKET,
		...
		log_err(errno,
			"socket(domain=%d, type=%lx, protocol=%d) failed",
...

	{
		socklen_t len;
		len = sizeof(default_sndbuf);
		getsockopt(netif->fd, SOL_SOCKET, SO_SNDBUF, &default_sndbuf,
			   &len);
		log_dbg("Net SNDBUF %d", default_sndbuf);
		len = sizeof(default_sndbuf);
		getsockopt(netif->fd, SOL_SOCKET, SO_RCVBUF, &default_rcvbuf,
			   &len);
		log_dbg("Net RCVBUF %d", default_rcvbuf);
	}
#endif
	...
	if (netif->flags & NET_PROMISC) {
		...
		if (ioctl(netif->fd, SIOCGIFFLAGS, (caddr_t) & ifr) == -1) {
			...
			if (ioctl(netif->fd, SIOCSIFFLAGS, (caddr_t) & ifr) ==
			    -1) {
				log_err(errno,
					"Could not set flag IFF_PROMISC");
			}
		}
...
int net_open_eth(net_interface * netif)
{
	...
	for (devnum = 0; devnum < 255; devnum++) {	/* TODO 255 */
		...
		if ((netif->fd = open(devname, O_RDWR)) >= 0)
			break;
		if (errno != EBUSY)
			break;
	}
...
static int rx_ring(net_interface * iface, net_handler func, void *ctx)
{
	...
	for (cnt = 0; cnt < iface->rx_ring.cnt; ++cnt) {
		...
		if (_options.logfacility > 100)
			log_dbg("RX len=%d spanlen=%d (idx %d)", h->tp_len,
				h->tp_snaplen, iface->ifindex);

		pkt_buffer_init(&pb, (uint8_t *) data, h->tp_snaplen,
				h->tp_mac);
		func(ctx, &pb);
...
next:
	...
	if (was_drop) {
		...
		len = sizeof(stats);
		if (!getsockopt
		    (iface->fd, SOL_PACKET, PACKET_STATISTICS, &stats, &len))
...
static void setup_one_ring(net_interface * iface, unsigned ring_size, int mtu,
			   int what)
{
	...
	while (req.tp_block_size > req.tp_frame_size
		...
		ret =
		    net_setsockopt(iface->fd, SOL_PACKET, what, &req,
				   sizeof(req));
		if (!ret)
			break;
	...
	if (ret) {
		log_err(errno, "Failed to set up the %s ring buffer; "
			"block_sz=%d block_nr=%d frame_sz=%d frame_nr=%d page_size=%d",
			name, req.tp_block_size, req.tp_block_nr,
			req.tp_frame_size, req.tp_frame_nr, page_size);
	...
	log_info
	    ("Created %s ring: len=%d; block size=%d; frame size=%d, cnt=%d",
...
static void setup_frames(struct ring *ring, void *data)
{
	...
	for (i = cnt = 0; i < blocks; i++)
		for (j = 0; j < frames; j++)
			ring->frames[cnt++] =
			    data + i * ring->block_size + j * ring->frame_size;
}
...
static void setup_rings(net_interface * iface, unsigned size, int mtu)
{
	...
	val = TPACKET_V2;
	ret =
	    net_setsockopt(iface->fd, SOL_PACKET, PACKET_VERSION, &val,
			   sizeof(val));
	...
	if (ret) {
		log_err(errno,
			"Failed to determine the header length of the ring buffer");
		return;
	}
	...
	val = 1;
	ret =
	    net_setsockopt(iface->fd, SOL_PACKET, PACKET_LOSS, &val,
			   sizeof(val));
...
static void setup_filter(net_interface * iface)
{
	...
	if (iface->idx > 0) {
		...
		if (net_setsockopt
		    (iface->fd, SOL_SOCKET, SO_ATTACH_FILTER, &prog,
		     sizeof(prog)))
