...
static proxy_request *get_request()
{
	...
	if (!requests) {
		...
		requests =
		    (proxy_request *) calloc(max_requests,
...
static int radius_reply(struct radius_t *this,
			struct radius_packet_t *pack, struct sockaddr_in *peer)
{
...
static void close_request(proxy_request * req)
{
	...
	req->url = req->data = req->post = req->dbuf = req->wbuf = 0;
...
static int http_aaa_finish(proxy_request * req)
{
	...
	case RADIUS_CODE_ACCESS_REQUEST:
		...
		radius_default_pack(radius, &req->radius_res,
				    req->
				    authorized ? RADIUS_CODE_ACCESS_ACCEPT :
				    req->
				    challenge ? RADIUS_CODE_ACCESS_CHALLENGE :
			...
			while ((ptr = strtok(parse, "\n"))) {
				...
				struct {
					...
					"ChilliSpot-Bandwidth-Max-Up:",
						    RADIUS_ATTR_VENDOR_SPECIFIC,
						    RADIUS_VENDOR_CHILLISPOT,
						    RADIUS_ATTR_CHILLISPOT_BANDWIDTH_MAX_UP,
						    0}, {
					...
					"ChilliSpot-Max-Input-Octets:",
						    RADIUS_ATTR_VENDOR_SPECIFIC,
						    RADIUS_VENDOR_CHILLISPOT,
						    RADIUS_ATTR_CHILLISPOT_MAX_INPUT_OCTETS,
						    0}, {
					"ChilliSpot-Max-Output-Octets:",
						    RADIUS_ATTR_VENDOR_SPECIFIC,
						    RADIUS_VENDOR_CHILLISPOT,
						    RADIUS_ATTR_CHILLISPOT_MAX_OUTPUT_OCTETS,
						    0}, {
					"ChilliSpot-Max-Total-Octets:",
						    RADIUS_ATTR_VENDOR_SPECIFIC,
						    RADIUS_VENDOR_CHILLISPOT,
						    RADIUS_ATTR_CHILLISPOT_MAX_TOTAL_OCTETS,
						    0}, {
					"ChilliSpot-Max-Input-Gigawords:",
						    RADIUS_ATTR_VENDOR_SPECIFIC,
						    RADIUS_VENDOR_CHILLISPOT,
						    RADIUS_ATTR_CHILLISPOT_MAX_INPUT_GIGAWORDS,
						    0}, {
					"ChilliSpot-Max-Output-Gigawords:",
						    RADIUS_ATTR_VENDOR_SPECIFIC,
						    RADIUS_VENDOR_CHILLISPOT,
						    RADIUS_ATTR_CHILLISPOT_MAX_OUTPUT_GIGAWORDS,
						    0}, {
					"ChilliSpot-Max-Total-Gigawords:",
						    RADIUS_ATTR_VENDOR_SPECIFIC,
						    RADIUS_VENDOR_CHILLISPOT,
						    RADIUS_ATTR_CHILLISPOT_MAX_TOTAL_GIGAWORDS,
						    0}, {
	...
	case RADIUS_CODE_ACCESS_REQUEST:
		{
			...
			radius_addattr(radius, &req->radius_res,
				       RADIUS_ATTR_MESSAGE_AUTHENTICATOR, 0, 0,
				       0, NULL, RADIUS_MD5LEN);

			memset(req->radius_res.authenticator, 0,
			       RADIUS_AUTHLEN);
			memcpy(req->radius_res.authenticator,
			       req->radius_req.authenticator, RADIUS_AUTHLEN);

			if (!radius_getattr
			    (&req->radius_res, &ma,
			     RADIUS_ATTR_MESSAGE_AUTHENTICATOR, 0, 0, 0)) {
				radius_hmac_md5(radius, &req->radius_res,
						radius->secret,
						radius->secretlen, ma->v.t);
			}
...
static int http_aaa_setup(struct radius_t *radius, proxy_request * req)
{
	...
	if ((curl = req->curl = curl_easy_init()) != NULL) {
		...
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION,
				 (curl_write_callback) bstring_data);
...
static int http_aaa_setup(struct radius_t *radius, proxy_request * req)
{
	...
	if (get_urlparts
	    ((char *)req->url->data, hostname, sizeof(hostname), &port,
	     &uripos))
		return -1;
...
static void process_radius(struct radius_t *radius,
			   ...
			   struct sockaddr_in *peer)
{
	...
	case RADIUS_CODE_ACCESS_REQUEST:
		...
		if (radius_getattr
		    (pack, &attr, RADIUS_ATTR_SERVICE_TYPE, 0, 0, 0)) {
	...
	case RADIUS_CODE_ACCOUNTING_REQUEST:
		...
		if (radius_getattr
		    (pack, &attr, RADIUS_ATTR_ACCT_STATUS_TYPE, 0, 0, 0)) {
			...
			case RADIUS_STATUS_TYPE_ACCOUNTING_OFF:
				...
				break;
			default:
				log_err(0, "unsupported acct-status-type %d",
					ntohl(attr->v.i));
	...
	if (!error) {
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_USER_NAME, 0, 0, 0)) {
		...
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_USER_PASSWORD, 0, 0, 0)) {
		...
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_CHAP_CHALLENGE, 0, 0, 0)) {
		...
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_CALLING_STATION_ID, 0, 0, 0)) {
			bcatcstr(req->url, "&mac=");
			bassignblk(tmp, attr->v.t, attr->l - 2);
			redir_urlencode(tmp, tmp2);
			bconcat(req->url, tmp2);
		...
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_FRAMED_IP_ADDRESS, 0, 0, 0)) {
		...
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_ACCT_SESSION_ID, 0, 0, 0)) {
		...
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_NAS_IDENTIFIER, 0, 0, 0)) {
			bcatcstr(req->url, "&nasid=");
			bassignblk(tmp, attr->v.t, attr->l - 2);
			redir_urlencode(tmp, tmp2);
			bconcat(req->url, tmp2);
		...
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_ACCT_SESSION_TIME, 0, 0, 0)) {
		...
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_EAP_MESSAGE, 0, 0, 0)) {
			int instance = 1;
			...
			do {
				attr = NULL;
				if (!radius_getattr
				    (pack, &attr, RADIUS_ATTR_EAP_MESSAGE, 0, 0,
				     instance++) && attr) {
					bcatblk(tmp, attr->v.t,
						(size_t) attr->l - 2);
				}
			} while (attr);
		...
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_ACCT_INPUT_OCTETS, 0, 0, 0)) {
			char *direction = _options.swapoctets ? "up" : "down";
			uint64_t val = (uint64_t) ntohl(attr->v.i);
			uint64_t input = val;
			if (!radius_getattr
			    (pack, &attr, RADIUS_ATTR_ACCT_INPUT_GIGAWORDS, 0,
			     0, 0)) {
				val = (uint64_t) ntohl(attr->v.i);
				input |= (val << 32);
			}
			bassignformat(tmp, "&bytes_%s=%lld", direction,
				      (long long)input);
			bconcat(req->url, tmp);
			if (!radius_getattr
			    (pack, &attr, RADIUS_ATTR_ACCT_INPUT_PACKETS, 0, 0,
			     0)) {
				uint32_t sval = ntohl(attr->v.i);
				bassignformat(tmp, "&pkts_%s=%ld", direction,
					      (long)sval);
				bconcat(req->url, tmp);
			}
		}
		...
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_ACCT_OUTPUT_OCTETS, 0, 0, 0)) {
			char *direction = _options.swapoctets ? "down" : "up";
			uint64_t val = (uint64_t) ntohl(attr->v.i);
			uint64_t output = val;
			if (!radius_getattr
			    (pack, &attr, RADIUS_ATTR_ACCT_OUTPUT_GIGAWORDS, 0,
			     0, 0)) {
				val = (uint64_t) ntohl(attr->v.i);
				output |= (val << 32);
			}
			...
			bconcat(req->url, tmp);
			if (!radius_getattr
			    (pack, &attr, RADIUS_ATTR_ACCT_OUTPUT_PACKETS, 0, 0,
			     0)) {
				uint32_t sval = ntohl(attr->v.i);
				bassignformat(tmp, "&pkts_%s=%ld", direction,
					      (long)sval);
...
int main(int argc, char **argv)
{
...
#else
	{
		...
		safe_strncpy(ifr.ifr_name, _options.dhcpif,
			     sizeof(ifr.ifr_name));

		if (ioctl(fd, SIOCGIFHWADDR, (caddr_t) & ifr) == 0) {
			memcpy(nas_hwaddr, ifr.ifr_hwaddr.sa_data,
			       PKT_ETH_ALEN);
		}
	...
	if (radius_new(&radius_auth, &radiuslisten,
		       ...
		       0, 0) || radius_init_q(radius_auth, 0)) {
	...
	if (radius_new(&radius_acct, &radiuslisten,
		       ...
		       0, 0) || radius_init_q(radius_acct, 0)) {
		log_err(0, "Failed to create radius");
		return -1;
	}
	...
	if (_options.gid && setgid(_options.gid)) {
		log_err(errno,
			"setgid(%d) failed while running with gid = %d\n",
			_options.gid, getgid());
	}

	if (_options.uid && setuid(_options.uid)) {
		log_err(errno,
			"setuid(%d) failed while running with uid = %d\n",
			_options.uid, getuid());
	}
	...
	while (keep_going) {
...
#ifdef USING_CURL
		curl_multi_fdset(curl_multi, &fdread, &fdwrite, &fdexcep,
				 &maxfd);
#else
		for (idx = 0; idx < max_requests; idx++) {
			conn_fd(&requests[idx].conn, &fdread, &fdwrite,
				&fdexcep, &maxfd);
		}
#endif
		...
		default:
			...
			while ((msg =
				curl_multi_info_read(curl_multi, &msgs_left))) {
...
#if(_debug_)
						log_dbg
						    ("HTTP completed with status %d\n",
						     msg->data.result);
#endif
						http_aaa_finish(&requests[idx]);
					} else {
						log_err(0,
							"Could not find request in queue");
					}
				}
			}
#else
			for (idx = 0; idx < max_requests; idx++) {
				conn_update(&requests[idx].conn, &fdread,
					    &fdwrite, &fdexcep);
			}
#endif
