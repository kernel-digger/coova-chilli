...
static bstring inject_fmt(redir_request * req, struct redir_conn_t *conn)
{
	...
	if (conn) {
		...
		redir_chartohex(conn->s_state.redir.uamchal, hexchal,
				REDIR_MD5LEN);
		redir_wispr2_reply(req->parent, conn, REDIR_NOTYET, 0, hexchal,
				   0, 0, req->ibuf);
	}
...
static redir_request *get_request()
{
	...
	if (!max_requests) {
		...
		requests =
		    (redir_request *) calloc(max_requests,
					     sizeof(redir_request));
...
static int redir_cli_rewrite(redir_request * req, struct conn_t *conn)
{
	if (conn_write_remaining(conn)) {
		int w = net_write(req->socket_fd,
				  ...
				  conn->read_buf->slen - conn->read_pos);
     ...
     */
		if (inject && !req->headers) {
			...
			if ((eoh = strstr((char *)req->hbuf->data, newline))) {
				...
				if (strncmp
				    ((char *)req->hbuf->data, "HTTP/1.", 7)
				    || strncmp((char *)req->hbuf->data + 8,
					       " 2", 2)) {

					log_dbg("Not HTTP/1.X 2XX reply");
					bcatblk(newhdr, req->hbuf->data,
						header_len + 4);
					...
					if (req->html) {
						...
						if (req->chunked) {
							bcatblk(newhdr, newline,
								2);
						}
					}
				}

				bcatblk(newhdr, eoh + 4,
					req->hbuf->slen - header_len - 4);
				if (req->clen > 0)	/* adjust clen */
					req->clen -=
					    (req->hbuf->slen - header_len - 4);
				redir_cli_write(req, newhdr->data,
						newhdr->slen);
				req->headers = 1;
				bdestroy(newhdr);
			}
...
static int check_regex(regex_t * re, char *regex, char *s)
{
	int ret;
...
redir_handle_url(struct redir_t *redir,
		 ...
		 struct sockaddr_in *peer, redir_request * req)
{
	int port = 80;
	int matches = 1;
	int i = -1;
	...
	if (conn->s_params.flags & UAM_INJECT_URL) {
		safe_strncpy((char *)req->inject_url,
			     (char *)conn->s_params.url, REDIRINJECT_MAX);
		hasInject = 1;
	} else if (_options.inject && *_options.inject) {
		safe_strncpy((char *)req->inject_url,
			     (char *)_options.inject, REDIRINJECT_MAX);
		hasInject = 1;
	} else {
#endif
		for (i = 0; i < MAX_REGEX_PASS_THROUGHS; i++) {
			...
			if (_options.regex_pass_throughs[i].regex_host[0]) {
				switch (check_regex
					(&_options.regex_pass_throughs[i].
					 re_host,
					 _options.regex_pass_throughs[i].
					 regex_host, httpreq->host)) {
				case -1:
					return -1;
				case 1:
					matches =
			...
			if (matches
			    && _options.regex_pass_throughs[i].regex_qs[0]) {
				switch (check_regex
					(&_options.regex_pass_throughs[i].re_qs,
					 _options.regex_pass_throughs[i].
					 regex_qs, httpreq->qs)) {
				case -1:
					return -1;
...
int redir_accept2(struct redir_t *redir, int idx)
{
	...
	if (idx == 1 && _options.uamui) {
		...
		safe_snprintf(buffer, sizeof(buffer), "%d",
		...
		default:
			if (status > 0) {
				...
				for (idx = 0; idx < max_requests; idx++) {
					 ...
					 */
					conn_select_update(&requests[idx].conn,
							   &sctx);
					...
					if (requests[idx].inuse
					    && requests[idx].socket_fd) {
						int fd =
						    requests[idx].socket_fd;

#ifdef HAVE_SSL
						if (requests[idx].sslcon) {
							if (openssl_check_accept
							    (requests[idx].
							     sslcon, 0) < 0) {
								log_dbg
								    ("ssl error %d",
						...
						case 1:
							{
								if (requests
									...
									if (requests[idx].sslcon) {
										 ...
										 */
										r = openssl_read(requests[idx].sslcon, b, sizeof(b) - 1, 0);
									} else
#endif
									...
									if (r <=
										...
										if (!(r == -1 && (errno == EWOULDBLOCK || errno == EAGAIN))) {
											if (redir_cli_rewrite(&requests[idx], &requests[idx].conn) == 0) {
												log_dbg
									     ...
									     0)
									{
										...
										if (r != w) {
											log_err
											    (errno,
											     "problem writing what we read from client");
											redir_conn_finish
											    (&requests
											     [idx].
											     conn,
