...
void location_close_conn(struct app_conn_t *conn, int close)
{
	...
	if (close)
		conn->loc_search_node->closed_sess_count++;
	else
		conn->loc_search_node->roamed_out_sess_count++;

	conn->loc_search_node->closed_bytes_up +=
	    (conn->s_state.output_octets_old - conn->s_state.output_octets);

	conn->loc_search_node->closed_bytes_down +=
	    (conn->s_state.input_octets_old - conn->s_state.input_octets);
...
static int
avl_comp(const void *k1, const void *k2, void *ptr __attribute__ ((unused)))
{
...
void location_add_conn(struct app_conn_t *appconn, char *loc)
{
	...
	if (appconn->loc_search_node != NULL) {
		...
		list_init_node(&appconn->loc_sess);
	...
	if (loc_search) {
...
#endif
		int active_users = 0, internet_users =
		    0, active_internet_users = 0;
		...
		if (timespan >= 1) {
			...
			if (loc_search->total_sess_count > 0) {
				...
				log_dbg("location has %d sessions attached! ",
					loc_search->total_sess_count);
				log_dbg("(last queried %d seconds ago)\n",
					(act_mainclock -
					 loc_search->last_queried));
				...
				while (ln != &loc_search->loc_sess_head) {
					appconn =
					    container_of(ln, struct app_conn_t,
							 loc_sess);
					...
					if (_options.uamgardendata) {
						garden_bytes_up =
						    (appconn->s_state.
						     ...
						     appconn->s_state.
						     garden_output_octets_old);
						garden_bytes_down =
						    (appconn->s_state.
						     garden_input_octets -
						     appconn->s_state.
						     garden_input_octets_old);
						if (_options.swapoctets) {
						...
						other_bytes_up =
						    (appconn->s_state.
						     other_output_octets -
						     appconn->s_state.
						     other_output_octets_old);
						other_bytes_down =
						    (appconn->s_state.
						     other_input_octets -
						     appconn->s_state.
						     other_input_octets_old);
						if (_options.swapoctets) {
							swap = other_bytes_up;
							other_bytes_up =
							    other_bytes_down;
					...
					if (list) {
						log_dbg
						    ("mac: %.2X-%.2X-%.2X-%.2X-%.2X-%.2X up: %d down: %d\n",
						     appconn->hismac[0],
						     appconn->hismac[1],
						     appconn->hismac[2],
						     appconn->hismac[3],
				...
				fprintf(stderr,
					"location has no sessions attached!\n");
				bassignformat(tmp,
					      json ?
			...
			if (list) {
				bassignformat(tmp, json
					      ...
					      :
					      "\n\tclosed_bytes_up = %d\n\tclosed_kbps_up = %d\n\tclosed_bytes_down = %d\n\tclosed_kbps_down = %d",
					      loc_search->closed_bytes_up,
					      (loc_search->closed_bytes_up *
					       8) / (1024 * timespan)
					      , loc_search->closed_bytes_down,
					      (loc_search->closed_bytes_down *
					       8) / (1024 * timespan));
				bconcat(s, tmp);
			...
			loc_search->closed_bytes_up =
			    loc_search->closed_bytes_down = 0;
			...
			if (_options.uamgardendata) {
				...
				other_total_bytes_down +=
				    loc_search->other_closed_bytes_down;
			...
			bassignformat(tmp, json ?
				      ...
				      "\n\ttotal_kbps_up = %d\n\ttotal_bytes_down = %d"
				      "\n\ttotal_kbps_down = %d",
				      active_users, internet_users,
				      active_internet_users, total_bytes_up,
				      (total_bytes_up * 8) / (1024 * timespan),
				      total_bytes_down,
				      (total_bytes_down * 8) / (1024 *
								timespan));
			bconcat(s, tmp);
#ifdef ENABLE_GARDENACCOUNTING
			if (_options.uamgardendata) {
				bassignformat(tmp, json ?
					      ...
					      "\n\tgarden_total_kbps_down = %d",
					      garden_total_bytes_up,
					      (garden_total_bytes_up * 8) /
					      (1024 * timespan),
					      garden_total_bytes_down,
					      (garden_total_bytes_down * 8) /
					      (1024 * timespan));
				bconcat(s, tmp);
				bassignformat(tmp, json ?
					      ",\"other_total_bytes_up\":%d,"
					      ...
					      "\n\tother_total_kbps_down = %d",
					      other_total_bytes_up,
					      (other_total_bytes_up * 8) /
					      (1024 * timespan),
					      other_total_bytes_down,
					      (other_total_bytes_down * 8) /
					      (1024 * timespan));
				bconcat(s, tmp);
			}
#endif
