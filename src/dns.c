...
ssize_t dns_fullname(char *data, size_t dlen,	/* buffer to store name */
...
static void add_A_to_garden(uint8_t * p)
{
	...
	if (pass_through_add(dhcp->pass_throughs,
			     MAX_PASS_THROUGHS, &dhcp->num_pass_throughs, &pt, 1
#ifdef HAVE_PATRICIA
			     , dhcp->ptree_dyn
#endif
	    )) ;
...
#define return_error { log_dbg("failed parsing DNS packet"); return -1; }
	...
	if (antidnstunnel && namelen > 128) {
		log_warn(0, "dropping dns for anti-dnstunnel (namelen: %d)",
			 namelen);
		return -1;
	}
	...
	if (q) {
		if (dns_fullname
		    ((char *)question, qsize, *pktp, *left, opkt, olen, 0) < 0)
			return_error;
		...
		*left = len;
		...
		if (!isReq && *qmatch == -1 &&
			...
			for (id = 0;
			     _options.uamdomains[id] && id < MAX_UAM_DOMAINS;
			     id++) {

				size_t qst_len = strlen((char *)question);
				size_t dom_len =
				    strlen(_options.uamdomains[id]);
	...
	if (antidnstunnel && !required) {
		log_warn(0,
			 "dropping dns for anti-dnstunnel (type %d: length %d)",
			 type, rdlen);
		return -1;
	}
