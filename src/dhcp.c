...
struct chilli_peer *get_chilli_peer(int id)
{
	...
	if (!chilli_peers) {
		...
		chilli_peers =
		    (struct chilli_peer *)calloc(8, sizeof(struct chilli_peer));
		p = chilli_peers + _options.peerid;
		memcpy(p->mac, dhcp_nexthop(dhcp), PKT_ETH_ALEN);
		memcpy(&p->addr, &_options.uamlisten,
		       sizeof(_options.uamlisten));
		      ...
		      struct pkt_iphdr_t *iph, int len, int dst)
{
	if (conn && _options.uamgardendata && _options.uamotherdata) {
		struct app_conn_t *appconn =
		    dhcp_get_appconn_pkt(conn, iph, dst);
		if (appconn) {
				...
				if (admin_session.s_state.authenticated) {
					if (dst)
						admin_session.s_state.
						    other_output_octets += len;
					else
						admin_session.s_state.
						    other_input_octets += len;
				}
			} else {
				if (dst)
					appconn->s_state.other_input_octets +=
...
struct app_conn_t *dhcp_get_appconn_ip(struct dhcp_conn_t *conn,
				       struct in_addr *dst)
{
...
struct app_conn_t *dhcp_get_appconn_pkt(struct dhcp_conn_t *conn,
					struct pkt_iphdr_t *iph, char is_dst)
{
#ifdef ENABLE_LAYER3
	...
	case DHCP_AUTH_ROUTER:
		{
			...
			dst.s_addr = ((is_dst) ? iph->daddr : iph->saddr);
...
static struct app_conn_t *dhcp_get_appconn_addr(struct dhcp_conn_t *conn,
						struct in_addr *dst)
{
#ifdef ENABLE_LAYER3
	switch (conn->authstate) {
 ...
 **/
int dhcp_sendGARP(struct dhcp_t *this, int idx)
{
	...
	if (!dhcp_hashget(this, &conn, hwaddr)) {
		...
		for (n = 0; n < DHCP_DNAT_MAX; n++) {
			if (conn->dnat[n].dst_ip) {
				uint8_t *ip =
				    (uint8_t *) & conn->dnat[n].dst_ip;
				log_dbg("Resetting dst %d.%d.%d.%d:%d", ip[0],
					ip[1], ip[2], ip[3],
					conn->dnat[n].dst_port);
			}
			if (conn->dnat[n].src_ip) {
				uint8_t *ip =
				    (uint8_t *) & conn->dnat[n].src_ip;
				log_dbg("Resetting src %d.%d.%d.%d:%d", ip[0],
					ip[1], ip[2], ip[3],
					conn->dnat[n].src_port);
			}
		}
	}
}
	      ...
	      unsigned char *hismac, uint8_t * packet, size_t length)
{
	...
	if (idx < 0) {
		...
		for (i = 0; i < MAX_RAWIF && this->rawif[i].fd; i++)
			ret =
			    dhcp_net_send(&this->rawif[i], hismac, packet,
					  length);
		return ret;
	}
...
int vlanupdate_script(struct dhcp_conn_t *conn, char *script, uint16_t oldtag)
{
	int status;
	uint16_t tag;
...
void dhcp_checktag(struct dhcp_conn_t *conn, uint8_t * pack)
{
	if (_options.ieee8021q && is_8021q(pack)) {
		...
		if ((tag & PKT_8021Q_MASK_VID) !=
			...
			log_dbg
			    ("IEEE 802.1Q: %.2x:%.2x:%.2x:%.2x:%.2x:%.2x on VLAN %d",
			...
			if (_options.vlanupdate) {
				vlanupdate_script(conn, _options.vlanupdate,
						  oldtag);
			}
			...
			if (_options.vlanlocation && appconn) {
				...
				safe_snprintf(vid, sizeof(vid), "%d",
					      (int)ntohs(tag &
							 PKT_8021Q_MASK_VID));
				chilli_learn_location((uint8_t *) vid,
						      strlen(vid), appconn, 1);
			}
#endif
...
int dhcp_newconn(struct dhcp_t *this,
		 struct dhcp_conn_t **conn, uint8_t * hwaddr)
{
	if (_options.debug)
		  ...
		  struct nfq_data *nfa, void *cbdata)
{
	...
	if (hw && ret > 0) {
		...
		struct pkt_tcphdr_t *pack_tcph =
		    (struct pkt_tcphdr_t *)(data + PKT_IP_HLEN);
		struct pkt_udphdr_t *pack_udph =
		    (struct pkt_udphdr_t *)(data + PKT_IP_HLEN);
		struct dhcp_conn_t *conn;
		struct in_addr addr;

		if (!dhcp_hashget(dhcp, &conn, hw->hw_addr)) {
			struct app_conn_t *appconn =
			    (struct app_conn_t *)conn->peer;
		...
		if (_options.debug) {
			...
			log_dbg
			    ("NFQUEUE: From %.2X-%.2X-%.2X-%.2X-%.2X-%.2X %s %s",
			...
			log_dbg("NFQUEUE: To %s %s %d %s",
				...
				pack_iph->protocol ==
				PKT_IP_PROTO_TCP ? "TCP" : "Other",
				pack_iph->protocol ==
				PKT_IP_PROTO_UDP ? ntohs(pack_udph->
							 dst) : pack_iph->
				protocol ==
				PKT_IP_PROTO_TCP ? ntohs(pack_tcph->dst) : 0,
				result == NF_ACCEPT ? "Accept" : "Drop");
		}
	}
		   ...
		   struct nfq_data *nfa, void *cbdata)
{
	...
	if (ret > 0) {
		...
		if (ippool_getip(ippool, &ipm, &addr)) {
			...
			if (_options.debug)
				log_dbg
				    ("dropping packet with unknown destination: %s",
				     inet_ntoa(addr));
	     ...
	     struct in_addr *uamlisten, uint16_t uamport, int noc2c)
{
...
#ifdef ENABLE_MULTILAN
	{
		...
		for (i = 0, idx = 1; i < MAX_MOREIF &&
			...
			if (net_init
			    (&dhcp->rawif[idx], _options.moreif[i].dhcpif,
#ifdef ETH_P_ALL
			     ETH_P_ALL,
#else
			     0,
	...
	if (getenv("NFQUEUE_IN") && getenv("NFQUEUE_OUT")) {
		int q1 = 0, q2 = 1;
		char *e;
		if ((e = getenv("NFQUEUE_IN")))
			q1 = atoi(e);
		if ((e = getenv("NFQUEUE_OUT")))
			q2 = atoi(e);
		...
		if (fd > 0) {
			...
			if (_options.dhcprelayip.s_addr)
				addr.sin_addr.s_addr =
				    _options.dhcprelayip.s_addr;
			else
				addr.sin_addr.s_addr =
				    _options.uamlisten.s_addr;
			...
			if (bind(fd, (struct sockaddr *)&addr, sizeof(addr)) <
			    0) {
				log_err(errno,
					"socket or bind failed for dhcp relay!");
				close(fd);
				fd = -1;
			}
		}
		...
		case '=':
			{
				...
				case 0:
					{
						...
						if (sscanf
						    (bp,
						     ...
						     &tmp[5]) != 6) {
							log_err(0,
								"Parse error in ethers file at line %d",
								lineno);
							state = -1;
						} else {
							mac[0] =
				...
				if (newline || state == 2) {
					log(LOG_NOTICE,
					    "Reserving IP MAC=%.2X-%.2X-%.2X-%.2X-%.2X-%.2X IP %s",
					    mac[0], mac[1], mac[2], mac[3],
					    mac[4], mac[5], inet_ntoa(ip));
...
void dhcp_free(struct dhcp_t *dhcp)
{
	...
	if (dhcp->hash)
		free(dhcp->hash);
	if (!_options.uid)
		dev_set_flags(dhcp->rawif[0].devname, dhcp->rawif[0].devflags);
...
static
void tun_sendRESET(struct tun_t *tun, uint8_t * pack,
		   struct app_conn_t *appconn)
{
#if !defined(HAVE_NETFILTER_QUEUE) && !defined(HAVE_NETFILTER_COOVA)
	uint8_t tcp_pack[1500];
	tun_encaps(tun, tcp_pack, tcprst(tcp_pack, pack, 1),
...
void dhcp_sendRESET(struct dhcp_conn_t *conn, uint8_t * pack, char reverse)
{
	...
	uint8_t tcp_pack[1500];
	struct dhcp_t *this = conn->parent;
	dhcp_send(this, dhcp_conn_idx(conn), conn->hismac, tcp_pack,
		  tcprst(tcp_pack, pack, reverse));
#endif
}
#endif
	     ...
	     size_t * plen, char isReq)
{
	...
	if (*plen < DHCP_DNS_HLEN + sizeofudp(pack)) {
		...
		struct app_conn_t *appconn =
		    dhcp_get_appconn_pkt(conn, pkt_iphdr(pack), !isReq);
#endif
...
#if(_debug_)
					log_dbg
					    ("Dropping unexpected DNS response");
#endif
					return 0;
				}
			}
	...
	return isReq ? dhcp_nakDNS(conn,pack,*plen) : 0; \
		...
		if (isReq &&
			...
			if (!match && aliasname) {
				...
				if (match) {
					memcpy(reply, &_options.uamalias.s_addr,
					       4);
				}
			}
			...
			if (match) {
				...
				/* UDP header */
				udp_len =
				    query_len + DHCP_DNS_HLEN + PKT_UDP_HLEN;
				...
				answer_iph->check = 0;	/* Calculate at end of packet */
				memcpy(&answer_iph->daddr, &iph->saddr,
				       PKT_IP_ALEN);
				memcpy(&answer_iph->saddr, &iph->daddr,
				       PKT_IP_ALEN);

				/* Ethernet header */
				memcpy(answer_ethh->dst, &ethh->src,
				       PKT_ETH_ALEN);
				memcpy(answer_ethh->src, &ethh->dst,
				       PKT_ETH_ALEN);
				...
				OTHER_SENDING(conn, answer_iph);
				dhcp_send(dhcp, dhcp_conn_idx(conn),
					  conn->hismac, answer, length);
				return 0;
			}
		}
#ifdef ENABLE_DNSLOG
		if (isReq && _options.dnslog) {
			int fd =
			    open(_options.dnslog, O_WRONLY | O_APPEND | O_CREAT,
				 0666);
			if (fd > 0) {
				char line[512];
				char *username = 0;
				...
				log_err(errno, "could not open log file %s",
					_options.dnslog);
			}
		}
#endif
...
#ifdef ENABLE_IPV6
		if (_options.ipv6 && mod > 0 && !isReq && an_mark
		    && ancount > 0) {
			...
			int new_ancount = 0;
			int len =
			    olen - (an_mark - ((uint8_t *) dnsp->records));

			for (i = 0; i < ancount; i++) {

				ssize_t namelen =
				    dns_fullname((char *)n, sizeof(n) - 1,
						 p, len, (uint8_t *) dnsp, olen,
						 0);
			...
			if (new_ancount > 0) {
				...
				if (newlen < 0 || newlen > 1500) {
					log_err(0,
						"Problem growing DNS packet from %d to %d (%d %d)",
						*plen, newlen, (an_mark - pack),
						bdiff);
				} else {
					uint16_t data_len =
					    bdiff + (uint16_t) (an_mark -
								((uint8_t *)
								 dnsp));
					...
					struct pkt_udphdr_t *udph =
					    pkt_udphdr(pack);
					log_dbg
					    ("Growing DNS packet from %d to %d (%d %d) dt=%d",
					     *plen, newlen, (an_mark - pack),
					     bdiff, data_len);
...
int dhcp_uam_nat(struct dhcp_conn_t *conn,
		 ...
		 struct pkt_iphdr_t *iph,
		 struct pkt_tcphdr_t *tcph, struct in_addr *addr, uint16_t port)
{
	int n;
	int pos = -1;
...
int dhcp_uam_unnat(struct dhcp_conn_t *conn,
		   struct pkt_ethhdr_t *ethh,
		   struct pkt_iphdr_t *iph, struct pkt_tcphdr_t *tcph)
{
	int n;
	for (n = 0; n < DHCP_DNAT_MAX; n++) {
...
#ifdef ENABLE_TAP
			if (_options.usetap) {
				memcpy(ethh->src, conn->dnat[n].mac,
				       PKT_ETH_ALEN);
			}
#endif
...
int dhcp_dnsDNAT(struct dhcp_conn_t *conn,
		 uint8_t * pack, size_t * len, char *do_checksum)
{
	...
	if (iph->protocol == PKT_IP_PROTO_UDP &&
	    udph->src == htons(DHCP_MDNS) && udph->dst == htons(DHCP_MDNS)) {
		log_dbg("mDNS packet");
		   ...
		   uint8_t * pack, size_t * len, char *do_checksum)
{
	...
	if ((this->anydns ||
		...
		if (this->anydns &&
		    conn->dnatdns && iph->saddr != conn->dnatdns) {
		      ...
		      struct pkt_ipphdr_t *ipph, int dst)
{
	...
	if (dhcp->ptree && dhcp->ptree_dyn) {
		...
		switch (garden_check(_options.pass_throughs,
				     &_options.num_pass_throughs, &pt, ipph, dst
#ifdef HAVE_PATRICIA
				     , 0
#endif
			)) {
...
#ifdef ENABLE_IPWHITELIST
	if (!found && _options.ipwhitelist && dhcp_ipwhitelist(ipph, dst))
		found = 1;
#endif
		...
		if (found) {
			...
			if (appconn) {
				...
				if (_options.swapoctets) {
					if (!dst)
						appconn->s_state.
						    garden_output_octets += len;
					else
						appconn->s_state.
						    garden_input_octets += len;
					if (admin_session.s_state.authenticated) {
						if (!dst)
							admin_session.s_state.
...
#ifdef ENABLE_UAMUIPORT
		     || (_options.uamuiport
			 && tcph->src == htons(_options.uamuiport))
#endif
		    )) {
...
#if(_debug_ > 0)
				log_dbg
				    ("rewriting packet for post-auth proxy %s:%d",
		  ...
		  char do_reset, char *do_checksum)
{
	...
	if (dhcp_garden_check(this, conn, 0, (struct pkt_ipphdr_t *)iph, 0))
		return 0;
 ...
 **/
int dhcp_getdefault(uint8_t * pack)
{
		...
		struct dhcp_conn_t *conn)
{
	...
	if (is_req_dhcp) {
		...
		memcpy(&pack_dhcp->sname, conn->dhcp_opts.sname,
		       DHCP_SNAME_LEN);
	...
	if (_options.dhcp_options[0]) {
		...
		if (!dhcp_gettag(req_dhcp, udph_len - PKT_UDP_HLEN,
			...
			while (opt && opt->t && opt->l) {
				...
				log_dbg("DHCP Type: %d Length: %d", (int)opt->t,
					(int)opt->l);
...
dhcp_handler(int type,
	     ...
	     uint8_t * pack, size_t len, uint8_t * packet, size_t pos)
{
		...
		case CHILLI_DHCP_ACK:
			{
				...
				if (!dhcp_gettag
				    (dhcpp,
				     ntohs(pkt_udphdr(pack)->len) -
				     PKT_UDP_HLEN, &opt82, DHCP_OPTION_82)) {
					...
					if (!appconn)
						chilli_getconn(&appconn,
							       dhcpp->yiaddr, 0,
							       0);

					if (appconn) {
						chilli_learn_location(opt82->v,
								      opt82->l,
								      appconn,
								      1);
					} else {
						log_dbg
						    ("no appconn for option 82");
					}
				}
			}
			break;
...
int dhcp_sendOFFER(struct dhcp_conn_t *conn, uint8_t * pack, size_t len)
{
	...
	pos = dhcp_handler(CHILLI_DHCP_OFFER, conn, pack, len, packet, pos);
	...
	return dhcp_send(this, dhcp_conn_idx(conn), conn->hismac, packet,
			 length);
}
...
int dhcp_sendACK(struct dhcp_conn_t *conn, uint8_t * pack, size_t len)
{
	...
	pos = dhcp_handler(CHILLI_DHCP_ACK, conn, pack, len, packet, pos);
...
int dhcp_sendNAK(struct dhcp_conn_t *conn, uint8_t * pack, size_t len)
{
	...
	pos = dhcp_handler(CHILLI_DHCP_NAK, conn, pack, len, packet, pos);
...
static int dhcp_relay(struct dhcp_t *this, uint8_t * pack, size_t len)
{
...

	{
		...
		if (!dhcp_gettag
		    (pack_dhcp, ntohs(pack_udph->len) - PKT_UDP_HLEN, &tag,
	...
	(void)dhcp_handler(CHILLI_DHCP_RELAY, 0, pack, len, 0, 0);

	log_dbg("Sending DHCP relay packet to %s", inet_ntoa(addr.sin_addr));
...
int dhcp_getreq(struct dhcp_ctx *ctx, uint8_t * pack, size_t len)
{
	...
	if (!dhcp_gettag
	    (pkt_dhcppkt(pack), ntohs(pack_udph->len) - PKT_UDP_HLEN,
	...
	case DHCPREQUEST:
		{
			...
			if (!send_ack)
				if (!memcmp
				    (&conn->hisip.s_addr, &addr.s_addr, 4))
					send_ack = 1;
		   ...
		   struct in_addr *dns1, struct in_addr *dns2)
{
	...
	if (_options.usetap && !_options.noarpentries) {
		...
		if (sockfd > 0) {
			...
			log_dbg
			    ("ARP Entry: %s -> %.2x:%.2x:%.2x:%.2x:%.2x:%.2x",
			     inet_ntoa(conn->hisip), conn->hismac[0],
			     conn->hismac[1], conn->hismac[2], conn->hismac[3],
			     conn->hismac[4], conn->hismac[5]);
...
int dhcp_receive_ip(struct dhcp_ctx *ctx, uint8_t * pack, size_t len)
{
	...
	if (eth_tot_len > (uint16_t) len) {
		log_dbg
		    ("dropping ip packet; ip-len=%d + eth-hdr=%d > read-len=%d",
		     iph_tot_len, sizeofeth(pack), (int)len);
	...
	if (iph_tot_len > _options.mtu ||
	    iphdr_more_frag(pack_iph) || iphdr_offset(pack_iph)) {
		uint8_t icmp_pack[1500];
	...
	if (pack_iph->protocol == PKT_IP_PROTO_UDP) {
		...
		if (udph_len < PKT_UDP_HLEN ||
			...
			log_dbg
			    ("dropping udp packet; ip-len=%d != udp-len=%d + ip-hdr=20",
			     (int)iph_tot_len, (int)udph_len);
			OTHER_RECEIVED(conn, pack_iph);
			return 0;
		}
	}
	...
	if ((memcmp(pack_ethh->dst, dhcp_nexthop(this), PKT_ETH_ALEN)) &&
		...
		if (pack_ethh->dst[0] == 0x01 &&
		    pack_ethh->dst[1] == 0x00 && pack_ethh->dst[2] == 0x5e) {
			log_dbg("Multicast: %.2X-%.2X-%.2X-%.2X-%.2X-%.2X",
				pack_ethh->dst[0], pack_ethh->dst[1],
				pack_ethh->dst[2], pack_ethh->dst[3],
				pack_ethh->dst[4], pack_ethh->dst[5]);
		} else {
#endif
#if(_debug_)
			log_dbg
			    ("Not for our MAC or broadcast: %.2X-%.2X-%.2X-%.2X-%.2X-%.2X",
			     pack_ethh->dst[0], pack_ethh->dst[1],
			     pack_ethh->dst[2], pack_ethh->dst[3],
			     pack_ethh->dst[4], pack_ethh->dst[5]);
#endif
	...
	if (is_dhcp && _options.layer3) {
      ...
      /** Relay the DHCP request **/
		...
		(void)dhcp_handler(CHILLI_DHCP_PROXY, 0, pack, len, 0, 0);
	...
	case PKT_IP_PROTO_UDP:
		...
		if ((pack_iph->daddr & ~_options.mask.s_addr) ==
			...
			if (pack_udph->dst == htons(137)) {
				log_dbg("NetBIOS NS to port %d",
					ntohs(pack_udph->dst));
				break;
			} else if (pack_udph->dst == htons(138)) {
				log_dbg("NetBIOS DGM to port %d",
					ntohs(pack_udph->dst));
				break;
			}
#endif
#if(_debug_)
			log_dbg("Broadcasted UDP to port %d",
				ntohs(pack_udph->dst));
#endif
			OTHER_RECEIVED(conn, pack_iph);
			return 0;
	...
	case PKT_IP_PROTO_TCP:
		...
		if ((pack_iph->daddr == _options.uamlogout.s_addr) &&
			...
			if (!appconn)
				appconn =
				    dhcp_get_appconn_pkt(conn, pack_iph, 0);
			if (appconn) {
				if (appconn->s_state.authenticated) {
					terminate_appconn(appconn,
							  RADIUS_TERMINATE_CAUSE_USER_REQUEST);
					log_dbg
					    ("Dropping session due to request for auto-logout ip");
					appconn->uamexit = 1;
				}
			}
		}
	...
	if (_options.uamalias.s_addr &&
		...
		do_checksum = 1;
		dhcp_uam_nat(conn, pack_ethh, pack_iph, pack_tcph,
			     &this->uamlisten,
#ifdef ENABLE_UAMUIPORT
...
int dhcp_receive_ipv6(struct dhcp_ctx *ctx, uint8_t * pack, size_t len)
{
	...
	log_dbg("Processing IPv6 ver=%d class=%d len=%d datalen=%d",
		...
		(int)ipv6_class(iphdr), (int)len, ip_datalen);
	...
	case ICMPv6_NEXT_HEADER:
		{
			...
			struct pkt_icmphdr_t *icmphdr = (struct pkt_icmphdr_t *)
			...
			case 133:	/* 133 Router Solicitation (NDP) */
				 ...
				 */
				{
					...
					*payload++ = 1;
					*payload++ = 1;
					memcpy(payload, dhcp_nexthop(dhcp),
					       PKT_ETH_ALEN);
					...
					*payload++ = 0;
					...
					packet_ip6h->ver_class_label =
					    iphdr->ver_class_label;
					packet_ip6h->next_header =
					    iphdr->next_header;
					packet_ip6h->hop_limit = 255;
					packet_ip6h->data_len = htons(data_len);

					memcpy(packet_ip6h->src_addr,
					       &this->rawif[0].address_v6.
					       s6_addr, PKT_IPv6_ALEN);
					memcpy(packet_ip6h->dst_addr,
					       iphdr->src_addr, PKT_IPv6_ALEN);

					memcpy(packet_ethh->dst, ethh->src,
					       PKT_ETH_ALEN);
					memcpy(packet_ethh->src,
					       dhcp_nexthop(dhcp),
					       PKT_ETH_ALEN);

					chksum6(packet_ip6h);

					return dhcp_send(this, ctx->idx,
							 ethh->src, packet,
							 sizeofeth(packet) +
							 sizeof(struct
								pkt_ip6hdr_t) +
	...
	case PKT_IP_PROTO_UDP:
		{
			...
			if (ntohs(udphdr->src) == 546
			    && ntohs(udphdr->dst) == 547) {
				...
				int optlen =
				    ip_datalen - sizeof(struct pkt_udphdr_t)
				    - sizeof(struct pkt_dhcp6hdr_t);
...

						{
							...
							*payload++ =
							...
							packet_ip6h->
							    next_header =
							    iphdr->next_header;
							packet_ip6h->hop_limit =
							    255;
							packet_ip6h->data_len =
							    htons(data_len);
	...
	case PKT_IP_PROTO_TCP:
	case PKT_IP_PROTO_UDP:
		if (iphdr->dst_addr[0] == 0x11 && iphdr->dst_addr[1] == 0x12) {
			...
			ip4hdr =
			    (struct pkt_iphdr_t *)(pack + 20 + sizeofeth(pack));
...
int dhcp_pppoes(struct dhcp_ctx *ctx, uint8_t * packet, size_t length)
{
	...
	if (hdr->version_type == PKT_PPPoE_VERSION && hdr->code == 0x0000) {
		...
		if (len > 0) {
			...
			case PKT_PPP_PROTO_LCP:
				{
					struct pkt_ppp_lcp_t *lcp =
					    (struct pkt_ppp_lcp_t *)(p + 2);
...
int dhcp_pppoed(struct dhcp_ctx *ctx, uint8_t * packet, size_t length)
{
	uint8_t *p = packet + sizeofeth(packet);
	struct pkt_pppoe_hdr_t *hdr = (struct pkt_pppoe_hdr_t *)p;
	if (hdr->version_type == PKT_PPPoE_VERSION && hdr->session_id == 0x0000) {
		int len = ntohs(hdr->length);
		uint16_t t, l;
		...
		while (len > 0) {
			...
			case PPPoE_TAG_ServiceName:
				log_dbg("PPPoE Service-Name: %.*s",
					l,
					p + sizeof(struct pkt_pppoe_taghdr_t));
				break;
		...
		case PKT_PPPoE_PADI:
			{
				...
				taglen = 5;
				tag =
				    (struct pkt_pppoe_taghdr_t *)(answer +
								  length);
				...
				if (host_uniq_len) {
					...
					tag->length = htons(taglen);
					length +=
					    sizeof(struct pkt_pppoe_taghdr_t);
					memcpy(answer + length, host_uniq,
					       host_uniq_len);
					length += taglen;
				}
				...
				memcpy(&answer_ethh->dst, &ethh->src,
				       PKT_ETH_ALEN);
				memcpy(&answer_ethh->src, dhcp_nexthop(dhcp),
				       PKT_ETH_ALEN);
...
int dhcp_chillipkt(struct dhcp_ctx *ctx, uint8_t * packet, size_t length)
{
	...
	log_dbg("CHILLI: peer %d decrypting %d bytes", _options.peerid, n);
			...
			if (olen > sizeof(struct pkt_chillihdr_t)) {
				...
				case CHILLI_PEER_INIT:
					...
					if (chilli_hdr->from == _options.peerid) {
						log_err(0,
							"peer %d possible conflicting peerid, oops",
							_options.peerid);
					} else {
						dhcp_sendCHILLI
						    (CHILLI_PEER_HELLO, 0, 0);
				...
				case CHILLI_PEER_HELLO:
					...
					if (chilli_hdr->from == _options.peerid) {
						log_err(0,
							"peer %d possible conflicting peerid",
							_options.peerid);
					} else {
						update_peer(chilli_hdr);
					}
...
int dhcp_decaps(struct dhcp_t *this, int idx)
{
	...
	if (idx) {
		...
		if (iface) {
			length =
			    recv(iface->fd, buf, sizeof(buf), MSG_DONTWAIT);
			if (length > 0) {
				nfq_handle_packet(iface->h, (char *)buf,
						  length);
			}
			return length;
		}
...
int dhcp_relay_decaps(struct dhcp_t *this, int idx)
{
	...
	if (addr.sin_addr.s_addr != _options.dhcpgwip.s_addr &&
		...
		log_dbg
		    ("Received DHCP response from host (%s) other than our gateway",
	...
	if (addr.sin_port != htons(_options.dhcpgwport)) {
		log_dbg
		    ("Received DHCP response from port (%d) other than our gateway",
		     ntohs(addr.sin_port));
		return -1;
	}
...

	{
		...
		fullpack_iph->tot_len =
		    htons(length + PKT_UDP_HLEN + PKT_IP_HLEN);
		...
		} else if (packet.flags[0] & 0x80
			   || message_type->v[0] == DHCPNAK) {
			log_dbg("DHCP: Nak or Broadcast");
			packet.flags[0] = 0x80;
			dhcp_ethhdr(conn, fullpack, bmac, dhcp_nexthop(this),
				    PKT_ETH_PROTO_IP);
...
int dhcp_data_req(struct dhcp_conn_t *conn, struct pkt_buffer *pb, int ethhdr)
{
	...
	case DHCP_AUTH_NONE:
		...
		if (dhcp_undoDNAT(conn, packet, &length, 1, &do_checksum)
		    && !allowed) {
#if(_debug_ > 1)
...
int dhcp_receive_arp(struct dhcp_ctx *ctx, uint8_t * pack, size_t len)
{
	...
	if (_options.layer3) {
		if (taraddr.s_addr == _options.dhcplisten.s_addr) {
			if (dhcp_hashget(this, &conn, pack_arp->sha)) {
				log_dbg("ARP: Address not found: %s",
					inet_ntoa(reqaddr));
			...
			if (_options.ieee8021q) {
#if(_debug_ > 1)
				log_dbg("%s calling dhcp_checktag",
					__FUNCTION__);
#endif
				dhcp_checktag(conn, pack);
			}
	...
	if (dhcp_hashget(this, &conn, pack_arp->sha)) {
		...
		if (!this->allowdyn
			...
			log_dbg("ARP: Unknown client and no dynip: %s",
				inet_ntoa(taraddr));
			return 0;
		}
	...
	if (!_options.uamanyip) {
		...
		if (memcmp(&conn->ourip.s_addr, &taraddr.s_addr, 4) &&
			...
			if (_options.debug) {
				log_dbg
				    ("ARP: Did not ask for router address: %s",
				     inet_ntoa(conn->ourip));
				log_dbg("ARP: Asked for target: %s",
					inet_ntoa(taraddr));
			}
...
int dhcp_receive_eapol(struct dhcp_ctx *ctx, uint8_t * pack)
{
	...
	log_dbg("IEEE 802.1x Packet: %.2x, %.2x %d",
		pack_dot1x->ver, pack_dot1x->type, ntohs(pack_dot1x->len));
...
int dhcp_eapol_ind(struct dhcp_t *this)
{
	...
	if ((length =
	     net_read_eth(&this->rawif[0], packet, sizeof(packet))) < 0)
		return -1;
	...
	if (_options.debug) {
		struct pkt_ethhdr_t *ethh = pkt_ethhdr(packet);
		log_dbg
		    ("eapol_decaps: src=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x dst=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x prot=%.4x",
		     ethh->src[0], ethh->src[1], ethh->src[2], ethh->src[3],
		     ethh->src[4], ethh->src[5], ethh->dst[0], ethh->dst[1],
		     ethh->dst[2], ethh->dst[3], ethh->dst[4], ethh->dst[5],
		     ntohs(ethh->prot));
	}
#endif
