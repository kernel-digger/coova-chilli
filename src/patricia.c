...
prefix_t *patricia_prefix_new2(int family, void *dest, int bitlen,
			       prefix_t * prefix)
{
...
prefix_t *patricia_prefix_ref(prefix_t * prefix)
{
	...
	if (prefix->ref_count == 0) {
		...
		return (patricia_prefix_new2
			(prefix->family, &prefix->add, prefix->bitlen, NULL));
	}
	prefix->ref_count++;
	...
	while (node->bit < bitlen || node->prefix == NULL) {
		...
		if (node->bit < patricia->maxbits &&
			...
			if (node->prefix) {
				...
				log_dbg("patricia_lookup: take right at %u",
					node->bit);
			}
#endif /* PATRICIA_DEBUG */
			node = node->r;
			...
			if (node->prefix) {
				...
				log_dbg("patricia_lookup: take left at %u",
					node->bit);
			}
#endif /* PATRICIA_DEBUG */
			node = node->l;
