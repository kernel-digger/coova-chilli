...
static int usage(char *program)
{
	...
	fprintf(stderr,
		"Usage: %s [ -s <socket> ] [ -P <port> ] <command> [<arguments>]\n",
		program);
	fprintf(stderr,
	...
	for (i = 0; commands[i].command; i++) {
		fprintf(stderr, "%s%s", i > 0 ? ", " : "", commands[i].command);
	}
		...
		for (i = 0; i < count; i++) {
			...
			if (!strcmp(argv[argidx], args[i].name)) {
				...
				if (args[i].flag) {
					*(args[i].flag) |= args[i].flagbit;
					...
					if (is_data == 0)
						...
						fprintf(stderr,
							"data can only be once and by itself\n");
						return argidx;
					}
				} else {
					if (i > 1) {
				...
				case CMDSOCK_FIELD_MAC:
					parse_mac(((uint8_t *) args[i].field),
						  argv[argidx + 1]);
					break;
				case CMDSOCK_FIELD_STRING:
					safe_strncpy(((char *)args[i].field),
						     argv[argidx + 1],
						     args[i].length);
					break;
				case CMDSOCK_FIELD_INTEGER:
					...
					case 1:
						*((uint8_t *) args[i].field) |=
						    (uint8_t)
						    atoi(argv[argidx + 1]);
						break;
					case 2:
						*((uint16_t *) args[i].field) |=
						    (uint16_t)
						    atoi(argv[argidx + 1]);
						break;
					case 4:
						*((uint32_t *) args[i].field) |=
						    (uint32_t)
						    atol(argv[argidx + 1]);
						break;
					case 8:
						*((uint64_t *) args[i].field) |=
						    (uint64_t)
						    atol(argv[argidx + 1]);
						break;
					}
				...
				case CMDSOCK_FIELD_IPV4:
					{
						...
						((struct in_addr *)args[i].
						 field)->s_addr = ip.s_addr;
						break;
					}
				}
				break;
...
static int chilli_communicate(int s,
			      struct cmdsock_request *buffer, size_t blen)
{
	char line[1024];
	int len;
			...
			case CMDSOCK_DHCP_RELEASE:
				{
					if (argc < argidx + 1) {
						fprintf(stderr,
							"%s requires a MAC address argument\n",
							cmd);
						return usage(argv[0]);
					}

					if (parse_mac
					    (request.mac, argv[argidx]))
						return usage(argv[0]);
	...
	for (i = 0; i < globbuf.gl_pathc; i++) {
		...
		len =
		    offsetof(struct sockaddr_un,
			     sun_path) + strlen(remote.sun_path);
