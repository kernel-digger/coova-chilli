...
struct redir_t {
	...
	*/
	...
	int (*cb_handle_url) (struct redir_t * redir,
			      ...
			      struct sockaddr_in * peer, redir_request * rreq);
};
...
int redir_new(struct redir_t **redir, struct in_addr *addr, int port,
	      int uiport);
...
int redir_setchallenge(struct redir_t *redir, struct in_addr *addr,
		       uint8_t * challenge);
