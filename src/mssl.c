...
int SSL_pending(SSL * ssl)
{
	unsigned long available =
	    ...
	    ((unsigned long)(ssl->insock.end) -
	     (unsigned long)(ssl->insock.start)) : 0;
		...
		void *certValidatorArgs)
{
	...
	if (certValidator != NULL) {
		matrixSslSetCertValidator(ssl->ssl, certValidator,
					  certValidatorArgs);
	} else {
		matrixSslSetCertValidator(ssl->ssl, _always_true_validator,
					  certValidatorArgs);
	}
...
int _ssl_doHandshake(SSL * ssl)
{
	...
	short cipherSuite = 0;
	err =
	    matrixSslEncodeClientHello(ssl->ssl, &(ssl->outsock), cipherSuite);
...
int _psSocketWrite(int sock, sslBuf_t * out)
{
	...
	while (out->start < out->end) {
		bytes =
		    send(sock, out->start, (int)(out->end - out->start),
			 MSG_NOSIGNAL);
...
static int _ssl_read(SSL * ssl, char *buf, int len)
{
	...
	if (ssl->insock.buf < ssl->insock.start) {
		if (ssl->insock.start == ssl->insock.end) {
			...
			memmove(ssl->insock.buf, ssl->insock.start,
...
readMore:
	if (ssl->insock.end == ssl->insock.start || performRead) {
		...
		bytes = recv(ssl->fd, (char *)ssl->insock.end,
			     (int)((ssl->insock.buf + ssl->insock.size) -
				   ssl->insock.end), MSG_NOSIGNAL);
		if (bytes == -1) {
			ssl->status = errno;
...
decodeMore:
	...
	rc = matrixSslDecode(ssl->ssl, &ssl->insock, &ssl->inbuf, &error,
	...
	case SSL_SEND_RESPONSE:
		...
		bytes = send(ssl->fd, (char *)ssl->inbuf.start,
			     (int)(ssl->inbuf.end - ssl->inbuf.start),
			     MSG_NOSIGNAL);
		if (bytes == -1) {
			ssl->status = errno;
	...
	case SSL_PARTIAL:
		...
		if (ssl->insock.start == ssl->insock.buf && ssl->insock.end ==
			...
			ssl->insock.size *= 2;
			ssl->insock.start = ssl->insock.buf =
			    (unsigned char *)realloc(ssl->insock.buf,
						     ssl->insock.size);
			ssl->insock.end =
			    ssl->insock.buf + (ssl->insock.size / 2);
		}
		if (!performRead) {
			performRead = 1;
			free(ssl->inbuf.buf);
...
int _ssl_write(SSL * ssl, char *buf, int len)
{
	...
	if (ssl->outsock.buf < ssl->outsock.start) {
		if (ssl->outsock.start == ssl->outsock.end) {
			ssl->outsock.start = ssl->outsock.end =
			    ssl->outsock.buf;
		} else {
			memmove(ssl->outsock.buf, ssl->outsock.start,
				ssl->outsock.end - ssl->outsock.start);
			ssl->outsock.end -=
...
retryEncode:
		rc = matrixSslEncode(ssl->ssl, (unsigned char *)buf, len,
				     &ssl->outsock);
		switch (rc) {
		case SSL_ERROR:
			return -1;
		case SSL_FULL:
			...
			ssl->outsock.size *= 2;
			ssl->outsock.buf =
			    (unsigned char *)realloc(ssl->outsock.buf,
						     ssl->outsock.size);
			ssl->outsock.end =
			    ssl->outsock.buf + (ssl->outsock.end -
						ssl->outsock.start);
			ssl->outsock.start = ssl->outsock.buf;
			goto retryEncode;
		}
