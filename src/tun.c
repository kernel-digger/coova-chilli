...
int tun_name2idx(struct tun_t *tun, char *name)
{
	...
	/* Not found? Check for discovery */
	{
		...
		if (rti) {
			...
			safe_strncpy(netif.devname, rti->devname,
				     sizeof(netif.devname));
...
int tun_discover(struct tun_t *this)
{
	...
	for (i = 0; i < len; ++i) {
		...
		safe_strncpy(netif.devname, ifr->ifr_name,
			     sizeof(netif.devname));
		...
		if (-1 < ioctl(fd, SIOCGIFNETMASK, (caddr_t) ifr)) {
			...
			log_err(errno, "ioctl(SIOCGIFNETMASK)");
		...
		if (-1 < ioctl(fd, SIOCGIFHWADDR, (caddr_t) ifr)) {
			...
			log_err(errno, "ioctl(SIOCGIFHWADDR)");
#else
#ifdef SIOCGENADDR
		if (-1 < ioctl(fd, SIOCGENADDR, (caddr_t) ifr)) {
			...
			log_err(errno, "ioctl(SIOCGENADDR)");
#else
#warning Do not know how to find interface hardware address
		...
		if (netif.devflags & IFF_POINTOPOINT) {
			if (-1 < ioctl(fd, SIOCGIFDSTADDR, (caddr_t) ifr)) {
				...
				log_dbg("\tPoint-to-Point:\t%s",
					inet_ntoa(inaddr(ifr_dstaddr)));

			} else
				log_err(errno, "ioctl(SIOCGIFDSTADDR)");
		}
		...
		if (netif.devflags & IFF_BROADCAST) {
			if (-1 < ioctl(fd, SIOCGIFBRDADDR, (caddr_t) ifr)) {
				...
				log_dbg("\tBroadcast:\t%s",
					inet_ntoa(inaddr(ifr_addr)));

			} else
				log_err(errno, "ioctl(SIOCGIFBRDADDR)");
		}
		...
		if (-1 < ioctl(fd, SIOCGIFMTU, (caddr_t) ifr)) {
			...
			log_err(errno, "ioctl(SIOCGIFMTU)");
...

		{
			...
			if (newif) {
				...
				log_dbg("no room for interface %s",
					netif.devname);
			}
		}
	}
		...
		struct in_addr *dstaddr, struct in_addr *netmask)
{
	...
	if (getsockname(fd, (struct sockaddr *)&local, (socklen_t *) & addr_len)
	    < 0) {
...
#error  "Unknown platform!"
...
#if defined(IFF_ONE_QUEUE) && defined(SIOCSIFTXQLEN)
	{
		...
		if ((nfd = socket(AF_INET, SOCK_DGRAM, 0)) >= 0) {
			...
			if (ioctl(nfd, SIOCSIFTXQLEN, (void *)&nifr) >= 0)
				log_info("TX queue length set to %d",
					 _options.txqlen);
			else
				log_err(errno,
					"Cannot set tx queue length on %s",
					ifr.ifr_name);

			close(nfd);
		} else {
			log_err(errno, "Cannot open socket on %s",
				ifr.ifr_name);
		}
	}
#endif
	...
	if (_options.usetap) {
		...
		if ((fd = socket(AF_INET, SOCK_DGRAM, 0)) >= 0) {
			...
			memcpy(netif->hwaddr, ifr.ifr_hwaddr.sa_data,
			       PKT_ETH_ALEN);
			log_dbg("tap-mac: %s %.2X-%.2X-%.2X-%.2X-%.2X-%.2X",
				ifr.ifr_name, netif->hwaddr[0],
				netif->hwaddr[1], netif->hwaddr[2],
				netif->hwaddr[3], netif->hwaddr[4],
				netif->hwaddr[5]);
			close(fd);
		}
	}
	...
	safe_snprintf(netif->devname, sizeof(netif->devname), "tun%d", ppa);
...
#if(_debug_ > 1)
	log_dbg("tun_decaps(idx=%d, len=%d)", tun(c->this, c->idx).ifindex,
		length);
#endif
		...
		if ((int)ntohs(iph->tot_len) + ethsize > length) {
			log_dbg
			    ("dropping ip packet; ip-len=%d + eth-hdr=%d > read-len=%d",
			     (int)ntohs(iph->tot_len), ethsize, (int)length);
			return -1;
		}
	}
...
int tun_decaps(struct tun_t *this, int idx)
{
	...
	if (idx > 0)
		length =
		    net_read_dispatch_eth(&tun(this, idx), tun_decaps_cb, &c);
	else
...
again:
	...
	set_env("DHCPIF", VAL_STRING, _options.dhcpif ? _options.dhcpif : "",
		0);
	...
	if (execl(
			 ...
			 script,
#endif
			 script, tuntap(tun).devname, saddr, smask,
			 (char *)0) != 0) {
