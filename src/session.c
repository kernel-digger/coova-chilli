	...
	if (!_options.layer3) {
		...
		if (hismac) {
			...
			safe_snprintf(mac, sizeof(mac),
				      "%.2X-%.2X-%.2X-%.2X-%.2X-%.2X",
				      (unsigned int)hismac[0],
				      (unsigned int)hismac[1],
				      (unsigned int)hismac[2],
				      (unsigned int)hismac[3],
				      (unsigned int)hismac[4],
				      (unsigned int)hismac[5]);
			bcatcstr(json, mac);
		}
#ifdef ENABLE_LAYER3
	}
...
int session_json_params(struct session_state *state,
			struct session_params *params, bstring json, int init)
{
	...
	bassignformat(tmp, "%ld",
		      mainclock_towall(init ? mainclock_now() : starttime));
	bconcat(json, tmp);
	...
	if (_options.ieee8021q && state->tag8021q) {
		bcatcstr(json, ",\"vlan\":");
		bassignformat(tmp, "%d",
			      (int)ntohs(state->tag8021q & PKT_8021Q_MASK_VID));
		bconcat(json, tmp);
	}
#endif
