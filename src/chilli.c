...
int child_remove_pid(pid_t pid)
{
	...
	if (children) {
		...
		if (list->next) {
			...
			list->next = node->next;
#if(_debug_)
			log_dbg("Freed child process %d [%s]", node->pid,
				node->name);
#endif
			free(node);
			return 0;
...
static int proc_status(char *name, pid_t pid)
{
	...
	while ((read = getline(&line, &len, fp)) != -1) {
		if (!memcmp(line, name, strlen(name))) {
			int i;
			if (sscanf(line + strlen(name) + 1, "%d %s", &i, buffer)
			    == 2) {
				ret = i;
				if (buffer[0] == 'm')
					ret *= 1000;
...
void child_print(bstring s)
{
	...
	while (node) {
		...
		case CHILLI_PROC:
			n = "Main";
			break;
		case CHILLI_PROC_DAEMON:
			n = "Daemon";
			break;
		case CHILLI_PROC_REDIR:
			n = "Redirect";
			break;
		case CHILLI_PROC_SCRIPT:
			n = "Script";
			break;
		}
...
static void _sigchld(int signum)
{
	...
	while ((pid = waitpid(-1, &stat, WNOHANG)) > 0) {
		...
		if (!_options.debug && radsec_pid > 0 && radsec_pid == pid) {
			log_err(0,
				"Having to re-launch chilli_radsec... PID %d exited",
				pid);
			launch_chilliradsec();
		}
#endif
		...
		if (!_options.debug && proxy_pid > 0 && proxy_pid == pid) {
			log_err(0,
				"Having to re-launch chilli_proxy... PID %d exited",
				pid);
			launch_chilliproxy();
		}
#endif
		...
		if (!_options.debug && redir_pid > 0 && redir_pid == pid) {
			log_err(0,
				"Having to re-launch chilli_redir... PID %d exited",
				pid);
			launch_chilliredir();
		}
#endif
...
static void set_sessionid(struct app_conn_t *appconn, char full)
{
	...
	if (full) {
		...
		safe_snprintf(appconn->s_state.chilli_sessionid,
			      ...
			      called[4], called[5], appconn->rt, appconn->unit);
	}
#endif
}
...
static inline int
leaky_bucket(struct app_conn_t *conn, uint64_t octetsup, uint64_t octetsdown)
{
	int result = 0;
	uint64_t timediff;
	...
	if (conn->s_params.bandwidthmaxdown) {
		uint64_t bytes =
		    (timediff * conn->s_params.bandwidthmaxdown) / 8;
		...
		if ((conn->s_state.bucketdown + octetsdown) >
			...
			if (_options.debug)
				log_dbg
				    ("Leaky bucket deleting downlink packet");
			result = -1;
		} else {
...
void set_env(char *name, char type, void *value, int len)
{
	...
	case VAL_ULONG:
		safe_snprintf(s, sizeof(s), "%ld",
			      (long int)*(uint32_t *) value);
		v = s;
		break;

	case VAL_ULONG64:
		safe_snprintf(s, sizeof(s), "%ld",
			      (long int)*(uint64_t *) value);
		v = s;
		break;
	...
	if (name != NULL && v != NULL) {
		if (setenv(name, v, 1) != 0) {
			log_err(errno, "setenv(%s, %s, 1) did not return 0!",
				name, v);
		}
	}
}
...
int runscript(struct app_conn_t *appconn, char *script, char *loc, char *oloc)
{
	...
	set_env("STATE", VAL_STRING, appconn->s_state.redir.statebuf,
		appconn->s_state.redir.statelen);
	set_env("CLASS", VAL_STRING, appconn->s_state.redir.classbuf,
		appconn->s_state.redir.classlen);
	set_env("CUI", VAL_STRING, appconn->s_state.redir.cuibuf,
		appconn->s_state.redir.cuilen);
	set_env("SESSION_TIMEOUT", VAL_ULONG64,
	...
	set_env("CALLED_STATION_ID", VAL_MAC_ADDR,
		chilli_called_station(&appconn->s_state), 0);
	...
	set_env("ACCT_INTERIM_INTERVAL", VAL_USHORT,
		&appconn->s_params.interim_interval, 0);
	set_env("WISPR_LOCATION_ID", VAL_STRING, _options.radiuslocationid, 0);
	set_env("WISPR_LOCATION_NAME", VAL_STRING, _options.radiuslocationname,
		0);
	set_env("WISPR_BANDWIDTH_MAX_UP", VAL_ULONG,
		&appconn->s_params.bandwidthmaxup, 0);
	set_env("WISPR_BANDWIDTH_MAX_DOWN", VAL_ULONG,
		&appconn->s_params.bandwidthmaxdown, 0);
	/*set_env("WISPR-SESSION_TERMINATE_TIME", VAL_USHORT, &appconn->sessionterminatetime, 0); */
	set_env("CHILLISPOT_MAX_INPUT_OCTETS", VAL_ULONG64,
		&appconn->s_params.maxinputoctets, 0);
	set_env("CHILLISPOT_MAX_OUTPUT_OCTETS", VAL_ULONG64,
		&appconn->s_params.maxoutputoctets, 0);
	set_env("CHILLISPOT_MAX_TOTAL_OCTETS", VAL_ULONG64,
		&appconn->s_params.maxtotaloctets, 0);
	set_env("INPUT_OCTETS", VAL_ULONG64, &appconn->s_state.input_octets, 0);
	set_env("OUTPUT_OCTETS", VAL_ULONG64, &appconn->s_state.output_octets,
		0);
...
int static freeconn(struct app_conn_t *conn)
{
	...
	if (conn->loc_search_node != NULL)
		location_close_conn(conn, 1);
#endif
...
#endif
	if (appconn->s_params.url[0] && appconn->s_params.flags & UAM_CLEAR_URL) {
...
#endif
			if (appconn->dnlink)
				((struct dhcp_conn_t *)appconn->dnlink)->
	...
	if (conn->s_state.authenticated == 1) {
		if ((conn->s_params.sessiontimeout) &&
			...
			terminate_appconn(conn,
			   ...
			   &&
			   (mainclock_rtdiff
			    (conn->s_params.sessionterminatetime) > 0)) {
#ifdef ENABLE_SESSIONSTATE
			conn->s_state.session_state =
			...
			terminate_appconn(conn,
			   ...
			   && (interimtime >=
			       conn->s_params.interim_interval)) {

#ifdef ENABLE_MODULES
			{
				int i;
				for (i = 0; i < MAX_MODULES; i++) {
					if (!_options.modules[i].name[0])
						break;
					if (_options.modules[i].ctx) {
						struct chilli_module *m =
						    (struct chilli_module *)
						    _options.modules[i].ctx;
						if (m->session_update)
...
#endif
				   !(dhcpconn =
				     (struct dhcp_conn_t *)conn->dnlink)) {
...
int static killconn()
{
	...
	for (conn = firstusedconn; conn; conn = conn->next) {
		if ((conn->inuse != 0) && (conn->s_state.authenticated == 1)) {
			terminate_appconn(conn,
					  RADIUS_TERMINATE_CAUSE_NAS_REBOOT);
		}
	}

	if (admin_session.s_state.authenticated) {
		admin_session.s_state.terminate_cause =
		    RADIUS_TERMINATE_CAUSE_NAS_REBOOT;
		     ...
		     struct in_addr *hisip, struct session_state *state)
{
	...
	if (_options.ieee8021q && state->tag8021q) {
		radius_addattr(radius, pack, RADIUS_ATTR_VENDOR_SPECIFIC,
			       RADIUS_VENDOR_CHILLISPOT,
			       RADIUS_ATTR_CHILLISPOT_VLAN_ID,
			       (uint32_t) ntohs(state->
						tag8021q & PKT_8021Q_MASK_VID),
			       0, 0);
	}
#endif
	...
	if (*sessionid) {
		radius_addattr(radius, pack, RADIUS_ATTR_ACCT_SESSION_ID, 0, 0,
			       0, (uint8_t *) sessionid, strlen(sessionid));
	}
#ifdef ENABLE_SESSIONID
	...
	case ACCT_USER:
		if (state->redir.classlen) {
			log_dbg("RADIUS Request + Class(%d)",
		...
		if (state->redir.cuilen > 1) {
			log_dbg("RADIUS Request + CUI(%d)",
				state->redir.cuilen);
			radius_addattr(radius, pack,
				       RADIUS_ATTR_CHARGEABLE_USER_IDENTITY, 0,
				       0, 0, state->redir.cuibuf,
				       state->redir.cuilen);
		}

		if (state->redir.statelen) {
			log_dbg("RADIUS Request + State(%d)",
	...
	if (hisip && hisip->s_addr) {
		radius_addattr(radius, pack, RADIUS_ATTR_FRAMED_IP_ADDRESS, 0,
			       0, ntohl(hisip->s_addr), NULL, 0);
	}
	...
	if (hismac) {
		safe_snprintf(mac, sizeof(mac), "%.2X-%.2X-%.2X-%.2X-%.2X-%.2X",
			      hismac[0], hismac[1],
			      hismac[2], hismac[3], hismac[4], hismac[5]);

		radius_addattr(radius, pack, RADIUS_ATTR_CALLING_STATION_ID, 0,
			       0, 0, (uint8_t *) mac, MACSTRLEN);
	}
	...
	if (state->location[0]) {
		...
		if (state->location_changes) {
			radius_addattr(radius, pack,
				       ...
				       (uint32_t) state->location_changes, 0,
				       0);
		}
	}
#endif
	...
	if (_options.radiusnasid) {
		radius_addattr(radius, pack, RADIUS_ATTR_NAS_IDENTIFIER, 0, 0,
			       0, (uint8_t *) _options.radiusnasid,
	...
	if (_options.radiuslocationid) {
		radius_addattr(radius, pack, RADIUS_ATTR_VENDOR_SPECIFIC,
			       RADIUS_VENDOR_WISPR,
			       RADIUS_ATTR_WISPR_LOCATION_ID, 0,
	...
	if (_options.radiuslocationname) {
		radius_addattr(radius, pack, RADIUS_ATTR_VENDOR_SPECIFIC,
			       RADIUS_VENDOR_WISPR,
			       RADIUS_ATTR_WISPR_LOCATION_NAME, 0,
...
#ifdef ENABLE_MODULES
	{
		...
		for (i = 0; i < MAX_MODULES; i++) {
			...
			if (_options.modules[i].ctx) {
				struct chilli_module *m =
				    (struct chilli_module *)_options.modules[i].
				    ctx;
				if (m->radius_handler) {
					int res =
					    m->radius_handler(radius, 0, pack,
							      0);
					switch (res) {
...
int chilli_auth_radius(struct radius_t *radius)
{
	...
	if (radius_default_pack
	    (radius, &radius_pack, RADIUS_CODE_ACCESS_REQUEST)) {
		log_err(0, "radius_default_pack() failed");
		return ret;
	}

	radius_addattr(radius, &radius_pack, RADIUS_ATTR_USER_NAME, 0, 0, 0,
		       (uint8_t *) _options.adminuser,
		       strlen(_options.adminuser));

	if (_options.adminpasswd)
		radius_addattr(radius, &radius_pack, RADIUS_ATTR_USER_PASSWORD,
			       0, 0, 0, (uint8_t *) _options.adminpasswd,
			       strlen(_options.adminpasswd));

	chilli_req_attrs(radius, &radius_pack,
			 ACCT_USER,
			 RADIUS_SERVICE_TYPE_ADMIN_USER, 0,
			 0, 0, 0, &admin_session.s_state);
		       ...
		       uint8_t * dhcp_pkt, size_t dhcp_len)
{
	...
	if (!username) {
		...
		safe_strncpy(appconn->s_state.redir.username, mac,
			     USERNAMESIZE);
		...
		safe_strncpy(appconn->s_state.redir.username, username,
			     USERNAMESIZE);
...
int static radius_access_reject(struct app_conn_t *conn)
{
	...
	if (radius_default_pack
	    (radius, &radius_pack, RADIUS_CODE_ACCESS_REJECT)) {
		log_err(0, "radius_default_pack() failed");
		return -1;
	}

	radius_pack.id = conn->radiusid;
	radius_resp(radius, &radius_pack, &conn->radiuspeer,
		    conn->authenticator);
	return 0;
}
...
int static radius_access_challenge(struct app_conn_t *conn)
{
	...
	if (radius_default_pack
	    (radius, &radius_pack, RADIUS_CODE_ACCESS_CHALLENGE)) {
		log_err(0, "radius_default_pack() failed");
		return -1;
	}
	...
	do {
		...
		if (radius_addattr
		    (radius, &radius_pack, RADIUS_ATTR_EAP_MESSAGE, 0, 0, 0,
...
int static radius_access_accept(struct app_conn_t *conn)
{
	...
	if (radius_default_pack
	    (radius, &radius_pack, RADIUS_CODE_ACCESS_ACCEPT)) {
		log_err(0, "radius_default_pack() failed");
		return -1;
	}
	...
	while (offset < conn->challen) {
		...
		radius_addattr(radius, &radius_pack, RADIUS_ATTR_EAP_MESSAGE, 0,
			       0, 0, conn->chal + offset, eaplen);
	...
	if (conn->sendlen) {
		...
		radius_addattr(radius, &radius_pack,
			       RADIUS_ATTR_VENDOR_SPECIFIC, RADIUS_VENDOR_MS,
		    ...
		    struct app_conn_t *conn, uint8_t status_type)
{
		...
		if (conn->is_adminsession) {
...
#if defined(HAVE_SYS_SYSINFO_H) && defined(HAVE_SYSINFO)
			{
				...
				if (sysinfo(&the_info)) {
					...
					fav[0] =
					    ((float)the_info.loads[0]) /
					    shiftfloat;
					fav[1] =
					    ((float)the_info.loads[1]) /
					    shiftfloat;
					fav[2] =
					    ((float)the_info.loads[2]) /
					    shiftfloat;

					radius_addattr(radius, &radius_pack,
						       RADIUS_ATTR_VENDOR_SPECIFIC,
						       RADIUS_VENDOR_CHILLISPOT,
						       RADIUS_ATTR_CHILLISPOT_SYS_UPTIME,
						       (uint32_t) the_info.
						       uptime, NULL, 0);

					safe_snprintf(b, sizeof(b), "%f %f %f",
						      fav[0], fav[1], fav[2]);

					radius_addattr(radius, &radius_pack,
						       RADIUS_ATTR_VENDOR_SPECIFIC,
						       RADIUS_VENDOR_CHILLISPOT,
						       RADIUS_ATTR_CHILLISPOT_SYS_LOADAVG,
						       0, (uint8_t *) b,
						       strlen(b));

					safe_snprintf(b, sizeof(b),
						      "%ld %ld %ld %ld",
						      the_info.totalram,
						      the_info.freeram,
						      the_info.sharedram,
	...
	if (status_type == RADIUS_STATUS_TYPE_STOP ||
		...
		case ACCT_GARDEN:
			...
			timediff =
			    mainclock_diffu(conn->s_state.garden_start_time);

			radius_addattr(radius, &radius_pack,
				       RADIUS_ATTR_ACCT_SESSION_TIME, 0, 0,
				       timediff, NULL, 0);
			break;
#endif
		case ACCT_USER:
			radius_addattr(radius, &radius_pack,
				       RADIUS_ATTR_ACCT_INPUT_OCTETS, 0, 0,
				       (uint32_t) conn->s_state.input_octets,
				       NULL, 0);
			radius_addattr(radius, &radius_pack,
				       RADIUS_ATTR_ACCT_OUTPUT_OCTETS, 0, 0,
				       (uint32_t) conn->s_state.output_octets,
				       NULL, 0);

			radius_addattr(radius, &radius_pack,
				       RADIUS_ATTR_ACCT_INPUT_GIGAWORDS, 0, 0,
				       (uint32_t) (conn->s_state.
						   input_octets >> 32), NULL,
				       0);
			radius_addattr(radius, &radius_pack,
				       RADIUS_ATTR_ACCT_OUTPUT_GIGAWORDS, 0, 0,
				       (uint32_t) (conn->s_state.
						   output_octets >> 32), NULL,
				       0);

			radius_addattr(radius, &radius_pack,
				       RADIUS_ATTR_ACCT_INPUT_PACKETS, 0, 0,
				       conn->s_state.input_packets, NULL, 0);
			radius_addattr(radius, &radius_pack,
				       RADIUS_ATTR_ACCT_OUTPUT_PACKETS, 0, 0,
		...
		if (incl_garden && _options.uamgardendata) {
			radius_addattr(radius, &radius_pack,
				       ...
				       (uint32_t) conn->s_state.
				       garden_input_octets, NULL, 0);
			radius_addattr(radius, &radius_pack,
				       RADIUS_ATTR_VENDOR_SPECIFIC,
				       RADIUS_VENDOR_CHILLISPOT,
				       RADIUS_ATTR_CHILLISPOT_GARDEN_OUTPUT_OCTETS,
				       (uint32_t) conn->s_state.
				       garden_output_octets, NULL, 0);
			...
			if (_options.uamotherdata) {
				radius_addattr(radius, &radius_pack,
					       ...
					       (uint32_t) conn->s_state.
					       other_input_octets, NULL, 0);
				radius_addattr(radius, &radius_pack,
					       RADIUS_ATTR_VENDOR_SPECIFIC,
					       RADIUS_VENDOR_CHILLISPOT,
					       RADIUS_ATTR_CHILLISPOT_OTHER_OUTPUT_OCTETS,
					       (uint32_t) conn->s_state.
					       other_output_octets, NULL, 0);
	...
	if (status_type == RADIUS_STATUS_TYPE_STOP ||
		...
		case ACCT_USER:
			radius_addattr(radius, &radius_pack,
				       RADIUS_ATTR_ACCT_TERMINATE_CAUSE, 0, 0,
				       conn->s_state.terminate_cause, NULL, 0);
			break;
		}
	}
#ifdef ENABLE_SESSIONSTATE
...
int chilli_assign_snat(struct app_conn_t *appconn, int force)
{
	...
	if (!_options.uamanyip)
		return 0;
	if (!_options.uamnatanyip)
		return 0;
	if (appconn->natip.s_addr && !force)
		return 0;
	...
	if (_options.uamnatanyipex_addr.s_addr &&
		...
		log_dbg
		    ("Excluding ip %s from SNAT becuase it is in uamnatanyipex",
		     inet_ntoa(appconn->hisip));
		return 0;
	}

	if ((appconn->hisip.s_addr & _options.mask.s_addr) ==
	    _options.net.s_addr)
		return 0;
	...
	if (udph && udph->src == htons(DHCP_BOOTPS)) {
		...
		if (pdhcp && pdhcp->op == DHCP_BOOTREPLY &&
			...
			if (!appconn) {
				...
				if (!appconn) {
					log_err(0, "could not allocate for %s",
						inet_ntoa(src));
					return 1;
				}
			}
			...
			if (_options.usetap) {
				struct pkt_ethhdr_t *ethh =
				    pkt_ethhdr(pkt_buffer_head(pb));

				log_dbg("forwarding layer3 dhcp-broadcast: %s",
					inet_ntoa(*dst));
...
int cb_tun_ind(struct tun_t *tun, struct pkt_buffer *pb, int idx)
{
		...
		case PKT_ETH_PROTO_ARP:
			{
				...
				size_t length =
				    PKT_ETH_HLEN + sizeof(struct arp_packet_t);
				 ...
				 */
				memcpy(&reqaddr.s_addr, p_arp->tpa,
				       PKT_IP_ALEN);

				if (_options.debug)
					log_dbg
					    ("arp: ifidx=%d src=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x "
					     "dst=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x "
					     "prot=%.4x (asking for %s)",
					     tun(tun, idx).ifindex,
					     ethh->src[0], ethh->src[1],
					     ethh->src[2], ethh->src[3],
					     ethh->src[4], ethh->src[5],
					     ethh->dst[0], ethh->dst[1],
					     ethh->dst[2], ethh->dst[3],
					     ethh->dst[4], ethh->dst[5],
					     ntohs(ethh->prot),
					     inet_ntoa(reqaddr));
				...
				if ((appconn =
				     (struct app_conn_t *)ipm->peer) == NULL
				    || (appconn->dnlink) == NULL) {
					log_err(0,
						"No peer protocol defined for ARP request");
					return 0;
				}
				...
				memcpy(packet_arp->sha, dhcp->rawif[0].hwaddr,
				       PKT_ETH_ALEN);
				...
				if (_options.uamanyip && appconn->natip.s_addr) {
					memcpy(packet_arp->spa,
					       &appconn->natip.s_addr,
					       PKT_IP_ALEN);
					if (_options.debug) {
						char ip[56];
						char snatip[56];
						...
						strcpy(snatip,
						       inet_ntoa(appconn->
								 natip));
						log_dbg
						    ("SNAT anyip in ARP response from %s to %s",
						     ip, snatip);
					}
				} else {
					memcpy(packet_arp->spa,
				...
				/* Target address */
				memcpy(packet_arp->tha, p_arp->sha,
				       PKT_ETH_ALEN);
				memcpy(packet_arp->tpa, p_arp->spa,
				       PKT_IP_ALEN);

				/* Ethernet header */
				memcpy(packet_ethh->dst, p_ethh->src,
				       PKT_ETH_ALEN);
				/*memcpy(packet_ethh->src, appconn->hismac, PKT_ETH_ALEN); */
				memcpy(packet_ethh->src, dhcp->rawif[0].hwaddr,
				       PKT_ETH_ALEN);
				...
				if (_options.debug) {
					log_dbg
					    ("arp-reply: src=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x "
					     "dst=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x",
					     packet_ethh->src[0],
					     packet_ethh->src[1],
					     packet_ethh->src[2],
					...
					memcpy(&reqaddr.s_addr, packet_arp->spa,
					       PKT_IP_ALEN);
					log_dbg
					    ("arp-reply: source sha=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x spa=%s",
					     packet_arp->sha[0],
					     packet_arp->sha[1],
					     packet_arp->sha[2],
					     packet_arp->sha[3],
					...
					memcpy(&reqaddr.s_addr, packet_arp->tpa,
					       PKT_IP_ALEN);
					log_dbg
					    ("arp-reply: target tha=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x tpa=%s",
					     packet_arp->tha[0],
					     packet_arp->tha[1],
					     packet_arp->tha[2],
					     packet_arp->tha[3],
				...
				return tun_write(tun, (uint8_t *) & packet,
						 length, idx);
			}
		default:
	...
	if (ippool_getip(ippool, &ipm, &dst)) {
...
#ifdef ENABLE_SSDP
		if (fwd_ssdp(&dst, ipph, udph, pb, ethhdr))
			return 0;
#endif
		...
		if (_options.debug)
			log_dbg("dropping packet with unknown destination: %s",
				inet_ntoa(dst));
   ...
   */
...
#if(_debug_ > 1)
			log_dbg
			    ("SNAT anyip replace %s back to %s; snat was: %s",
	...
	case DNPROT_LAYER3:
#endif
		dhcp_data_req((struct dhcp_conn_t *)appconn->dnlink, pb,
			      ethhdr);
		break;
		      ...
		      struct sockaddr_in *baddress, struct redir_conn_t *conn)
{
	...
	if ((appconn = (struct app_conn_t *)ipm->peer) == NULL ||
		...
		log_warn(0, "No peer protocol defined app-null=%d",
			 appconn == 0);
		return -1;
	}
	 ...
	 */
	{
		...
		for (n = 0; n < DHCP_DNAT_MAX; n++) {
			...
			if (dhcpconn->dnat[n].src_port == address->sin_port) {
				if (dhcpconn->dnat[n].dst_port ==
				    htons(DHCP_HTTPS)
#ifdef ENABLE_UAMUIPORT
				    || (_options.uamuissl
					&& dhcpconn->dnat[n].dst_port ==
					htons(_options.uamuiport))
#endif
				    ) {
#if(_debug_)
		      ...
		      struct app_conn_t *appconn, char force)
{
...
#ifdef ENABLE_GARDENACCOUNTING
	if (_options.uamgardendata &&
	    has_new_location && restart_accounting && appconn->hisip.s_addr) {
	...
	if (appconn->s_state.authenticated == 1 &&
		...
		if (appconn->s_params.sessiontimeout) {
			uint32_t sessiontime =
			    mainclock_diffu(appconn->s_state.start_time);
		...
		if (appconn->s_state.authenticated == 1)
			acct_req(ACCT_USER, appconn,
				 RADIUS_STATUS_TYPE_INTERIM_UPDATE);

#ifdef ENABLE_GARDENACCOUNTING
		if (_options.uamgardendata)
			acct_req(ACCT_GARDEN, appconn,
				 RADIUS_STATUS_TYPE_INTERIM_UPDATE);
#endif
			 ...
			 struct app_conn_t *appconn, char force)
{
	...
	if (_options.location_copy_called) {
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_CALLED_STATION_ID, 0, 0, 0)) {
			if (attr->l - 2 <
			    sizeof(appconn->s_state.redir.called) - 1) {
				appconn->s_state.redir.calledlen = attr->l - 2;
				memcpy(appconn->s_state.redir.called, attr->v.t,
				       appconn->s_state.redir.calledlen);
				appconn->s_state.redir.called[appconn->s_state.
							      redir.calledlen] =
				    0;
			}
		}
	...
	do {
		attr = NULL;
		if (!radius_getattr
		    (pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC, 0, 0,
		     instance++)) {
	...
	if (_options.proxy_loc[0].attr) {
		...
		for (i = 0; i < PROXYVSA_ATTR_CNT; i++) {
			...
			if (!_options.proxy_loc[i].attr_vsa) {
...
#if(_debug_)
				log_dbg("looking for attr %d",
					_options.proxy_loc[i].attr);
#endif

				if (radius_getattr
				    (pack, &attr, _options.proxy_loc[i].attr, 0,
				     0, 0)) {
					log_dbg("didn't find attr %d",
						_options.proxy_loc[i].attr);
					attr = 0;
				}
			} else {
				...
				if (radius_getattr(pack, &attr,
						   RADIUS_ATTR_VENDOR_SPECIFIC,
						   _options.proxy_loc[i].
						   attr_vsa,
						   _options.proxy_loc[i].attr,
						   0)) {
					log_dbg("didn't find attr %d/%d",
						_options.proxy_loc[i].attr_vsa,
						_options.proxy_loc[i].attr);
					attr = 0;
				}
		...
		if (attr && attr->l > 2) {
			chilli_learn_location(attr->v.t, attr->l - 2, appconn,
					      force);
		}
	}
#endif
...
int accounting_request(struct radius_packet_t *pack, struct sockaddr_in *peer)
{
	...
	if (RADIUS_STATUS_TYPE_ACCOUNTING_ON != status_type &&
		...
		if (!radius_getattr(pack, &nasipattr,
			...
			if ((nasipattr->l - 2) != sizeof(appconn->nasip)) {
				...
				return radius_resp(radius, &radius_pack, peer,
						   pack->authenticator);
			}
			nasip = nasipattr->v.i;
		}
		...
		if (hismacattr) {
			...
			if (_options.layer3) {
				...
				if (dhcp_hashget(dhcp, &dhcpconn, hismac)) {
					if (dhcp_newconn
					    (dhcp, &dhcpconn, hismac)) {
						log_err(0,
							"Out of connections");
						return radius_resp(radius,
								   &radius_pack,
								   peer,
								   pack->
								   authenticator);
				...
				if (!(dhcpconn->peer)) {
					...
					return radius_resp(radius, &radius_pack,
							   peer,
							   pack->authenticator);
				}
			...
			if (chilli_getconn(&appconn, 0, nasip, nasport)) {
				...
				radius_resp(radius, &radius_pack, peer,
					    pack->authenticator);
				return 0;
			}
		} else {
		...
		if (appconn->radiuswait) {
			if (appconn->radiuswait == 2) {
				log_dbg
				    ("Giving up on previous packet.. not dropping this one");
...
int access_request(struct radius_packet_t *pack, struct sockaddr_in *peer)
{
	struct radius_packet_t radius_pack;
	...
	if (!radius_getattr(pack, &hismacattr,
		...
		log_dbg("Calling Station ID is: %.*x", hismacattr->l - 2,
			hismacattr->v.t);

		if ((macstrlen =
		     (size_t) hismacattr->l - 2) >= (RADIUS_ATTR_VLEN - 1)) {
	...
	if (hisipattr) {	/* Find user based on IP address */
		...
		if ((appconn = (struct app_conn_t *)ipm->peer) == NULL ||
		    (dhcpconn =
		     (struct dhcp_conn_t *)appconn->dnlink) == NULL) {
		...
		log_err(0,
			"Framed-IP-Address or Calling-Station-ID required in RADIUS request");
		return radius_resp(radius, &radius_pack, peer,
				   pack->authenticator);
	}
	...
	if (!radius_getattr(pack, &pwdattr, RADIUS_ATTR_USER_PASSWORD, 0, 0, 0)) {
		if (radius_pwdecode
		    (radius, (uint8_t *) pwd, RADIUS_ATTR_VLEN, &pwdlen,
	...
	do {
		eapattr = NULL;
		if (!radius_getattr
		    (pack, &eapattr, RADIUS_ATTR_EAP_MESSAGE, 0, 0,
		     instance++)) {
			if ((resplen + (size_t) eapattr->l - 2) > MAX_EAP_LEN) {
				log(LOG_INFO, "EAP message too long %d %d",
				    resplen, (int)eapattr->l - 2);
				return radius_resp(radius, &radius_pack, peer,
						   pack->authenticator);
			}
			memcpy(resp + resplen, eapattr->v.t,
			       (size_t) eapattr->l - 2);
	...
	if ((appconn->s_state.authenticated == 1) &&
	    ((strlen(appconn->s_state.redir.username) != uidattr->l - 2) ||
	     (memcmp
	      (appconn->s_state.redir.username, uidattr->v.t,
	       uidattr->l - 2)))) {
	...
	/* NAS IP */
	if (!radius_getattr
	    (pack, &nasipattr, RADIUS_ATTR_NAS_IP_ADDRESS, 0, 0, 0)) {
		if ((nasipattr->l - 2) != sizeof(appconn->nasip)) {
			log_err(0, "Wrong length of NAS IP address");
			return radius_resp(radius, &radius_pack, peer,
					   pack->authenticator);
		  ...
		  struct sockaddr_in *peer)
{
	...
	default:
		log_err(0, "Unsupported radius request received: %d",
			pack->code);
		return 0;
	}
}
...
session_disconnect(struct app_conn_t *appconn,
		   struct dhcp_conn_t *dhcpconn, int term_cause)
{
	...
	if (appconn->uplink) {
		...
		if (_options.uamanyip) {
			if (!appconn->natip.s_addr) {
				...
				if (ippool_getip
				    (ippool, &natipm, &appconn->natip) == 0) {
					if (ippool_freeip(ippool, natipm)) {
						log_err(0,
							"ippool_freeip(%s) failed for nat ip!",
							inet_ntoa(appconn->
								  natip));
					}
				}
		...
		if (_options.usetap) {
			...
			if (sockfd > 0) {
				...
				((struct sockaddr_in *)&req.arp_pa)->sin_addr.
				    s_addr = appconn->hisip.s_addr;
				req.arp_flags = ATF_PERM | ATF_PUBL;

				safe_strncpy(req.arp_dev, tuntap(tun).devname,
					     sizeof(req.arp_dev));
		      ...
		      struct app_conn_t *appconn, int reconfig)
{
	...
	/* Interim interval */
	if (!radius_getattr
	    (pack, &attr, RADIUS_ATTR_ACCT_INTERIM_INTERVAL, 0, 0, 0)) {
		params->interim_interval = ntohl(attr->v.i);
		if (params->interim_interval < 60) {
			log_err(0,
				"Received too small radius Acct-Interim-Interval: %d; resettings to default.",
	...
	if (!radius_getattr(pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
		...
		params->maxinputoctets |=
		    ((uint64_t) ntohl(attr->v.i) & 0xffffffff) << 32;

	/* Max output gigawords */
	if (!radius_getattr(pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
			    RADIUS_VENDOR_CHILLISPOT,
			    RADIUS_ATTR_CHILLISPOT_MAX_OUTPUT_GIGAWORDS, 0))
		params->maxoutputoctets |=
		    ((uint64_t) ntohl(attr->v.i) & 0xffffffff) << 32;
...
#include EX_CONFIG_RADIUS_SESSION
	...
	if (tun) {
		...
		if (!radius_getattr(pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
				    RADIUS_VENDOR_CHILLISPOT,
				    RADIUS_ATTR_CHILLISPOT_ROUTE_TO_INTERFACE,
				    0)) {
			char name[256];
...
#ifdef ENABLE_CHILLISPOTCONFIG
	{
		...
		while (!radius_getnextattr(pack, &attr,
			...
			if (len == strlen(uamauth)
			    && !memcmp(val, uamauth, len)) {
				log_dbg("received require-uam-auth");
				params->flags |= REQUIRE_UAM_AUTH;
			} else if (len == strlen(splash)
				   && !memcmp(val, splash, len)) {
				...
				if (len == 5 && !memcmp(name, "reset", 5)) {
					...
					if (appconn && appconn->ptree) {
						patricia_destroy(appconn->ptree,
								 free);
						appconn->ptree = NULL;
					}
#endif
					...
					pass_throughs_from_string(params->
								  pass_throughs,
	...
	if (!radius_getattr(pack, &attr, RADIUS_ATTR_VENDOR_SPECIFIC,
		...
		if (result == 8) {	/* Timezone */
			...
			log_warn(0,
				 "Invalid WISPr-Session-Terminate-Time received: %s",
				 attrs);
		}
	} else if (!reconfig)
		params->sessionterminatetime = 0;
			 ...
			 struct radius_packet_t *pack_req, void *cbp)
{
	...
	if (_options.adminupdatefile) {
		...
		if (!radius_getnextattr(pack, &attr,
			...
			int fd =
			    open(hs_temp, O_RDWR | O_TRUNC | O_CREAT, 0644);

			if (fd > 0) {

				do {
					if (safe_write
					    (fd, attr->v.t, attr->l - 2) < 0
					    || safe_write(fd, "\n", 1) < 0) {
						log_err(errno,
							"adminupdatefile");
						break;
			 ...
			 */
			{
				...
				if (newfd > 0) {
					...
					if (differ) {
						log_dbg
						    ("Writing out new hs.conf file with administraive-user settings");
			...
			struct radius_packet_t *pack_req, void *cbp)
{
	...
	if (!radius_getattr
	    (pack, &hisipattr, RADIUS_ATTR_FRAMED_IP_ADDRESS, 0, 0, 0)
#ifdef ENABLE_DHCPRADIUS
		...
		if (!radius_getattr
		    (pack, &hisipattr, RADIUS_ATTR_FRAMED_IP_NETMASK, 0, 0, 0)
#ifdef ENABLE_DHCPRADIUS
		    || !radius_getattr(pack, &hisipattr,
				       RADIUS_ATTR_VENDOR_SPECIFIC,
				       RADIUS_VENDOR_CHILLISPOT,
				       RADIUS_ATTR_CHILLISPOT_DHCP_IP_NETMASK,
				       0)
#endif
		    ) {
			if ((hisipattr->l - 2) != sizeof(struct in_addr)) {
			...
			log_dbg("Framed IP netmask set to: %s",
				inet_ntoa(hismask));
		}
	} else {
	...
	if (force_ip) {
		if (appconn->uplink) {
			struct ippoolm_t *ipm =
			    (struct ippoolm_t *)appconn->uplink;
			...
			if (hisip.s_addr) {
				...
				if (ipm->addr.s_addr != hisip.s_addr) {
					uint8_t
					    hwaddr[sizeof(dhcpconn->hismac)];
					memcpy(hwaddr, dhcpconn->hismac,
					       sizeof(hwaddr));

					log_dbg("Old ip address freed %s",
						inet_ntoa(ipm->addr));
					log_dbg("Resetting ip address to %s",
						inet_ntoa(hisip));
	...
	if (_options.dhcpradius) {
		...
		if (dhcpconn) {
			if (!radius_getattr(pack, &attr,
					    RADIUS_ATTR_VENDOR_SPECIFIC,
					    RADIUS_VENDOR_CHILLISPOT,
					    RADIUS_ATTR_CHILLISPOT_DHCP_SERVER_NAME,
					    0)) {
				memcpy(dhcpconn->dhcp_opts.sname, attr->v.t,
				       attr->l - 2);
			}
			...
			if (!radius_getattr(pack, &attr,
					    RADIUS_ATTR_VENDOR_SPECIFIC,
					    RADIUS_VENDOR_CHILLISPOT,
					    RADIUS_ATTR_CHILLISPOT_DHCP_FILENAME,
					    0)) {
				memcpy(dhcpconn->dhcp_opts.file, attr->v.t,
				       attr->l - 2);
			}
			...
			if (!radius_getattr(pack, &attr,
					    RADIUS_ATTR_VENDOR_SPECIFIC,
					    RADIUS_VENDOR_CHILLISPOT,
					    RADIUS_ATTR_CHILLISPOT_DHCP_OPTION,
					    0)) {
				memcpy(dhcpconn->dhcp_opts.options, attr->v.t,
				       dhcpconn->dhcp_opts.option_length =
				       attr->l - 2);
			}
			...
			if (!radius_getattr(pack, &attr,
					    RADIUS_ATTR_VENDOR_SPECIFIC,
					    RADIUS_VENDOR_CHILLISPOT,
					    RADIUS_ATTR_CHILLISPOT_DHCP_DNS1,
					    0)) {
			...
			if (!radius_getattr(pack, &attr,
					    RADIUS_ATTR_VENDOR_SPECIFIC,
					    RADIUS_VENDOR_CHILLISPOT,
					    RADIUS_ATTR_CHILLISPOT_DHCP_DNS2,
					    0)) {
				if ((attr->l - 2) == sizeof(struct in_addr)) {
			...
			if (!radius_getattr(pack, &attr,
					    RADIUS_ATTR_VENDOR_SPECIFIC,
					    RADIUS_VENDOR_CHILLISPOT,
					    RADIUS_ATTR_CHILLISPOT_DHCP_GATEWAY,
					    0)) {
				if ((attr->l - 2) == sizeof(struct in_addr)) {
			...
			if (!radius_getattr(pack, &attr,
					    RADIUS_ATTR_VENDOR_SPECIFIC,
					    RADIUS_VENDOR_CHILLISPOT,
					    RADIUS_ATTR_CHILLISPOT_DHCP_DOMAIN,
					    0)) {
				if (attr->l - 2 < DHCP_DOMAIN_LEN) {
					strncpy(dhcpconn->domain,
						(char *)attr->v.t, attr->l - 2);
	...
	if (pack->code == RADIUS_CODE_ACCESS_CHALLENGE) {
		...
		do {
			...
			if (!radius_getattr
			    (pack, &eapattr, RADIUS_ATTR_EAP_MESSAGE, 0, 0,
			     instance++)) {
				if ((appconn->challen + eapattr->l - 2) >
				    MAX_EAP_LEN) {
					log(LOG_INFO,
					    "EAP message too long %d %d",
					    appconn->challen,
					    (int)eapattr->l - 2);
					return dnprot_reject(appconn);
				}
	...
	if (!radius_getattr(pack, &classattr, RADIUS_ATTR_CLASS, 0, 0, 0)) {
		...
		memcpy(appconn->s_state.redir.classbuf, classattr->v.t,
		       classattr->l - 2);
	...
	if (appconn->s_params.sessionterminatetime) {
		if (mainclock_rtdiff(appconn->s_params.sessionterminatetime) >
		    0) {
			log(LOG_WARNING,
			    "WISPr-Session-Terminate-Time in the past received, rejecting");
			return dnprot_reject(appconn);
		}
	}
#ifdef ENABLE_RADPROXY
	...
	if (!radius_getattr(pack, &sendattr, RADIUS_ATTR_VENDOR_SPECIFIC,
			    ...
			    RADIUS_ATTR_MS_MPPE_SEND_KEY, 0)) {
		if (radius_keydecode
		    (radius, appconn->sendkey, RADIUS_ATTR_VLEN,
		     &appconn->sendlen, (uint8_t *) & sendattr->v.t,
		     sendattr->l - 2, pack_req->authenticator, radius->secret,
		     radius->secretlen)) {
	...
	if (!radius_getattr(pack, &recvattr, RADIUS_ATTR_VENDOR_SPECIFIC,
			    ...
			    RADIUS_ATTR_MS_MPPE_RECV_KEY, 0)) {
		if (radius_keydecode
		    (radius, appconn->recvkey, RADIUS_ATTR_VLEN,
		     &appconn->recvlen, (uint8_t *) & recvattr->v.t,
		     recvattr->l - 2, pack_req->authenticator, radius->secret,
		     radius->secretlen)) {
			log_err(0, "radius_keydecode() failed!");
			return dnprot_reject(appconn);
		}
	...
	if (!radius_getattr(pack, &lmntattr, RADIUS_ATTR_VENDOR_SPECIFIC,
		...
		if (radius_pwdecode
		    (radius, appconn->lmntkeys, RADIUS_MPPEKEYSSIZE,
		     &appconn->lmntlen, (uint8_t *) & lmntattr->v.t,
		     lmntattr->l - 2, pack_req->authenticator, radius->secret,
		     radius->secretlen)) {
			log_err(0, "radius_pwdecode() failed");
			return dnprot_reject(appconn);
		}
	...
	if (!radius_getattr(pack, &succattr, RADIUS_ATTR_VENDOR_SPECIFIC,
		...
		if ((succattr->l - 5) != MS2SUCCSIZE) {
			log_err(0, "Wrong length of MS-CHAP2 success: %d",
				succattr->l - 5);
			return dnprot_reject(appconn);
		}
		memcpy(appconn->ms2succ, ((void *)&succattr->v.t) + 3,
		       MS2SUCCSIZE);
	}
#endif
		      ...
		      struct sockaddr_in *peer)
{
	...
	if (!radius_getattr(pack, &sattr, RADIUS_ATTR_ACCT_SESSION_ID, 0, 0, 0))
		if (_options.debug)
			log_dbg
			    ("Session-id present in disconnect. Only disconnecting that session\n");
	...
	for (appconn = firstusedconn; appconn; appconn = appconn->next) {
		...
		if ((strlen(appconn->s_state.redir.username) == uattr->l - 2 &&
			...
			config_radius_session(&appconn->s_params, pack, appconn,
					      0);
		    ...
		    uint8_t * dhcp_pkt, size_t dhcp_len)
{
		...
		if (!conn->is_reserved) {
			...
			if ((_options.macoklen) &&
				...
				if (_options.macallowlocal) {
					...
					safe_strncpy(appconn->s_state.redir.
						     username, mac,
						     USERNAMESIZE);

					if (_options.macsuffix) {
						size_t ulen =
						    strlen(appconn->s_state.
							   redir.username);
						safe_strncpy(appconn->s_state.
							     redir.username +
							     ulen,
							     _options.macsuffix,
							     USERNAMESIZE -
					 ...
					 */
					upprot_getip(appconn, &appconn->reqip,
						     0);

					log_info
					    ("Granted MAC=%s with IP=%s access without radius auth",
					 ...
					 */
					auth_radius(appconn, 0, 0, dhcp_pkt,
						    dhcp_len);
	...
	if (!ipm) {
		...
		if (appconn->dnprot != DNPROT_DHCP_NONE
		    && appconn->hisip.s_addr) {
			log_warn(0,
				 "Requested IP address when already allocated (hisip %s)",
				 inet_ntoa(appconn->hisip));
...
int cb_dhcp_connect(struct dhcp_conn_t *conn)
{
	...
	log(LOG_NOTICE,
	    ...
	    conn->hismac[0], conn->hismac[1], conn->hismac[2], conn->hismac[3],
...
int chilli_getinfo(struct app_conn_t *appconn, bstring b, int fmt)
{
	...
	case LIST_JSON_FMT:
		if (appconn->s_state.authenticated)
			session_json_fmt(&appconn->s_state, &appconn->s_params,
					 b, 0);
		break;
#endif
	default:
		{
			...
			bassignformat(tmp, " %.*s %d %.*s",
				      appconn->s_state.
				      sessionid[0] ? strlen(appconn->s_state.
							    sessionid) : 1,
				      appconn->s_state.sessionid[0] ? appconn->
				      s_state.sessionid : "-",
				      appconn->s_state.authenticated,
				      appconn->s_state.redir.
				      username[0] ? strlen(appconn->s_state.
							   redir.username) : 1,
				      appconn->s_state.redir.
				      username[0] ? appconn->s_state.redir.
				      username : "-");
			bconcat(b, tmp);
...
static void clear_appconn(struct app_conn_t *appconn)
{
	appconn->s_params.bandwidthmaxup =
...
int cb_dhcp_disconnect(struct dhcp_conn_t *conn, int term_cause)
{
	...
	log(LOG_INFO, "DHCP Released MAC=%.2X-%.2X-%.2X-%.2X-%.2X-%.2X IP=%s",
	    conn->hismac[0], conn->hismac[1],
	    conn->hismac[2], conn->hismac[3],
	    conn->hismac[4], conn->hismac[5], inet_ntoa(conn->hisip));
...
{
	struct app_conn_t *appconn =
	    dhcp_get_appconn_pkt(conn, pkt_iphdr(pack), 0);
		...
		if (!skip) {
			if (_options.swapoctets) {
				...
				if (admin_session.s_state.authenticated) {
					...
					admin_session.s_state.input_octets +=
					    len;
				}
			} else {
				...
				if (admin_session.s_state.authenticated) {
					...
					admin_session.s_state.output_octets +=
					    len;
				}
			}
		}
...
int cb_dhcp_eap_ind(struct dhcp_conn_t *conn, uint8_t * pack, size_t len)
{
	...
	if ((appconn->dnprot == DNPROT_DHCP_NONE) ||
		...
		if ((eap->code == 2) &&	/* Response */
			...
			memcpy(appconn->s_state.redir.username, eap->payload,
			       len - 5);
	...
	if (appconn->dnprot != DNPROT_EAPOL) {
		log_warn(0,
			 "Received EAP message, processing for authentication");
		appconn->dnprot = DNPROT_EAPOL;
		return 0;
	}
...
int static uam_msg(struct redir_msg_t *msg)
{
	...
	if (_options.uamlisten.s_addr != _options.dhcplisten.s_addr) {
		msg->mdata.address.sin_addr.s_addr =
		    msg->mdata.address.sin_addr.s_addr & ~(_options.mask.
							   s_addr);
		msg->mdata.address.sin_addr.s_addr |=
		    _options.dhcplisten.s_addr & _options.mask.s_addr;
	}
	...
	if (ippool_getip(ippool, &ipm, &msg->mdata.address.sin_addr)) {
		if (_options.debug)
			log_dbg("UAM login with unknown IP address: %s",
				inet_ntoa(msg->mdata.address.sin_addr));
		return 0;
	}
	...
	if (appconn->s_state.authenticated == 0 || msg->mtype == REDIR_LOGOUT) {
		...
		if (msg->mdata.opt & REDIR_MSG_OPT_REDIR)
			memcpy(&appconn->s_state.redir, &msg->mdata.redir,
			       sizeof(msg->mdata.redir));

		if (msg->mdata.opt & REDIR_MSG_OPT_PARAMS)
			memcpy(&appconn->s_params, &msg->mdata.params,
			       sizeof(msg->mdata.params));
	...
	case REDIR_ABORT:
		...
		log_info("Received UAM abort from IP=%s",
			 inet_ntoa(appconn->hisip));
					...
					int *has_criteria)
{
	...
	if (!appconn && req->d.sess.sessionid[0] != 0) {
		...
		while (aconn) {
			if (!strcmp
			    (aconn->s_state.sessionid, req->d.sess.sessionid)) {
				appconn = aconn;
				break;
			}
			aconn = aconn->next;
	...
	case CMDSOCK_INSPECT:
		{
			...
			log_dbg
			    ("looking to inspect ip=%s/mac=%.2X%.2X%.2X%.2X%.2X%.2X",
			     inet_ntoa(req->ip), req->mac[0], req->mac[1],
			     req->mac[2], req->mac[3], req->mac[4],
			     req->mac[5]);
			...
			if (!appconn && !dhcpconn) {
				...
				if (!appconn)
					appconn =
					    (struct app_conn_t *)dhcpconn->peer;
				...
				if (appconn->s_state.authenticated) {
					sessiontime =
					    mainclock_diffu(appconn->s_state.
							    start_time);
					idletime =
					    mainclock_diffu(appconn->s_state.
							    last_sent_time);
				...
				{
					char buffer[128];
					redir_chartohex(appconn->s_state.redir.
							uamchal, buffer,
							REDIR_MD5LEN);
					bassignformat(tmp, "%20s: %s\n",
						      "challenge", buffer);
					bconcat(s, tmp);
				}
				...
				bassignformat(tmp,
					      ...
					      "user name",
					      appconn->s_state.redir.
					      username[0] ? strlen(appconn->
								   s_state.
								   redir.
								   username) :
					      1,
					      appconn->s_state.redir.
					      username[0] ? appconn->s_state.
					      redir.username : "-");
				bconcat(s, tmp);
				...
				bassignformat(tmp,
					      ...
					      sessiontime,
					      "session timeout",
					      (int)appconn->s_params.
					      sessiontimeout, "idle time",
				...
				bassignformat(tmp, "%20s:", "flags");
				if (appconn->s_params.flags & REQUIRE_UAM_AUTH)
					bcatcstr(tmp, " require-uam-auth");
				if (appconn->s_params.
				    flags & REQUIRE_UAM_SPLASH)
	...
	case CMDSOCK_REM_GARDEN:
		{
			...
			log_dbg("looking to %s to garden ip=%s/sessionid=%s",
				remove ? "remove" : "add", inet_ntoa(req->ip),
				req->d.sess.sessionid);
				...
				while (appconn) {
					if (appconn->inuse &&
					    ((req->ip.s_addr != 0
					     ...
					     ||
					     (!memcmp
					      (appconn->hismac, req->mac,
					       PKT_ETH_ALEN))
					    )) {

						log_dbg
						    ("remote %s garden for session %s",
						     remove ? "rem" : "add",
						     appconn->s_state.
						     sessionid);
	...
	case CMDSOCK_ROUTE_GW:
		{
			if (req->type == CMDSOCK_ROUTE_GW) {
				log_dbg("setting route for idx %d",
					req->d.sess.params.routeidx);
				copy_mac6(tun(tun, req->d.sess.params.routeidx).
					  gwaddr, req->mac);
			} else {
				struct dhcp_conn_t *conn = dhcp->firstusedconn;
				log_dbg("looking to alter session %s",
					inet_ntoa(req->ip));
	...
	case CMDSOCK_ROUTE:
		{
			...
			if (safe_write(sock, b->data, b->slen) == b->slen) {
				for (i = 0; !err && i < tun->_interface_count;
				     i++) {
					char gw[56];
					...
					bassignformat(b,
						      "idx: %d dev: %s %s %.2X-%.2X-%.2X-%.2X-%.2X-%.2X "
					...
					if (safe_write(sock, b->data, b->slen) <
					    0)
						err = 1;
				}
	...
	case CMDSOCK_AUTHORIZE:
		if (dhcp) {
			...
			if (appconn) {
				...
				switch (req->type) {
				case CMDSOCK_LOGIN:
					auth_radius(appconn, uname,
						    req->d.sess.password, 0, 0);
					break;
				case CMDSOCK_AUTHORIZE:
					dnprot_accept(appconn);
					break;
			    ...
			    struct rtmon_route *route)
{
	...
	for (i = 0; i < tun->_interface_count; i++) {
		if (tun->_interfaces[i].ifindex == route->if_index) {
			memcpy(tun->_interfaces[i].gwaddr, route->gwaddr,
			       sizeof(tun->_interfaces[i].gwaddr));
			tun->_interfaces[i].gateway.s_addr =
...
static inline void macauth_reserved()
{
	...
	while (conn) {
		if (conn->is_reserved && conn->peer) {
			appconn = (struct app_conn_t *)conn->peer;
			if (!appconn->s_state.authenticated) {
				auth_radius((struct app_conn_t *)conn->peer, 0,
					    0, 0, 0);
			}
		}
		conn = conn->next;
...
static int session_timeout()
{
	...
	while (conn) {
		...
		if (mainclock_diff(check_conn->s_state.last_sent_time) >
			...
			session_disconnect(check_conn, 0,
					   RADIUS_TERMINATE_CAUSE_LOST_CARRIER);
		}
	}
...
int chilli_main(int argc, char **argv)
{
	...
	if (!_options.foreground) {
		...
		if (daemon(1, 1)) {
			...
			if (!ok) {
				log_err(errno,
					"could not save configuration options! [%s]",
					file2);
				exit(1);
			}
			...
			if (!options_binload(file2)) {
				log_err(errno,
					"could not reload configuration! [%s]",
					file2);
				exit(1);
			}
		}
	...
	if (chilli_fork == 0)
		...
		return chilli_io(ctrl_main_to_io[0],
				 ctrl_io_to_main[1],
				 pkt_main_to_io[0], pkt_io_to_main[1]);
#endif

	chilli_pid = getpid();

	/* This has to be done after we have our final pid */
	log_pid((_options.pidfile
		 && *_options.pidfile) ? _options.pidfile : DEFPIDFILE);
	...
	log_dbg("clock realtime sec %d nsec %d", startup_real.tv_sec,
		startup_real.tv_nsec);
#ifdef CLOCK_MONOTONIC
	if (clock_gettime(CLOCK_MONOTONIC, &startup_mono) < 0) {
		log_err(errno, "getting startup (monotonic) time");
	}
	log_dbg("clock monotonic sec %d nsec %d", startup_mono.tv_sec,
		startup_mono.tv_nsec);
#endif
#endif
	...
	if (ippool_new(&ippool,
		       ...
		       _options.dhcpend,
		       _options.statip,
		       _options.allowdyn, _options.allowstat)) {
	...
	if (dhcp_new(&dhcp,
		     ...
		     &_options.uamlisten, _options.uamport, _options.noc2c)) {
		log_err(0, "Failed to create dhcp listener on %s",
			_options.dhcpif);
		exit(1);
	}
	...
	if (dhcp_set(dhcp, _options.ethers, (_options.debug & DEBUG_DHCP))) {
	...
	if (cmdsock < 0) {
		log_err(errno, "Failed to initialize chilli query socket");
		return -1;
	}
#endif
...
#else
		log_err(0,
			"Feature is not supported; use --enable-chilliradsec");
		_options.radsec = 0;
#endif
...
#else
		log_err(0,
			"Feature is not supported; use --enable-chilliproxy");
#endif
	}
...
#else
		log_err(0,
			"Feature is not supported; use --enable-chilliredir");
		_options.redir = 0;
#endif
	}
#if(_debug_)
	...
	{
#ifdef ENABLE_ACCOUNTING_ONOFF
		acct_req(ACCT_USER, &admin_session,
			 RADIUS_STATUS_TYPE_ACCOUNTING_ON);
#endif
#ifdef HAVE_NETFILTER_COOVA
		if (_options.kname) {
			kmod_coova_clear();
  ...
  /******************************************************************/
	...
	net_select_reg(&sctx,
		       (tun)->_tuntap.fd,
		       SELECT_READ, (select_callback) tun_decaps, tun, 0);
#endif
...
#ifdef USING_IPC_MSG
		if ((msgresult =
		     TEMP_FAILURE_RETRY(msgrcv
					(redir->msgid, (void *)&msg,
					 sizeof(msg.mdata), 0,
					 IPC_NOWAIT))) == -1) {
...
#else
		log_warn(0,
			 "Not stopping sessions! seskeepalive should be used with compile option --enable-binstatusfile");
#endif
	} else {
		killconn();
#ifdef ENABLE_STATFILE
