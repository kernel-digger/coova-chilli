...
openssl_env *initssl()
{
	if (sslenv_svr == 0) {
		...
		openssl_env_init(sslenv_svr =
				 calloc(1, sizeof(openssl_env)), 0, 1);
	}
	return sslenv_svr;
}
...
openssl_env *initssl_cli()
{
	if (sslenv_cli == 0) {
		...
		openssl_env_init(sslenv_cli =
				 calloc(1, sizeof(openssl_env)), 0, 0);
	}
	return sslenv_cli;
}

#ifdef HAVE_OPENSSL
static int openssl_verify_peer_cb(int ok, X509_STORE_CTX * ctx)
{
...
int openssl_use_certificate(openssl_env * env, char *file)
{
	if (file)
		if (SSL_CTX_use_certificate_file
		    (env->ctx, file, SSL_FILETYPE_PEM) > 0)
			return 1;
...
int openssl_use_privatekey(openssl_env * env, char *file)
{
	...
	if (file) {
		if ((err1 =
		     SSL_CTX_use_PrivateKey_file(env->ctx, file,
						 SSL_FILETYPE_PEM)) > 0
...
#if(0)
static RSA *openssl_tmpRSA_cb(SSL * ssl, int export, int len)
{
...
static DH *openssl_tmpDH_cb(SSL * ssl, int export, int len)
{
	openssl_con *con = (openssl_con *) SSL_get_app_data(ssl);
	openssl_env *env = con->env;
	DH *dh = 0;
...
static void openssl_tmp_genkeys(openssl_env * env)
{

	if ((env->tmpKeys[OPENSSL_TMPKEY_RSA512] =
	     RSA_generate_key(512, RSA_F4, NULL, NULL)) == NULL) {
		log_err(errno, "could not generate tmp 512bit RSA key\n");
	}

	if ((env->tmpKeys[OPENSSL_TMPKEY_RSA1024] =
	     RSA_generate_key(1024, RSA_F4, NULL, NULL)) == NULL) {
		log_err(errno, "could not generate tmp 1024bit RSA key\n");
	}

	if ((env->tmpKeys[OPENSSL_TMPKEY_DH512] =
	     openssl_dh_tmpkey(512)) == NULL) {
		log_err(errno, "could not generate tmp 512bit DH key\n");
	}

	if ((env->tmpKeys[OPENSSL_TMPKEY_DH1024] =
	     openssl_dh_tmpkey(1024)) == NULL) {
		log_err(errno, "could not generate tmp 512bit DH key\n");
	}
}
#endif

int _openssl_env_init(openssl_env * env, char *engine, int server)
{
	/*
...
int openssl_env_init(openssl_env * env, char *engine, int server)
{
...
#ifdef HAVE_OPENSSL
	{
		...
		if (_options.sslkeypass) {
			SSL_CTX_set_default_passwd_cb(env->ctx,
						      _openssl_passwd);
		}
		...
		if (!openssl_use_certificate(env, _options.sslcertfile) ||
			...
			log_err(0,
				"failed reading setup sslcertfile and/or sslkeyfile");
			return 0;
		}

		if (_options.sslcafile) {
			if (!openssl_cacert_location
			    (env, _options.sslcafile, 0)) {
	...
	log_dbg("SSL pass: %s",
		_options.sslkeypass ? _options.sslkeypass : "null");
	log_dbg("SSL ca: %s", _options.sslcafile ? _options.sslcafile : "null");
	if (matrixSslReadKeys(&env->keys,
			      _options.sslcertfile,
			      _options.sslkeyfile,
			      _options.sslkeypass, _options.sslcafile) < 0) {
		log_err(errno,
			"could not load ssl certificate or and/or key file");
		return 0;
	}
...
int openssl_check_accept(openssl_con * c, struct redir_conn_t *conn)
{
	...
	if (!SSL_is_init_finished(c->con)) {
		...
		if ((rc = SSL_accept(c->con)) <= 0) {
			...
			if (peer_cert) {
				...
				if (conn)
					conn->s_params.flags |= ADMIN_LOGIN;
				...
				if (_options.debug) {
					...
					char b[512];
					log_dbg
					    ("Debugging: SSL Information:\n");
					cipher = SSL_get_current_cipher(c->con);
					log_dbg
					    ("  Protocol: %s, %s with %.*s bit key\n",
					     SSL_CIPHER_get_version(cipher),
					     (char *)
					     SSL_CIPHER_get_name(cipher),
					     sprintf(b, "%d",
						     EVP_PKEY_bits(pktmp)), b);
					log_dbg("  Subject:  %s\n", subj);
...
int openssl_error(openssl_con * con, int ret, char *func)
{
	...
	if (con->con) {
		...
		log_dbg("SSL: (%s()) %s", func,
			...
			err ==
			SSL_ERROR_WANT_X509_LOOKUP ? "Lookup (continue)" : err
			== SSL_ERROR_SYSCALL ? "Syscall error, abort!" : err ==
			SSL_ERROR_SSL ? "SSL error, abort!" : "Error");
#endif
		switch (err) {
		case SSL_ERROR_NONE:
			return 0;
...
void openssl_shutdown(openssl_con * con, int state)
{
	...
	if (con) {
		switch (state) {
		case 0:
			SSL_set_shutdown(con->con, SSL_RECEIVED_SHUTDOWN);
			break;
		case 1:
			SSL_set_shutdown(con->con, SSL_SENT_SHUTDOWN);
			break;
		case 2:
			SSL_set_shutdown(con->con,
...
int openssl_write(openssl_con * con, char *b, int l, int t)
{
	...
	if (t) {
		...
		if (select(fd + 1, (fd_set *) 0, &wfds, (fd_set *) 0, &tv) ==
		    -1)
			return -1;
		if (!FD_ISSET(fd, &wfds))
			return 0;
